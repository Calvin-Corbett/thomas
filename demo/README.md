...
Canonical benchmark definitions now live under `benchmarks/`. This `demo/` section is legacy compatibility guidance for the older benchmark surface.

Run a direct before/after benchmark where:
- baseline = auto-selected competitor lane (`tool-agent` for tool-required packs, `raw` otherwise)
...
- `summary.json` (metric-first summary)
