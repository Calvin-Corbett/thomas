...
class AgentSafetyConfig:
    ...
    def worktree_maintenance_ignore_prefixes(self) -> list[str]:
        return list(
            self._get(
                "worktree_hygiene",
                "maintenance_ignore_prefixes",
                default=[
                    "output/",
                    "runtime/",
                    "artifacts/",
                    ...
                    "playwright-mcp/",
                    "demo/agentic-runs/",
                    "demo/task_pack.agentic.",
                ],
            )
        )

    def worktree_maintenance_log_file(self) -> str:
        return str(self._get("worktree_hygiene", "maintenance_log_file", default="@state/maintenance/events.jsonl"))

    def worktree_maintenance_audit_log_file(self) -> str:
        return str(self._get("worktree_hygiene", "maintenance_audit_log_file", default="@state/maintenance/audit.jsonl"))

    def worktree_max_auto_checkpoints_per_hour(self) -> int:
        return int(self._get("worktree_hygiene", "max_auto_checkpoints_per_hour", default=6))

    def worktree_max_checkpointed_lines_per_hour(self) -> int:
        return int(self._get("worktree_hygiene", "max_checkpointed_lines_per_hour", default=6000))

    def worktree_max_checkpoint_failures_before_stop(self) -> int:
        return int(self._get("worktree_hygiene", "max_checkpoint_failures_before_stop", default=3))
    ...
    def protected_runtime_sensitive_files(self) -> list[str]:
        return list(self._get("protected", "runtime_sensitive_files", default=[]))

    def protected_release_sensitive_files(self) -> list[str]:
        return list(self._get("protected", "release_sensitive_files", default=[]))

    def protected_immutable_policy_files(self) -> list[str]:
        runtime_sensitive = {str(path) for path in self.protected_runtime_sensitive_files()}
        release_sensitive = {str(path) for path in self.protected_release_sensitive_files()}
        return [
            path
            for path in self.protected_policy_files()
            if str(path) not in runtime_sensitive and str(path) not in release_sensitive
        ]
    ...
    def all_protected_files(self) -> set[str]:
        return set(
            self.protected_immutable_policy_files()
            + self.protected_runtime_sensitive_files()
            + self.protected_release_sensitive_files()
