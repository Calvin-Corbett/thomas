...
BLOCKED_TRACKED_EXACT = {
    ...
    ".meta_unreferenced_docs.txt",
    "__codex_jsrepl_test.txt",
    "__codex_marker.txt",
    "private_comparison_notes.docx",
    "claude-code-vs-thomas-comparison.html",
    ...
    "thomas_search.db",
    "thomas-feature-inventory.html",
    "thomas-repo-review.html",
    "private_subcommand_audit.json",
...
PERSONAL_PATH_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("windows_user_path", re.compile(r"\b[A-Za-z]:\\Users\\[A-Za-z0-9_.-]+\\")),
    ("mac_user_path", re.compile(r"/Users/[A-Za-z0-9_.-]+/")),
    ("linux_home_path", re.compile(r"/home/[a-z][a-z0-9_.-]+/")),
)
PERSONAL_PATH_ALLOWLIST_FRAGMENTS = (
    "/home/node/",
    "/home/user/",
    "/Users/<user>/",
    "C:/Users/<user>/",
...
def _is_allowed_personal_path_line(line: str) -> bool:
    return any(fragment in str(line or "") for fragment in PERSONAL_PATH_ALLOWLIST_FRAGMENTS)
...
def _check_blocked_tracked_files(tracked: Sequence[str]) -> list[str]:
    ...
    for raw in tracked:
        ...
        if any(lower.endswith(suffix) for suffix in BLOCKED_TRACKED_SUFFIXES):
            violations.append(path)
            continue
        if any(lower.startswith(prefix) for prefix in BLOCKED_TRACKED_PREFIXES):
            violations.append(path)
    return sorted(set(violations))


def _scan_for_personal_paths(repo_root: Path, tracked: Sequence[str]) -> list[dict[str, Any]]:
    findings: list[dict[str, Any]] = []
    skipped_suffixes = (
        ".db",
        ".docx",
    ...
    for rel_path in tracked:
        ...
        if not normalized or normalized.lower().endswith(skipped_suffixes):
            continue
        ...
        try:
            ...
            continue
        for index, line in enumerate(text.splitlines(), start=1):
            if _is_allowed_personal_path_line(line):
                continue
            for name, pattern in PERSONAL_PATH_PATTERNS:
                if pattern.search(line):
                    findings.append(
                        {
                            "file": normalized,
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    if blocked:
        errors.append(
            "blocked sensitive/public-private files are tracked: "
            + ", ".join(blocked[:8])
    ...
    personal_path_findings = _scan_for_personal_paths(repo_root, tracked)
    if personal_path_findings:
        sample = personal_path_findings[:5]
        preview = "; ".join(f"{item['file']}:{item['line']} [{item['pattern']}]" for item in sample)
        suffix = f"; ... (+{len(personal_path_findings) - 5} more)" if len(personal_path_findings) > 5 else ""
        errors.append(f"personal local paths detected: {preview}{suffix}")
    ...
    payload: dict[str, Any] = {
        ...
        "summary": {
            ...
            "personal_path_finding_count": len(personal_path_findings),
    ...
    if personal_path_findings:
        payload["personal_path_findings"] = personal_path_findings[:20]
