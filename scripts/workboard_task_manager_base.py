...
try:
    from scripts.workboard_paths import (
        ...
        problem_root_token_for,
        task_plan_root_token_for,
    )
except ImportError:  # pragma: no cover
    from workboard_paths import default_workboard_path, problem_root_token_for, task_plan_root_token_for  # type: ignore
...
DEFAULT_PLAN_ROOT = task_plan_root_token_for(DEFAULT_WORKBOARD, repo_root=ROOT)
DEFAULT_PROBLEM_ROOT = problem_root_token_for(DEFAULT_WORKBOARD, repo_root=ROOT)
...
DEFAULT_MAX_DISPATCH_PER_CYCLE = 1
...
def _row_scope_overlap(scope: str, row_scopes: Sequence[str]) -> bool:
    ...
    return any(_norm(row_scope) == scope_key for row_scope in row_scopes)
