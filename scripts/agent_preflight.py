...
from scripts.agent_maintenance import attempt_maintenance_checkpoint, maintenance_quota_status  # noqa: E402
from scripts.agent_safety_config import load_config  # noqa: E402
from scripts.check_repo_hygiene import evaluate_worktree_change_budget  # noqa: E402
...
AUTO_MAINTENANCE_CHECKPOINT_ENV = "THOMAS_AUTO_MAINTENANCE_CHECKPOINT"
MAINTENANCE_AGENT_ENV_KEYS = ("THOMAS_AGENT_ID", "AGENT_ID")
...
def _auto_maintenance_checkpoint_enabled() -> bool:
    return _is_truthy(os.getenv(AUTO_MAINTENANCE_CHECKPOINT_ENV, ""))


def _maintenance_agent_id(explicit_agent: str = "") -> str:
    if str(explicit_agent or "").strip():
        return str(explicit_agent).strip()
    for key in MAINTENANCE_AGENT_ENV_KEYS:
...
def _maintenance_recovery_summary(maintenance: dict[str, Any]) -> str:
    summary = str(maintenance.get("recovery_summary") or "").strip()
    if summary:
        return summary
    checkpoint_attempt = maintenance.get("checkpoint_attempt")
    if isinstance(checkpoint_attempt, dict):
        return str(checkpoint_attempt.get("recovery_summary") or "").strip()
    return ""


def _maintenance_recovery_steps(maintenance: dict[str, Any]) -> list[str]:
    steps = [str(step or "").strip() for step in list(maintenance.get("recovery_steps") or []) if str(step or "").strip()]
    if steps:
        return steps
    checkpoint_attempt = maintenance.get("checkpoint_attempt")
    if isinstance(checkpoint_attempt, dict):
        return [
            str(step or "").strip()
            for step in list(checkpoint_attempt.get("recovery_steps") or [])
...
def _result(
    ...
    *,
    ...
    summary = str(recovery_summary or "").strip()
    if summary:
        payload["recovery_summary"] = summary
    steps = [str(step or "").strip() for step in list(recovery_steps or []) if str(step or "").strip()]
    if steps:
        payload["recovery_steps"] = steps
    return payload
...
def _check_worktree_clean(
    root: Path,
    *,
    auto_maintenance_checkpoint: bool | None = None,
    ...
    if dirty_lines:
        ...
        if total_changed_lines > threshold:
            maintenance = maintenance_quota_status(root, total_changed_lines=total_changed_lines)
            blocked_reason = str(maintenance.get("blocked_reason") or "").strip()
            should_attempt_checkpoint = (
                bool(auto_maintenance_checkpoint)
                if auto_maintenance_checkpoint is not None
                else _auto_maintenance_checkpoint_enabled()
            )
            resolved_maintenance_agent = _maintenance_agent_id(maintenance_agent)
            if should_attempt_checkpoint and maintenance.get("can_attempt_checkpoint"):
                checkpoint_attempt = attempt_maintenance_checkpoint(
                    root=root,
                    agent=resolved_maintenance_agent,
                    total_changed_lines=total_changed_lines,
                )
                maintenance["checkpoint_attempt"] = checkpoint_attempt
                if checkpoint_attempt.get("ok"):
                    post_checkpoint = _check_worktree_clean(
                        root,
                        auto_maintenance_checkpoint=False,
                        maintenance_agent="",
                    )
                    if post_checkpoint.get("status") == "ok":
                        return _result(
                            "worktree-clean",
                            "ok",
                            "Automatic maintenance checkpoint succeeded and the worktree is now clean.",
                            user_action="Normal edits may resume.",
                            mode="maintenance",
                            maintenance=maintenance,
                            recovery_summary=_maintenance_recovery_summary(maintenance),
                            recovery_steps=_maintenance_recovery_steps(maintenance),
                        )
                    post_maintenance = (
                        dict(post_checkpoint.get("maintenance"))
                        if isinstance(post_checkpoint.get("maintenance"), dict)
                        else {}
                    )
                    post_maintenance["checkpoint_attempt"] = checkpoint_attempt
                    post_checkpoint["maintenance"] = post_maintenance
                    return post_checkpoint
                attempt_message = str(checkpoint_attempt.get("message") or "").strip()
                blocked_reason = attempt_message or blocked_reason
                maintenance["blocked_reason"] = blocked_reason
            recovery_summary = _maintenance_recovery_summary(maintenance)
            recovery_steps = _maintenance_recovery_steps(maintenance)
            user_action = (
                "Enter maintenance mode: attempt a private-checkpoint commit via "
                f'`{maintenance["suggested_checkpoint_command"]}` '
                "or divert risky runtime changes into green verification first. "
                "Only resume normal edits after the checkpoint or green pass succeeds."
            )
            if recovery_summary:
                user_action = f"Follow maintenance recovery plan: {recovery_summary}."
            if blocked_reason:
                user_action = (
                    "Maintenance quota is exhausted. Stop new implementation work, divert risky runtime changes into "
                    "green verification, or wait for the maintenance window to reset. "
                    f"Blocked reason: {blocked_reason}."
                )
                if recovery_summary:
                    user_action += f" Recovery plan: {recovery_summary}."
            return _result(
                "worktree-clean",
                "blocked",
                (
                    "Worktree maintenance mode required: "
                    f"{summary}, {total_changed_lines} changed lines exceeds "
                    f"max_uncommitted_changed_lines={threshold}."
                    + (f" Maintenance checkpoint blocked: {blocked_reason}." if blocked_reason else "")
                ),
                user_action=user_action,
                mode="maintenance",
                maintenance=maintenance,
        ...
        return _result(
            ...
            user_action=(
                "Stop before normal implementation work. Enter maintenance mode to checkpoint or clean the current "
                "work first, or use an explicit dirty-worktree override for cleanup/remediation lanes only."
            ),
            mode="maintenance",
        )
    ...
    *,
    ...
    checks = [
        ...
        _check_worktree_clean(
            resolved_root,
            auto_maintenance_checkpoint=auto_maintenance_checkpoint,
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--auto-maintenance-checkpoint",
        action="store_true",
        help="Attempt an automatic private-checkpoint commit when maintenance mode triggers.",
    )
    parser.add_argument("--maintenance-agent", default="", help="Agent id to use for automatic maintenance checkpoints.")
...
def _text_output(payload: dict[str, Any]) -> str:
    ...
    for check in payload["checks"]:
        ...
        lines.append(f"  - {check['status']} {check['id']}{mode_suffix}: {check['message']}{suffix}")
        recovery_summary = str(check.get("recovery_summary") or "").strip()
        recovery_steps = [
            str(step or "").strip()
            for step in list(check.get("recovery_steps") or [])
            if str(step or "").strip()
        ]
        if recovery_summary:
        ...
        if recovery_steps:
            ...
            for step in recovery_steps:
                lines.append(f"      - {step}")
        maintenance = check.get("maintenance") if isinstance(check.get("maintenance"), dict) else None
        if maintenance:
            blocked_reason = str(maintenance.get("blocked_reason") or "").strip()
            lines.append(
                "    maintenance: "
                f"can_attempt_checkpoint={maintenance.get('can_attempt_checkpoint')} "
                f"remaining_auto_checkpoints={maintenance.get('remaining_auto_checkpoints')} "
                f"remaining_checkpointed_lines={maintenance.get('remaining_checkpointed_lines')}"
            )
            checkpoint_attempt = (
                maintenance.get("checkpoint_attempt") if isinstance(maintenance.get("checkpoint_attempt"), dict) else None
            )
            if checkpoint_attempt:
                lines.append(
                    "    checkpoint_attempt: "
                    f"ok={checkpoint_attempt.get('ok')} "
                    f"attempted={checkpoint_attempt.get('attempted')} "
                    f"message={checkpoint_attempt.get('message', '')}"
                )
            if blocked_reason:
                lines.append(f"    maintenance_blocked_reason: {blocked_reason}")
    return "\n".join(lines)
...
def run(argv: list[str] | None = None) -> int:
    ...
    payload = evaluate_preflight(
        cwd=cwd,
        auto_maintenance_checkpoint=bool(args.auto_maintenance_checkpoint),
        maintenance_agent=str(args.maintenance_agent or "").strip(),
    )
    if args.json:
