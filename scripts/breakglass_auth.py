...
BREAKGLASS_TOGGLE_RECEIPT_TTL_S: Final[float] = 120.0
SCOPE_FALLBACK_RECEIPT_TTL_S: Final[float] = 300.0
...
@dataclass(frozen=True)
class BreakglassToggleReceipt:
    actor: str
    method: str | None = None


@dataclass(frozen=True)
class ScopeFallbackReceipt:
    actor: str
    agent: str
    scopes: tuple[str, ...]
    reason: str
...
_BREAKGLASS_TOGGLE_RECEIPTS: dict[str, tuple[float, str, str | None]] = {}
...
def _scope_fallback_receipts_dir() -> Path:
    return _state_root() / "scope_fallback" / "receipts"


def _normalize_scope_fallback_scopes(scopes: list[str] | tuple[str, ...]) -> tuple[str, ...]:
    normalized = {
        str(scope or "").strip().replace("\\", "/").strip("/")
        for scope in scopes
        if str(scope or "").strip().replace("\\", "/").strip("/")
    }
...
def _build_breakglass_toggle_confirmation_copy(*, enabled: bool, current_user: str) -> tuple[str, str, str]:
    action = "enable" if enabled else "disable"
    protection = "Protected Override Approval"
    content = "\n".join(
        [
            f"Account: {current_user or 'current signed-in user'}",
            f"Requested change: {action} {protection}",
            "",
            (
                "This setting controls whether Thomas can even request a protected override. "
                "Changing it requires fresh Windows sign-in approval."
            ),
            "",
            "Continue to open the Windows sign-in prompt.",
        ]
    )
    return WINDOWS_CONFIRMATION_CAPTION, f"Approve {protection} change?", content


def _prune_breakglass_toggle_receipts(*, now: float | None = None) -> None:
    current = time.monotonic() if now is None else float(now)
    expired = [
        key for key, (expires_at, _actor, _method) in _BREAKGLASS_TOGGLE_RECEIPTS.items() if expires_at <= current
    ]
    for key in expired:
        _BREAKGLASS_TOGGLE_RECEIPTS.pop(key, None)


def _issue_breakglass_toggle_receipt(*, actor: str, method: str | None) -> str:
    _prune_breakglass_toggle_receipts()
    receipt = secrets.token_urlsafe(24)
    _BREAKGLASS_TOGGLE_RECEIPTS[receipt] = (
        time.monotonic() + BREAKGLASS_TOGGLE_RECEIPT_TTL_S,
        str(actor or "").strip(),
...
def consume_breakglass_toggle_receipt(receipt: str | None) -> BreakglassToggleReceipt | None:
    key = str(receipt or "").strip()
    if not key:
        return None
    _prune_breakglass_toggle_receipts()
    entry = _BREAKGLASS_TOGGLE_RECEIPTS.pop(key, None)
    if entry is None:
        return None
    expires_at, actor, method = entry
    if expires_at <= time.monotonic():
        return None
    actor_value = str(actor or "").strip()
    if not actor_value:
        return None
    return BreakglassToggleReceipt(actor=actor_value, method=method)


def _issue_scope_fallback_receipt(
    *,
    actor: str,
    method: str | None,
    agent: str,
    ...
    payload = {
        ...
        "scopes": list(_normalize_scope_fallback_scopes(scopes)),
        "reason": str(reason or "").strip(),
        "expires_at": time.time() + SCOPE_FALLBACK_RECEIPT_TTL_S,
    }
    receipts_dir = _scope_fallback_receipts_dir()
    receipts_dir.mkdir(parents=True, exist_ok=True)
    (receipts_dir / f"{receipt}.json").write_text(json.dumps(payload, sort_keys=True), encoding="utf-8")
    return receipt


def consume_scope_fallback_receipt(
    receipt: str | None,
    *,
    agent: str,
    scopes: tuple[str, ...],
    reason: str,
) -> ScopeFallbackReceipt | None:
    key = str(receipt or "").strip()
    if not key:
        return None
    path = _scope_fallback_receipts_dir() / f"{key}.json"
    if not path.exists():
        return None
    try:
    ...
    stored_agent = str(payload.get("agent") or "").strip()
    stored_reason = str(payload.get("reason") or "").strip()
    stored_method = str(payload.get("method") or "").strip() or None
    raw_scopes = payload.get("scopes")
    if not actor or not stored_agent or not stored_reason or not isinstance(raw_scopes, list):
        return None
    expected_scopes = _normalize_scope_fallback_scopes(scopes)
    actual_scopes = _normalize_scope_fallback_scopes(tuple(str(item or "") for item in raw_scopes))
    if stored_agent != str(agent or "").strip():
        return None
    if stored_reason != str(reason or "").strip():
        return None
    if actual_scopes != expected_scopes:
        return None
    return ScopeFallbackReceipt(
        actor=actor,
        agent=stored_agent,
...
def authorize_scope_fallback(
    *,
    ...
    normalized_scopes = _normalize_scope_fallback_scopes(scopes)
    if not normalized_agent:
        return BreakglassAuthorization(
            ok=False,
            message="scope fallback authorization requires an agent id",
            method="invalid-request",
        )
    if not normalized_scopes:
        return BreakglassAuthorization(
            ok=False,
            message="scope fallback authorization requires at least one scope",
            method="invalid-request",
        )
    if not normalized_reason:
        return BreakglassAuthorization(
            ok=False,
            message="scope fallback authorization requires a short approval reason",
            method="invalid-request",
        )

    scope_preview = ", ".join(normalized_scopes[:3])
    if len(normalized_scopes) > 3:
        scope_preview += ", ..."
    result = authorize_breakglass(
        purpose="scope fallback approval",
        agent=normalized_agent,
        ticket="SCOPE-FALLBACK",
        reason=(
            "Approve explicit scoped fallback for paths: "
            f"{scope_preview}\n\nReason: {normalized_reason}"
        ),
        skip_hooks=["workboard_changed_files", "workboard_agent_claim"],
    )
    if not result.ok:
        return result

    actor = str(result.actor or "").strip()
    if not actor:
        return BreakglassAuthorization(
            ok=False,
            message="scope fallback approval completed without an authenticated actor",
            method=result.method,
        )

    receipt = _issue_scope_fallback_receipt(
        actor=actor,
        method=result.method,
        agent=normalized_agent,
...
def authorize_breakglass_toggle(*, enabled: bool) -> BreakglassAuthorization:
    if os.name != "nt":
        return BreakglassAuthorization(
            ok=False,
            message="breakglass preference changes are only supported on Windows interactive sessions",
            method="unsupported-platform",
        )
    current_user = _current_windows_sam_name()
    if not current_user:
    ...
    confirm_title, confirm_instruction, confirm_content = _build_breakglass_toggle_confirmation_copy(
    ...
    if not approved:
        return BreakglassAuthorization(
            ok=False,
            message="breakglass preference change cancelled before Windows sign-in",
            actor=current_user,
            method=WINDOWS_CREDENTIAL_METHOD,
            cancelled=True,
        )
    ...
    receipt = _issue_breakglass_toggle_receipt(
        actor=str(result.actor or current_user).strip() or current_user,
        method=result.method,
    )
    return BreakglassAuthorization(
        ok=True,
