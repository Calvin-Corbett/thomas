...
try:
    from scripts.workboard_board_sections import (
        NONE_ENTRY,
        _bullet_indices,
        _ensure_active_tasks_section,
        _find_active_tasks_section,
        _find_claim_section,
        _find_issues_section,
        _find_section,
    ...
    from workboard_board_sections import (  # type: ignore
        NONE_ENTRY,
        _bullet_indices,
        _ensure_active_tasks_section,
        _find_active_tasks_section,
...
try:
    from scripts.workboard_paths import (
        coordination_dir_for,
        default_workboard_path,
        repo_root_for,
        temp_task_creator_scope_for,
    )
except ImportError:  # pragma: no cover
...
try:
    from scripts.workboard_locking import _atomic_write, _file_lock
except ImportError:  # pragma: no cover
    from workboard_locking import _atomic_write, _file_lock  # type: ignore
...
COORDINATION_DIR = coordination_dir_for(DEFAULT_WORKBOARD, repo_root=ROOT)
...
TEMP_TASK_CREATOR_SCOPE = temp_task_creator_scope_for(DEFAULT_WORKBOARD, repo_root=ROOT)
...
def _validate_dirty_release_reason(reason: str) -> str:
    ...
    if len(reason) < DEFAULT_DIRTY_RELEASE_REASON_MIN_LEN:
        raise ValueError(
            "dirty release reason is required "
            f"(provide --dirty-release-reason with at least {DEFAULT_DIRTY_RELEASE_REASON_MIN_LEN} characters)"
        )
    return reason
...
def _presence_repo_root(workboard_path: Path) -> Path:
    target_root = repo_root_for(workboard_path, repo_root=ROOT)
    if agent_presence is None:
        return target_root
    try:
        detected = agent_presence.repo_root()
        if detected:
            detected_path = Path(str(detected)).resolve()
            if detected_path == target_root:
...
def _active_task_id_for_agent(agent: str) -> str:
    return f"{_task_id_from_agent(agent)}-task"
...
def _is_temp_task_creator_task(task: str) -> bool:
    token = str(task or "").strip().lower()
    return token.startswith(str(TEMP_TASK_CREATOR_TASK_TAG).strip().lower())
...
def _detect_agent_default() -> str | None:
    for key in ("AGENT_ID", "THOMAS_AGENT_ID", "CODEX_AGENT_ID"):
        value = str(os.getenv(key) or "").strip()
        if value:
            return value
    for key in ("THOMAS_AGENT_NAME", "CODEX_AGENT_NAME", "AGENT_NAME"):
        value = str(os.getenv(key) or "").strip()
        if value:
            return value
...
def _resolve_agent(agent: str | None) -> str:
    ...
    if not agent:
        ...
        if detected:
            return detected
        raise ValueError("agent is required; unable to auto-detect agent; pass --agent")
    return agent
