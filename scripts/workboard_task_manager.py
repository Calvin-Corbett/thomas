...
try:
    from scripts.workboard_paths import problem_root_token_for, resolve_workboard_path, task_plan_root_token_for
except ImportError:  # pragma: no cover
    from workboard_paths import problem_root_token_for, resolve_workboard_path, task_plan_root_token_for  # type: ignore
...
__all__ = ["run", "workboard_claim", "dispatch_idle_agents_once"]
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    plan_root = str(args.plan_root).strip() or DEFAULT_PLAN_ROOT
    problem_root = str(args.problem_root).strip() or DEFAULT_PROBLEM_ROOT
    if plan_root == DEFAULT_PLAN_ROOT:
        plan_root = task_plan_root_token_for(workboard_path, repo_root=ROOT)
    if problem_root == DEFAULT_PROBLEM_ROOT:
        problem_root = problem_root_token_for(workboard_path, repo_root=ROOT)
