...
def _suggest_split_paths(rel: str) -> list[str]:
    path = Path(rel)
    stem = path.stem
    ...
    if suffix == ".py":
        ...
        candidates = [
            ...
            parent / f"{stem}.types{suffix}",
            parent / f"{stem}.actions{suffix}",
        ]
    elif suffix == ".css":
        candidates = [
    ...
    *,
            ...
            violations.append(
                {
                    ...
                    "is_new_file": prior == 0,
                    "suggested_split_paths": _suggest_split_paths(rel),
                }
            )
    ...
    if json_output:
        ...
        for v in violations:
            ...
            for suggestion in list(v.get("suggested_split_paths") or [])[:3]:
                print(f"      split: {suggestion}")
        print("  Next step: move the added logic into sibling modules and restage the smaller entry file.")
