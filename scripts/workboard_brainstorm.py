...
def session_status(
    ...
    *,
    ...
    session_clean = _sanitize("session_id", session_id)
    lines = workboard_path.read_text(encoding="utf-8").splitlines()
    sessions_section = _find_section(lines, heading_prefix=SESSIONS_HEADING)
    notes_section = _find_section(lines, heading_prefix=NOTES_HEADING)
    ...
    row = _session_lookup(sessions, session_clean)
    if row is None:
        return False, {"error": f"brainstorm session `{session_clean}` not found"}
    invited = _agent_set(row.get("invited_agents", "none"))
    responded = _agent_set(row.get("responded_agents", "none"))
    pending = sorted(invited - responded) if invited else []
    rows = [dict(item) for item in notes if _norm(item.get("session_id", "")) == _norm(session_clean)]
    return True, {
        "session": dict(row),
        "notes": rows,
        "note_count": len(rows),
...
def list_sessions(
    ...
    *,
    ...
    lines = workboard_path.read_text(encoding="utf-8").splitlines()
    section = _find_section(lines, heading_prefix=SESSIONS_HEADING)
    if section is None:
        return True, {"sessions": [], "session_count": 0}
    rows, errors = _load_sessions(lines, section)
    if errors:
        return False, {"error": "brainstorm session parse failed", "violations": errors}
    wanted = _norm(state)
    out: list[dict[str, str]] = []
    for row in rows:
        if wanted and _norm(row.get("state", "")) != wanted:
            continue
        out.append(dict(row))
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.start:
            initiator = str(args.initiator or args.agent or args.facilitator or DEFAULT_FACILITATOR).strip()
            ok, payload = start_session(
                workboard_path,
                task_id=str(args.task_id).strip(),
                summary=str(args.summary).strip(),
                objective=str(args.objective).strip(),
                initiator=initiator,
                facilitator=str(args.facilitator).strip() or DEFAULT_FACILITATOR,
                all_hands=bool(args.all_hands),
                invite_agents=_split_agents(str(args.invite_agents).strip()),
