#!/usr/bin/env python3
"""Manage active agent claims in the current project workboard."""
...
try:
    from . import workboard_claim_dispatch as _dispatch_mod
    from . import workboard_claim_ops as _ops_mod
    from . import workboard_claim_utils as _utils_mod
    from .workboard_claim_utils import (
        ...
        DEFAULT_WORKBOARD,
        LOCK_FILE,
        RELEASE_OVERRIDE_AUDIT_LOG,
        _claimed_scope_dirty_paths,
        _detect_agent_default,
        _detect_branch_name,
        _file_lock,
        _find_active_tasks_section,
        _find_claim_section,
        _is_temp_task_creator_task,
        _resolve_agent,
    ...
    import workboard_claim_dispatch as _dispatch_mod  # type: ignore
    import workboard_claim_ops as _ops_mod  # type: ignore
    import workboard_claim_utils as _utils_mod  # type: ignore
...
agent_presence = _utils_mod.agent_presence
_send_temp_task_creator_notice = _dispatch_mod._send_temp_task_creator_notice
_is_temp_task_creator_task = _utils_mod._is_temp_task_creator_task
release_temp_task_creator = _dispatch_mod.release_temp_task_creator
suggest_delegation = _dispatch_mod.suggest_delegation


def _sync_public_overrides() -> None:
    _utils_mod.CLAIM_OVERRIDE_AUDIT_LOG = CLAIM_OVERRIDE_AUDIT_LOG
    _utils_mod.RELEASE_OVERRIDE_AUDIT_LOG = RELEASE_OVERRIDE_AUDIT_LOG
    _utils_mod.LOCK_FILE = LOCK_FILE
    _utils_mod._claimed_scope_dirty_paths = _claimed_scope_dirty_paths
    _utils_mod._detect_agent_default = _detect_agent_default
    _utils_mod._detect_branch_name = _detect_branch_name
    _utils_mod._find_active_tasks_section = _find_active_tasks_section
    _utils_mod._find_claim_section = _find_claim_section
    _utils_mod._resolve_display_name = _resolve_display_name
    _utils_mod._scope_guard_supported = _scope_guard_supported

    _ops_mod.CLAIM_OVERRIDE_AUDIT_LOG = CLAIM_OVERRIDE_AUDIT_LOG
    _ops_mod.RELEASE_OVERRIDE_AUDIT_LOG = RELEASE_OVERRIDE_AUDIT_LOG
    _ops_mod.LOCK_FILE = LOCK_FILE
    _ops_mod._claimed_scope_dirty_paths = _claimed_scope_dirty_paths
    _ops_mod._file_lock = _file_lock
    _ops_mod._find_active_tasks_section = _find_active_tasks_section
    _ops_mod._find_claim_section = _find_claim_section
    _ops_mod._resolve_display_name = _resolve_display_name
    _ops_mod._scope_guard_supported = _scope_guard_supported

    _dispatch_mod._send_temp_task_creator_notice = _send_temp_task_creator_notice
    _dispatch_mod.claim = claim
    _dispatch_mod.release = release


def claim(*args, **kwargs):
    _sync_public_overrides()
    return _ops_mod.claim(*args, **kwargs)


def list_claims(*args, **kwargs):
    _sync_public_overrides()
    return _ops_mod.list_claims(*args, **kwargs)


def release(*args, **kwargs):
    _sync_public_overrides()
    return _ops_mod.release(*args, **kwargs)


def dispatch_workers(*args, **kwargs):
    _sync_public_overrides()
    return _dispatch_mod.dispatch_workers(*args, **kwargs)


def release_temp_task_creator(*args, **kwargs):
    _sync_public_overrides()
    return _dispatch_mod.release_temp_task_creator(*args, **kwargs)


def suggest_delegation(*args, **kwargs):
    _sync_public_overrides()
    return _dispatch_mod.suggest_delegation(*args, **kwargs)
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    mode_group.add_argument(
        "--release",
        action="store_true",
        help="Release the agent's active claim",
    )
    mode_group.add_argument(
        "--suggest-delegation",
        action="store_true",
