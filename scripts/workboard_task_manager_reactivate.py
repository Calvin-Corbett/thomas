...
def _reactivate_task(
    *,
    ...
    claim_name = str(name or agent).strip() or str(agent).strip()
    claim_parent = str(parent or "none").strip() or "none"
    claim_role = str(role or ("worker" if _norm(claim_parent) not in {"", "none"} else "solo")).strip() or "solo"
    claim_line = (
        f"- agent={agent}; name={claim_name}; role={claim_role}; parent={claim_parent}; "
        f"scope={scope}; task={summary}"
    )
    claim_found = False
