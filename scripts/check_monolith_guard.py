...
def _suggest_split_paths(rel: str, ext: str) -> list[str]:
    path = Path(rel)
    stem = path.stem
    suffix = path.suffix or (f".{ext}" if ext else "")
    parent = path.parent
    if ext == "py":
        candidates = [
            parent / f"{stem}_helpers.py",
            parent / f"{stem}_types.py",
            parent / f"{stem}_services.py",
        ]
    elif ext in {"js", "mjs", "cjs", "jsx", "ts", "tsx"}:
        candidates = [
            parent / f"{stem}.helpers{suffix}",
            parent / f"{stem}.view{suffix}",
            parent / f"{stem}.state{suffix}",
        ]
    elif ext == "css":
        candidates = [
            parent / f"{stem}_tokens.css",
            parent / f"{stem}_layout.css",
            parent / f"{stem}_states.css",
        ]
    elif ext == "html":
        candidates = [
            parent / f"{stem}_partials.html",
            parent / f"{stem}_content.html",
        ]
    else:
        candidates = [parent / f"{stem}_support{suffix}"]
    return [candidate.as_posix() for candidate in candidates]


def _attach_remediation(entry: dict[str, Any]) -> dict[str, Any]:
    updated = dict(entry)
    rel = str(updated.get("path") or "").strip()
    ext = str(updated.get("ext") or Path(rel).suffix.lstrip(".")).strip().lower()
    reason = str(updated.get("reason") or "").strip()
    if rel and rel != "<git-range>":
        updated["suggested_split_paths"] = _suggest_split_paths(rel, ext)
    if "legacy .part" in reason or "legacy monolith loader" in reason:
        updated["suggested_remediation"] = "Replace the legacy part-loader pattern with normal sibling modules and direct imports."
    elif "max_growth_lines" in reason:
        updated["suggested_remediation"] = (
            "Keep the entry file flat and move the newly added logic into sibling modules before restaging."
        )
    elif "soft limit" in reason or "hard limit" in reason:
        updated["suggested_remediation"] = (
            "Extract helpers, types, constants, or view/state logic into sibling modules until this file drops below the limit."
        )
    elif "waiver" in reason:
        updated["suggested_remediation"] = "Fix the waiver metadata intentionally instead of growing the file under an invalid waiver."
    elif "unable to compute growth range" in reason:
        updated["suggested_remediation"] = "Repair the git range lookup, then rerun the guard before committing."
    return updated
...
def run_guard(
    ...
    *,
    ...
    violations = [_attach_remediation(row) for row in violations]
    ...
    if args.json:
        ...
        if result["ok"]:
            ...
            for row in result["violations"]:
                ...
                remediation = str(row.get("suggested_remediation") or "").strip()
                if remediation:
                    print(f"    next: {remediation}")
                for suggestion in list(row.get("suggested_split_paths") or [])[:3]:
