...
- Code results now open from a real isolated loopback origin — the same mechanism Chat already uses for deliverables — so relative paths, dynamic imports and `fetch` behave exactly as they will for you. Browser console errors on the Code surface went from **108 to 2**. Only web assets are served, and the requested path is checked to stay inside the project, so previewing a page cannot hand out source or secrets sitting next to it.
