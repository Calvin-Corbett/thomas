...
from .app_routes_policy import build_mutating_route_policy_snapshot
from .app_routes_roguelite import register_roguelite_routes
from .app_routes_static import static_compat_response
...
def _setup_routes_and_handlers(
    ...
    def _register_spend_routes(app_ref: web.Application) -> None:
        """Register spend and runtime-profile APIs used by the main shell."""
        if not callable(_require_api_access):
            log.warning("Spend routes unavailable: missing API access guard")
            return
        try:
            from thomas.server.routes.spend import register_spend_routes

            register_spend_routes(
                app_ref,
                require_api_access=_require_api_access,
            )
        except (ImportError, ModuleNotFoundError, RuntimeError, KeyError) as e:
    ...
    async def static_compat(request: web.Request) -> web.StreamResponse:
        return await static_compat_response(request, web_dir)

    app.router.add_get("/static/{path:.*}", static_compat, name="static")

    register_roguelite_routes(app)

    app[APP_MUTATING_ROUTE_POLICY_SNAPSHOT] = build_mutating_route_policy_snapshot(app.router)
