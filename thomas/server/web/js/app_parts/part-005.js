...
export default [
    "",
    "    const SLASH_ENGINE_ACTIONS = {",
    "        '/issues': {",
    "            engine: 'code_issue_engine',",
    "            label: 'Code Issue Engine',",
    "        },",
    "        '/upgrade': {",
    "            engine: 'self_upgrade_engine',",
    "            label: 'Self Upgrade Engine',",
    "        },",
    "        '/sync': {",
    "            engine: 'workspace_sync_engine',",
    "            label: 'Workspace Sync Engine',",
    "        },",
    "        '/ui-audit': {",
    "            engine: 'ui_workflow_engine',",
    "            label: 'UI Workflow Engine',",
    "        },",
    "    };",
    "",
    "    function _clearSlashCommandInput() {",
    "        if (!composerTextarea) return;",
    "        composerTextarea.value = '';",
    "        composerTextarea.dispatchEvent(new Event('input'));",
    "    }",
    "",
    "    function _formatEngineStatusSummary(rows) {",
    "        const sorted = Array.isArray(rows) ? rows : [];",
    "        const healthy = sorted.filter((row) => Boolean(row && row.running) && !safeString(row.error)).length;",
    "        const details = sorted",
    "            .map((row) => {",
    "                const name = safeString(row.name);",
    "                if (!name) return '';",
    "                const label = row.running ? 'running' : 'stopped';",
    "                return `${name} ${label}` + (safeString(row.error) ? ` (${safeString(row.error)})` : '');",
    "            })",
    "            .filter(Boolean);",
    "        const detailsText = details.join(' • ');",
    "        const listText = detailsText.length > 200 ? `${detailsText.slice(0, 197)}...` : detailsText;",
    "        return `${healthy}/${sorted.length} engines healthy${listText ? `: ${listText}` : ''}`;",
    "    }",
    "",
    "    function _statusTone(rows) {",
    "        if (!Array.isArray(rows) || rows.length === 0) {",
    "            return 'warn';",
    "        }",
    "        return rows.some((row) => safeString(row.error) || !row.running) ? 'warn' : 'success';",
    "    }",
    "",
    "    function _engineResultSummary(label, payload) {",
    "        const data = payload && typeof payload === 'object' ? payload : {};",
    "        const parts = [];",
    "        const cycle = Number(data.cycle || data.cycles_completed || 0);",
    "        const score = Number(data.score || 0);",
    "        const warningCount = Number(data.warning_count || 0);",
    "        const unresolvedCount = Number(data.unresolved_count || 0);",
    "        const opportunityCount = Number(data.opportunity_count || 0);",
    "        const commitSha = safeString(data.commit_sha);",
    "        const skipReason = safeString(data.skip_reason);",
    "        if (Number.isFinite(cycle) && cycle > 0) parts.push(`cycle ${cycle}`);",
    "        if (Number.isFinite(score)) parts.push(`score ${score}`);",
    "        if (Number.isFinite(warningCount) && warningCount > 0) parts.push(`${warningCount} warning(s)`);",
    "        if (Number.isFinite(unresolvedCount) && unresolvedCount > 0) parts.push(`${unresolvedCount} issue(s)`);",
    "        if (Number.isFinite(opportunityCount)) parts.push(`${opportunityCount} opportunity(ies)`);",
    "        if (commitSha) parts.push(`commit ${commitSha.slice(0, 7)}`);",
    "        if (skipReason) parts.push(`skip: ${skipReason}`);",
    "        if (!parts.length) return `${label}: run request accepted.`;",
    "        return `${label}: ${parts.join(', ')}`;",
    "    }",
    "",
    "    async function _runEngineActionCommand(selected) {",
    "        const config = selected?.cmd ? SLASH_ENGINE_ACTIONS[selected.cmd] : null;",
    "        if (!config) return false;",
    "        const payload = {",
    "            engine: String(config.engine || '').trim(),",
    "            action: 'run_once',",
    "            force: true,",
    "            reason: 'slash_command',",
    "        };",
    "        if (!payload.engine) {",
    "            notifyUser('Missing engine mapping.', { tone: 'error', debugKind: 'engine-action' });",
    "            return true;",
    "        }",
    "        const response = await fetchJsonSafe('/api/engine-actions', {",
    "            method: 'POST',",
    "            headers: { 'Content-Type': 'application/json' },",
    "            body: JSON.stringify(payload),",
    "        });",
    "        const result = response.data && typeof response.data === 'object' ? response.data : null;",
    "        const runResult = result && typeof result.result === 'object' ? result.result : null;",
    "        const responseHasFailure = !response.ok || !result || result?.ok === false || runResult?.ok === false;",
    "        if (responseHasFailure) {",
    "            notifyUser(",
    "                `${safeString(config.label)} could not run now: ${safeString(runResult?.error) || safeString(result?.error) || `HTTP ${safeString(response.status)}`}`,",
    "                { tone: 'error', durationMs: 3200, debugKind: 'engine-action' },",
    "            );",
    "            return true;",
    "        }",
    "        const text = _engineResultSummary(safeString(config.label), runResult || result);",
    "        notifyUser(text, { tone: 'success', durationMs: 3000, debugKind: 'engine-action' });",
    "        return true;",
    "    }",
    "",
    "    async function _runEngineStatusCommand() {",
    "        const response = await fetchJsonSafe('/api/engines');",
    "        if (!response.ok) {",
    "            notifyUser(",
    "                `Engine status unavailable: ${safeString(response.text) || `HTTP ${safeString(response.status)}`}`,",
    "                { tone: 'error', durationMs: 2800, debugKind: 'engine-action' },",
    "            );",
    "            return;",
    "        }",
    "        const payload = response.data && typeof response.data === 'object' ? response.data : {};",
    "        const rows = parseEngineRows(payload);",
    "        const summary = _formatEngineStatusSummary(rows);",
    "        notifyUser(summary, {",
    "            tone: _statusTone(rows),",
    "            durationMs: 3800,",
    "            debugKind: 'engine-action',",
    "        });",
    "    }",
    "",
    "    function _buildSlashHelpText() {",
    "        const rows = Array.isArray(SLASH_COMMANDS) ? SLASH_COMMANDS : [];",
    "        const commandLines = rows",
    "            .map((item) => `${safeString(item?.cmd)}${safeString(item?.desc) ? ` - ${safeString(item.desc)}` : ''}`)",
    "            .filter(Boolean)",
    "            .sort();",
    "        const base = 'Keyboard: Esc=stop, Ctrl+Shift+N=new chat, /=commands.';",
    "        if (!commandLines.length) return base;",
    "        return `${base}\\\\nCommands:${commandLines.join('\\\\n')}`;",
    "    }",
    "",
    "    const SLASH_MODE_ALIASES = Object.freeze({",
    "        image: 'create_image',",
    "        write: 'write',",
    "        analyze: 'analyze',",
    "        code: 'code',",
    "    });",
    "",
    "    function _resolveSlashModeId(cmdRaw) {",
    "        const command = safeString(cmdRaw).trim().toLowerCase();",
    "        const modeId = command.startsWith('/') ? command.slice(1) : command;",
    "        if (!modeId) return '';",
    "        const alias = safeString(SLASH_MODE_ALIASES[modeId]).toLowerCase();",
    "        if (alias && composerResolveModePreset(alias)) {",
    "            return alias;",
    "        }",
    "        if (composerResolveModePreset(modeId)) {",
    "            return modeId;",
    "        }",
    "        if (!Array.isArray(SLASH_COMMANDS)) return '';",
    "        const inSlashCommands = SLASH_COMMANDS.some((item) => {",
    "            const value = safeString(item?.cmd).toLowerCase();",
    "            return value && value.slice(1) === modeId;",
    "        });",
    "        return inSlashCommands ? modeId : '';",
    "    }",
    "",
    "    function _isKnownModeCommand(cmdRaw) {",
    "        return Boolean(_resolveSlashModeId(cmdRaw));",
    "    }",
    "",
    "    // Handle action commands",
    "    if (selected.cmd === '/clear') {",
    "        _clearSlashCommandInput();",
    ...
    "        return;",
    "    }",
    "    if (selected.cmd === '/status') {",
    "        _clearSlashCommandInput();",
    "        void _runEngineStatusCommand();",
    "        return;",
    "    }",
    "    if (_runEngineActionCommand && SLASH_ENGINE_ACTIONS[selected.cmd]) {",
    "        _clearSlashCommandInput();",
    "        void _runEngineActionCommand(selected);",
    "        return;",
    "    }",
    "    if (selected.cmd === '/help') {",
    "        _clearSlashCommandInput();",
    "        notifyUser(_buildSlashHelpText(), {",
    "            tone: 'info',",
    "            durationMs: 4200,",
    "            debugKind: 'engine-action',",
    "        });",
    ...
    "    if (!_isKnownModeCommand(selected.cmd)) {",
    "        const command = safeString(selected.cmd);",
    "        notifyUser(`Unknown command ${command || ''}. Try /help for the full list.`, {",
    "            tone: 'warn',",
    "            durationMs: 2400,",
    "            debugKind: 'engine-action',",
    "        });",
    ...
    "        return;",
    "    }",
    "    const modeId = _resolveSlashModeId(selected.cmd);",
    "    composerTextarea.value = '';",
    "    composerTextarea.dispatchEvent(new Event('input'));",
