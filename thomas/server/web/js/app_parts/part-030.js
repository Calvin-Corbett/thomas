...
export default [
    ...
    "    const allowedStates = new Set(['idle', 'walking', 'working', 'break', 'yield', 'runaway', 'spawning', 'teleporting']);",
    ...
    "    const y = officeClamp(Number(room.y) || 58, 1, 90);",
    ...
    "    }",
    "    await refreshIdentityState();   // load preferences FIRST so fetchModels can read saved profile",
    "    await fetchModels();",
    "    await fetchChatHistory();",
    "    const storedActiveChatId = loadStoredActiveChatId();",
    "    const hasStoredSession = Boolean(storedActiveChatId)",
    "        && sidebarSessions.some((row) => safeString(row?.id) === storedActiveChatId);",
    "    if (hasStoredSession) {",
    ...
    "    }",
    "    await refreshTaskContinuity({",
    "        sessionOverride: safeString(sessionId) || safeString(activeChatId),",
    "        force: true,",
    "    });",
    "    _positionRobotDock();",
    ...
    "        }",
    "",
    "        // Runtime chat sessions are in-memory only; import saved conversation into",
    "        // a fresh session so follow-up turns continue with context.",
    "        const importPayload = {",
    "            profile: activeProfileNameForPersistence() || 'local',",
    "            conversation: loadedHistory.map((msg) => ({",
    ...
    "            }",
    "        } else {",
    "            const newRes = await fetchJsonSafe('/api/session/new', { method: 'POST' });",
    "            if (newRes.ok) {",
    "                const created = newRes.data || {};",
    "                if (safeString(created?.session_id)) {",
    "                    sessionId = safeString(created.session_id);",
    "                }",
    "            }",
    "        }",
