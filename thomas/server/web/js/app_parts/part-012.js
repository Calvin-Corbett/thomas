...
export default [
    ...
    "    const localX = officeClamp(event.clientX - rect.left, 0, rect.width);",
    "    const localY = officeClamp(event.clientY - rect.top, 0, rect.height);",
    "    const pad = Math.min(rect.width, rect.height) * OFFICE_MINIMAP_PAD_RATIO;",
    "    const mapW = Math.max(1, rect.width - (pad * 2));",
    "    const mapH = Math.max(1, rect.height - (pad * 2));",
    ...
    "",
    "    officeState.agents.forEach((agent) => {",
    "        if (!agent || agent.state === 'runaway' || agent.state === 'teleporting') return;",
    ...
    "        ['idle', 'walking', 'working', 'break', 'yield', 'runaway', 'spawning', 'teleporting'].forEach((stateName) => {",
    ...
    "        desk: '#6f89ae',",
    "    };",
    "    if (theme === 'engineering') return { floor: '#aac8ec', wall: '#2d5d93', accent: '#79a3d2', desk: '#587fa9' };",
    "    if (theme === 'content') return { floor: '#cbb8e8', wall: '#5b4d95', accent: '#a08fd1', desk: '#737cab' };",
    "    if (theme === 'research') return { floor: '#b2d8d0', wall: '#3b7d70', accent: '#86c1b1', desk: '#66889d' };",
    "    if (theme === 'ops') return { floor: '#a9c6e0', wall: '#34658b', accent: '#7ca6c9', desk: '#5c7fa3' };",
    "    if (theme === 'planning' || theme === 'design' || theme === 'support') {",
    "        return { floor: '#c3cde4', wall: '#4f6693', accent: '#95a8cc', desk: '#6c81a8' };",
    "    }",
    "    if (theme === 'pods' || theme === 'dynamic') {",
    "        return { floor: '#b8c4e2', wall: '#47598e', accent: '#889acd', desk: '#6278a6' };",
    "    }",
    "    if (theme === 'break' || theme === 'coffee') {",
    "        return { floor: '#c7d4e4', wall: '#546d90', accent: '#8ea4c3', desk: '#6f86a5' };",
    "    }",
    "    if (theme === 'lobby') return { floor: '#a8c9ea', wall: '#2e6498', accent: '#76abd9', desk: '#5887b3' };",
    "    return base;",
    "}",
    "",
    ...
    "    };",
    "",
    "    fillTile(0, 0, '#dbe3ef', '#ccd7e7', '#c0cde0', 'rgba(141, 161, 191, 0.42)');",
    "    fillTile(1, 0, '#d2def0', '#c5d3ea', '#b7c8e1', 'rgba(119, 146, 182, 0.38)');",
    "    fillTile(2, 0, '#ddd8ef', '#d0cae8', '#c3c0df', 'rgba(135, 124, 172, 0.34)');",
    "    fillTile(3, 0, '#d3e6e5', '#c4dcd9', '#b4cfcb', 'rgba(111, 152, 145, 0.34)');",
    "    fillTile(4, 0, '#e4e0d5', '#d7d1c3', '#c9bea9', 'rgba(154, 136, 106, 0.34)');",
    "    fillTile(5, 0, '#e6edf5', '#d8e1ee', '#cbd6e6', 'rgba(112, 137, 171, 0.38)');",
    "",
    "    fillTile(0, 1, '#e6edf6', '#d9e4f1', '#cedbeb', 'rgba(124, 145, 176, 0.42)');",
    "    fillTile(1, 1, '#d9e3ef', '#cdd9e8', '#c0cfdf', 'rgba(98, 121, 153, 0.44)');",
    "    fillTile(2, 1, '#cfdaea', '#c3d0e4', '#b6c6de', 'rgba(93, 118, 152, 0.4)');",
    "    fillTile(3, 1, '#dbe6f3', '#cfdeef', '#c0d2e8', 'rgba(103, 128, 162, 0.42)');",
    "    fillTile(4, 1, '#d4dde8', '#c8d2e0', '#bcc8da', 'rgba(101, 120, 151, 0.38)');",
    "    fillTile(5, 1, '#c8d4e7', '#bccbe1', '#aebfda', 'rgba(84, 109, 146, 0.44)');",
    "",
    "    fillTile(0, 2, '#dbe5f0', '#ced9e7', '#c2cfdf', 'rgba(108, 132, 164, 0.38)');",
    "    fillTile(1, 2, '#e2e8f1', '#d4dde9', '#c7d3e2', 'rgba(116, 136, 166, 0.34)');",
    "    fillTile(2, 2, '#dae5f1', '#cedbec', '#c0d2e8', 'rgba(102, 127, 163, 0.36)');",
    "    fillTile(3, 2, '#d6e3ef', '#cad9ea', '#bdd0e6', 'rgba(99, 123, 158, 0.36)');",
    "    fillTile(4, 2, '#dce6ef', '#d0dce8', '#c5d3e2', 'rgba(101, 123, 154, 0.34)');",
    "    fillTile(5, 2, '#e3ebf3', '#d5e0ec', '#cad8e8', 'rgba(113, 137, 168, 0.34)');",
    "",
    "    // Extra hallway surfaces and trim variants.",
    "    fillTile(6, 0, '#e9edf4', '#d7e0ec', '#c8d4e5', 'rgba(108, 129, 162, 0.28)');",
    "    fillTile(7, 0, '#e5dde5', '#d4c8d8', '#c5b9cb', 'rgba(116, 101, 126, 0.24)');",
    "    fillTile(8, 0, '#dce8e3', '#c9d8d2', '#bbcbc6', 'rgba(98, 123, 114, 0.24)');",
    "    fillTile(9, 0, '#e9e2d8', '#d8cdbc', '#cabca5', 'rgba(133, 113, 84, 0.26)');",
    "    fillTile(10, 0, '#d5e1ed', '#c6d6e8', '#b8c9df', 'rgba(87, 116, 152, 0.26)');",
    "    fillTile(11, 0, '#ece8f0', '#dcd5e8', '#ccc4dd', 'rgba(118, 104, 148, 0.24)');",
    "    fillTile(6, 1, '#d9e3ef', '#c9d7e8', '#bbccdf', 'rgba(96, 119, 151, 0.28)');",
    "    fillTile(7, 1, '#dfe7f1', '#d0dcea', '#c0d0e2', 'rgba(106, 127, 157, 0.3)');",
    "    fillTile(8, 1, '#d4dfe9', '#c4d2e1', '#b6c6da', 'rgba(92, 114, 145, 0.3)');",
    "    fillTile(9, 1, '#d0dbe7', '#c0cee0', '#b2c2d8', 'rgba(86, 108, 141, 0.32)');",
    "    fillTile(10, 1, '#d7e2ed', '#c8d6e4', '#bbcbde', 'rgba(95, 118, 148, 0.3)');",
    "    fillTile(11, 1, '#dce5ef', '#cdd9e7', '#bfd0e2', 'rgba(102, 124, 154, 0.3)');",
    "",
    "    const spritePalette = {",
    "        o: '#1f2d43',",
    "        a: '#415977',",
