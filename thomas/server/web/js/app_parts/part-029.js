...
"            },",
    ...
    "}",
    "",
"function moduleRenderMarketplaceCatalogV2(container, { mode, rows }) {",
"    if (!container) return;",
"    const state = moduleEnsureRuntime();",
"    if (!state) {",
"        moduleRenderList(container, { mode, section: 'queue', rows, emptyText: 'No modules right now.' });",
"        return;",
"    }",
"    const activeMode = MODULE_NAV_MODE_SET.has(mode) ? mode : (state.activeMode || 'marketplace');",
"    const sectionLabel = moduleMarketplaceSectionLabel(moduleMarketplaceSectionId(safeString(state?.subnavFocus?.[activeMode]) || 'featured'));",
"    const searchValue = safeString(state?.marketplace?.search).trim();",
"    const items = Array.isArray(rows) ? rows : [];",
"    const normalizeList = (raw) => {",
"        const list = Array.isArray(raw) ? raw : [];",
"        return list.map((value) => safeString(value)).filter(Boolean);",
"    };",
"    const installedCount = items.filter((item) => safeString(item?.statusLabel).toLowerCase() === 'enabled').length;",
"    container.innerHTML = '';",
"    const shell = document.createElement('div');",
"    shell.className = 'module-marketplace-shell';",
"    shell.innerHTML = `",
"        <section class=\\\"module-marketplace-toolbar\\\">",
"            <div class=\\\"module-marketplace-toolbar-copy\\\">",
"                <h4>Thomas Store</h4>",
"                <p>${escapeHtml(`Manage the ${sectionLabel} feature stream in one place.`)}</p>",
"            </div>",
"            <div class=\\\"module-marketplace-toolbar-actions\\\">",
"                <label class=\\\"module-marketplace-search-wrap\\\" aria-label=\\\"Search marketplace\\\">",
"                    <span class=\\\"module-marketplace-search-icon\\\">Search</span>",
"                    <input",
"                        type=\\\"search\\\"",
"                        class=\\\"module-marketplace-search\\\"",
"                        data-module-marketplace-search=\\\"1\\\"",
"                        data-module-mode=\\\"${escapeHtml(activeMode)}\\\"",
"                        placeholder=\\\"Search by title, feature, requirement, source\\\"",
"                        value=\\\"${escapeHtml(searchValue)}\\\"",
"                    />",
"                </label>",
"                <button type=\\\"button\\\" class=\\\"module-item-btn module-marketplace-clear-btn\\\" data-module-marketplace-clear=\\\"1\\\" data-module-mode=\\\"${escapeHtml(activeMode)}\\\" ${searchValue ? '' : 'hidden'}>Clear</button>",
"            </div>",
"        </section>",
"        <section class=\\\"module-marketplace-stats\\\">",
"            <span>${escapeHtml(`${items.length} item${items.length === 1 ? '' : 's'} in stream`)}</span>",
"            <span>${escapeHtml(`${installedCount} enabled`)}</span>",
"            ${searchValue ? `<span>${escapeHtml(`Filtered by ${searchValue}`)}</span>` : ''}",
"        </section>",
"    `;",
"    if (!items.length) {",
...
"        if (searchValue) {",
"            empty.textContent = `No items match ${searchValue} in ${sectionLabel}.`;",
"        } else if (sectionLabel === 'Featured') {",
"            empty.textContent = 'No installable items available yet.';",
"        } else {",
"            empty.textContent = `No items found in ${sectionLabel} yet.`;",
"        }",
...
"    stream.className = 'module-marketplace-stream';",
"    const frag = document.createDocumentFragment();",
"    items.forEach((item) => {",
"        const toneClass = moduleToneClass(item?.tone);",
"        const source = safeString(item?.source);",
"        const title = safeString(item?.title) || 'Item';",
"        const description = safeString(item?.description) || safeString(item?.meta) || 'No details provided.';",
"        const section = safeString(item?.sectionLabel) || 'Featured';",
"        const actions = Array.isArray(item?.actions) ? item.actions : [];",
"        const actionsHtml = actions.map((action) => `",
"            <button",
"                type=\\\"button\\\"",
"                class=\\\"module-item-btn module-marketplace-btn module-marketplace-inline-btn\\\"",
"                data-module-action-id=\\\"${escapeHtml(safeString(action?.id))}\\\"",
"                data-module-mode=\\\"${escapeHtml(activeMode)}\\\"",
"                data-module-section=\\\"${escapeHtml(safeString(item?.section) || 'queue')}\\\"",
...
"            </button>`).join('');",
"        const metaBits = [",
"            safeString(item?.statusLabel),",
"            safeString(item?.installedVersion) ? `Installed ${safeString(item.installedVersion)}` : '',",
"            safeString(item?.latestVersion) ? `Latest ${safeString(item.latestVersion)}` : '',",
"            safeString(item?.meta),",
"        ].filter(Boolean);",
"        const tagBits = [",
"            ...normalizeList(item?.capabilities),",
"            ...normalizeList(item?.requirements),",
"            ...normalizeList(item?.tags),",
"        ].filter(Boolean).slice(0, 8);",
"        const chipHtml = tagBits.map((chip) => `<span class=\\\"module-marketplace-chip\\\">${escapeHtml(chip)}</span>`).join('');",
"        const row = document.createElement('article');",
"        row.className = `module-marketplace-item${toneClass ? ` ${toneClass}` : ''}`;",
"        row.innerHTML = `",
"            <div class=\\\"module-marketplace-item-main\\\">",
"                <header class=\\\"module-marketplace-item-head\\\">",
"                    <div class=\\\"module-marketplace-item-title-wrap\\\">",
"                        <h5 class=\\\"module-marketplace-title\\\">${escapeHtml(title)}</h5>",
"                        <p class=\\\"module-marketplace-subtitle\\\">${escapeHtml(section)}${source ? ` - ${escapeHtml(source)}` : ''}</p>",
"                    </div>",
"                    ${item?.badge ? `<span class=\\\"module-marketplace-status ${toneClass || ''}\\\">${escapeHtml(item.badge)}</span>` : ''}",
"                </header>",
"                <p class=\\\"module-marketplace-description\\\">${escapeHtml(description)}</p>",
"                <p class=\\\"module-marketplace-meta-line\\\">${metaBits.length ? escapeHtml(metaBits.join(' - ')) : 'No release metadata.'}</p>",
"                ${chipHtml ? `<div class=\\\"module-marketplace-chip-row\\\">${chipHtml}</div>` : ''}",
"            </div>",
"            <div class=\\\"module-marketplace-item-actions\\\">${actionsHtml}</div>",
"        `;",
...
"}",
"",
"function moduleRenderMarketplaceCatalog(container, { mode, rows }) {",
    "    if (!container) return;",
"    const state = moduleEnsureRuntime();",
    "    if (!state) {",
    "        moduleRenderList(container, { mode, section: 'queue', rows, emptyText: 'No modules right now.' });",
...
"    const searchValue = safeString(state?.marketplace?.search);",
"    const sectionLabel = moduleMarketplaceSectionLabel(moduleMarketplaceSectionId(safeString(state?.subnavFocus?.[activeMode]) || 'featured'));",
"    const items = rawRows;",
    "    container.innerHTML = '';",
    "    const shell = document.createElement('div');",
    ...
    "    controls.className = 'module-marketplace-toolbar';",
    "    controls.innerHTML = `",
    "        <div class=\\\"module-marketplace-toolbar-copy\\\">",
    "            <h4>Thomas Store</h4>",
"            <p>${escapeHtml(`Explore ${items.length} installable features in the ${sectionLabel} stream.`)}</p>",
    "        </div>",
    "        <div class=\\\"module-marketplace-toolbar-actions\\\">",
    "            <label class=\\\"module-marketplace-search-wrap\\\" aria-label=\\\"Search marketplace\\\">",
    "                <span class=\\\"module-marketplace-search-icon\\\">",
"                    🔎",
    "                </span>",
    "                <input",
    "                    type=\\\"search\\\"",
    ...
    "                    data-module-marketplace-search=\\\"1\\\"",
    "                    data-module-mode=\\\"${escapeHtml(activeMode)}\\\"",
    "                    placeholder=\\\"Search by name, feature, requirement, source\\\"",
    "                    value=\\\"${escapeHtml(searchValue)}\\\"",
    "                />",
    "            </label>",
    "            <button type=\\\"button\\\" class=\\\"module-item-btn module-marketplace-clear-btn\\\" data-module-marketplace-clear=\\\"1\\\" data-module-mode=\\\"${escapeHtml(activeMode)}\\\" ${searchValue ? '' : 'hidden'}>Clear</button>",
    ...
    "        const empty = document.createElement('div');",
    "        empty.className = 'module-empty';",
    "        const searchSuffix = searchValue ? ` for \"${searchValue}\"` : '';",
    "        const section = safeString(state?.subnavFocus?.[activeMode]) || 'Featured';",
    "        if (searchValue) {",
    "            empty.textContent = `No ${safeString(section)} items match ${searchSuffix}. Try another term.`;",
    "        } else {",
    "            empty.textContent = `No items are available in ${safeString(section)} yet.`;",
    "        }",
    "        shell.appendChild(empty);",
    "        container.appendChild(shell);",
    "        return;",
...
"            <div class=\\\"module-marketplace-toolbar-copy\\\">",
"                <h4>Thomas Store</h4>",
"                <p>${escapeHtml(`Install, enable, and manage features from the ${sectionLabel} stream.`)}</p>",
"            </div>",
"            <div class=\\\"module-marketplace-toolbar-actions\\\">",
"                <label class=\\\"module-marketplace-search-wrap\\\" aria-label=\\\"Search marketplace\\\">",
"                    <span class=\\\"module-marketplace-search-icon\\\">Search</span>",
...
"        </section>",
"        <section class=\\\"module-marketplace-stats\\\">",
"            <span>${escapeHtml(`${totalCount} item${totalCount === 1 ? '' : 's'} in stream`)}</span>",
"            <span>${escapeHtml(`${installedCount} installed`)}</span>",
"            ${searchValue ? `<span>${escapeHtml(`Filtered by ${searchValue}`)}</span>` : ''}",
"        </section>",
"    `;",
"    if (!items.length) {",
...
"        empty.className = 'module-empty';",
"        if (searchValue) {",
"            empty.textContent = `No items found for \\\"${escapeHtml(searchValue)}\\\" in ${sectionLabel}.`;",
"        } else if (sectionLabel === 'Featured') {",
"            empty.textContent = 'No installable items available yet.';",
"        } else {",
"            empty.textContent = `No items found in ${sectionLabel} yet.`;",
...
"    items.forEach((item) => {",
"        const toneClass = moduleToneClass(item?.tone);",
"        const tone = toneClass ? `tone-${toneClass}` : '';",
"        const source = safeString(item?.source);",
"        const title = safeString(item?.title) || 'Item';",
"        const description = safeString(item?.description) || safeString(item?.meta) || 'No details provided.';",
"        const section = safeString(item?.sectionLabel) || 'Featured';",
...
"        const chipHtml = tagBits.map((chip) => `<span class=\\\"module-marketplace-chip\\\">${escapeHtml(chip)}</span>`).join('');",
"        const row = document.createElement('article');",
"        row.className = `module-marketplace-item ${tone}`;",
"        row.innerHTML = `",
"            <div class=\\\"module-marketplace-item-main\\\">",
"                <header class=\\\"module-marketplace-item-head\\\">",
"                    <div class=\\\"module-marketplace-item-title-wrap\\\">",
...
"                        <p class=\\\"module-marketplace-subtitle\\\">${escapeHtml(section)}${source ? ` · ${escapeHtml(source)}` : ''}</p>",
"                    </div>",
"                    ${item?.badge ? `<span class=\\\"module-marketplace-status ${tone}\\\">${escapeHtml(item.badge)}</span>` : ''}",
"                </header>",
"                <p class=\\\"module-marketplace-description\\\">${escapeHtml(description)}</p>",
"                <p class=\\\"module-marketplace-meta-line\\\">${metaBits.length ? escapeHtml(metaBits.join(' · ')) : 'No release metadata.'}</p>",
"                <div class=\\\"module-marketplace-chip-row\\\">${chipHtml}</div>",
"            </div>",
"            <div class=\\\"module-marketplace-item-actions\\\">${actionsHtml}</div>",
"        `;",
"        frag.appendChild(row);",
...
"}",
    "",
"function moduleRenderMarketplaceCatalogV3(container, { mode, rows }) {",
"    if (!container) return;",
"    const state = moduleEnsureRuntime();",
"    if (!state) {",
"        moduleRenderList(container, { mode, section: 'queue', rows, emptyText: 'No modules right now.' });",
...
"    const enabledCount = visibleRows.filter((item) => safeString(item?.statusLabel).toLowerCase() === 'enabled').length;",
"    const normalizeList = (raw) => {",
"        const list = Array.isArray(raw) ? raw : [];",
"        return list.map((value) => safeString(value)).filter(Boolean);",
"    };",
...
"            <div class=\\\"module-marketplace-toolbar-copy\\\">",
"                <h4>Thomas Store</h4>",
"                <p>${escapeHtml(`Manage ${sectionLabel} features. Keep only the tools you need.`)}</p>",
"            </div>",
"            <div class=\\\"module-marketplace-toolbar-actions\\\">",
"                <label class=\\\"module-marketplace-search-wrap\\\" aria-label=\\\"Search marketplace\\\">",
"                    <span class=\\\"module-marketplace-search-icon\\\">Search</span>",
...
"                        data-module-marketplace-search=\\\"1\\\"",
"                        data-module-mode=\\\"${escapeHtml(activeMode)}\\\"",
"                        placeholder=\\\"Search by title, source, feature, requirement\\\"",
"                        value=\\\"${escapeHtml(searchValue)}\\\"",
"                    />",
"                </label>",
"                <button type=\\\"button\\\" class=\\\"module-item-btn module-marketplace-clear-btn\\\" data-module-marketplace-clear=\\\"1\\\" data-module-mode=\\\"${escapeHtml(activeMode)}\\\" ${searchValue ? '' : 'hidden'}>Clear</button>",
...
"        </section>",
"        <section class=\\\"module-marketplace-stats\\\">",
"            <span>${escapeHtml(`${visibleRows.length} item${visibleRows.length === 1 ? '' : 's'} available`)}</span>",
"            <span>${escapeHtml(`${enabledCount} enabled`)}</span>",
"            ${searchValue ? `<span>${escapeHtml(`Filtered by ${searchValue}`)}</span>` : ''}",
"        </section>",
"    `;",
"",
...
"            empty.textContent = `No items match ${searchValue} in ${sectionLabel}.`;",
"        } else {",
"            empty.textContent = `No items are available in ${sectionLabel} yet.`;",
"        }",
"        shell.appendChild(empty);",
"        container.appendChild(shell);",
"        return;",
...
"        const title = safeString(item?.title) || 'Item';",
"        const source = safeString(item?.source);",
"        const itemSection = safeString(item?.sectionLabel) || 'Featured';",
"        const description = safeString(item?.description) || safeString(item?.meta) || 'No details provided.';",
"        const actions = Array.isArray(item?.actions) ? item.actions : [];",
"        const statusBits = [",
"            safeString(item?.statusLabel),",
...
"        const chips = chipBits.length ? chipBits.map((chip) => `<span class=\\\"module-marketplace-chip\\\">${escapeHtml(chip)}</span>`).join('') : '';",
...
"                    <div class=\\\"module-marketplace-item-title-wrap\\\">",
"                        <h5 class=\\\"module-marketplace-title\\\">${escapeHtml(title)}</h5>",
"                        <p class=\\\"module-marketplace-subtitle\\\">${escapeHtml(itemSection)}${source ? ` - ${escapeHtml(source)}` : ''}</p>",
"                    </div>",
"                    ${item?.badge ? `<span class=\\\"module-marketplace-status ${toneClass || ''}\\\">${escapeHtml(item.badge)}</span>` : ''}",
"                </header>",
"                <p class=\\\"module-marketplace-description\\\">${escapeHtml(description)}</p>",
"                <p class=\\\"module-marketplace-meta-line\\\">${statusBits.length ? statusBits.map((statusBit) => `<span class=\\\"module-marketplace-meta-bit\\\">${escapeHtml(safeString(statusBit))}</span>`).join('') : 'No metadata.'}</p>",
"                ${chips ? `<div class=\\\"module-marketplace-chip-row\\\">${chips}</div>` : ''}",
"            </div>",
"            <div class=\\\"module-marketplace-item-actions\\\">${actionsHtml}</div>",
"        `;",
...
"    const definition = moduleBuildDefinition(mode, snapshot, signals);",
    "    if (mode === 'marketplace') {",
    "        const refreshPromise = moduleRefreshMarketplace({ force: modeChanged });",
"        if (refreshPromise) {",
"            refreshPromise.finally(() => {",
"                const latest = moduleEnsureRuntime();",
"                if (latest && latest.activeMode === mode) {",
...
"    if (mode === 'marketplace') {",
    "        moduleRenderMarketplaceCatalogV3(moduleQueueList, { mode, rows: moduleVisibleItems(mode, 'queue', definition.queueRows) });",
"    } else {",
...
"        const section = safeString(target.dataset.moduleSection).toLowerCase() || 'queue';",
    ...
    "        if (mode === 'marketplace' && (actionId === 'enable' || actionId === 'disable') && itemId) {",
    "            if (target instanceof HTMLButtonElement) target.disabled = true;",
    "            try {",
    "                const result = await moduleToggleMarketplaceModule(itemId, actionId === 'enable');",
    "                if (!result?.ok) {",
    "                    moduleRecordActivity(mode, `${actionLabel} failed for ${itemId || 'item'}.`, 'error');",
    "                    moduleTouch(mode);",
    "                    moduleRender(mode, { touch: false });",
    "                    notifyUser(result?.error || `${actionLabel} failed.`, { tone: 'error', durationMs: 2200, debugKind: actionId || 'module-action' });",
    "                } else {",
    ...
    "            return;",
    "        }",
    "        if (actionId === 'install' || actionId === 'latest') {",
    "            if (target instanceof HTMLButtonElement) target.disabled = true;",
    "            try {",
    "                if (!actionUrl) {",
