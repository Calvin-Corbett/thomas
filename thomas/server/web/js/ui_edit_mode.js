(function () {
  ...
  function makeUi() {
    ...
    editor.innerHTML = `<div class="thomas-ui-edit-grid"></div><div class="thomas-ui-edit-toolbar" role="toolbar" aria-label="UI layout tools"><button class="thomas-ui-edit-grip" type="button" data-action="move-toolbar" aria-label="Move toolbar">⠿</button><b class="thomas-ui-edit-badge">EDIT</b><span class="thomas-ui-edit-name">Select a region</span><span class="thomas-ui-edit-breakpoint"></span><button type="button" data-action="undo" aria-label="Undo layout change">↶</button><button type="button" data-action="redo" aria-label="Redo layout change">↷</button><button type="button" data-action="snap" aria-label="Toggle snapping" aria-pressed="true">Snap</button><button type="button" data-action="lock" aria-label="Lock selected region">Lock</button><button type="button" data-action="reset" aria-label="Reset selected region">Reset item</button><button type="button" data-action="reset-view" aria-label="Reset this breakpoint layout">Reset view</button><button type="button" data-action="export" aria-label="Export all layouts">Export</button><button type="button" data-action="done" aria-label="Exit UI Edit Mode">Done</button></div><div class="thomas-ui-edit-selection" hidden><span></span>${HANDLES.map((edge) => `<button type="button" class="thomas-ui-resize-handle thomas-ui-resize-${edge}" data-edge="${edge}" aria-label="Resize ${edge}"></button>`).join("")}</div><i class="thomas-ui-guide thomas-ui-guide-x"></i><i class="thomas-ui-guide thomas-ui-guide-y"></i><div class="thomas-ui-edit-toast" role="status" aria-live="polite"></div>`;
    ...
    return { entry, editor, toolbar, selection: editor.querySelector(".thomas-ui-edit-selection"), name: editor.querySelector(".thomas-ui-edit-name"), point: editor.querySelector(".thomas-ui-edit-breakpoint"), toast: editor.querySelector(".thomas-ui-edit-toast"), guideX: editor.querySelector(".thomas-ui-guide-x"), guideY: editor.querySelector(".thomas-ui-guide-y") };
  }
  ...
  function flash(message) { ui.toast.textContent = message; clearTimeout(flash.timer); flash.timer = setTimeout(() => { ui.toast.textContent = ""; }, 1600); }
  ...
  function refreshSelection() {
    ...
    ui.selection.querySelectorAll("[data-edge]").forEach((handle) => { handle.hidden = !canResize; });
  }
  function sync() {
    ui.point.textContent = L.currentPoint(); ui.name.textContent = state.selected ? label(state.selected) : "Select a region";
    ...
    ["lock", "reset"].forEach((action) => { ui.toolbar.querySelector(`[data-action="${action}"]`).disabled = !state.selected; });
    ...
    ui.toolbar.querySelector('[data-action="snap"]').setAttribute("aria-pressed", String(state.snap));
    requestAnimationFrame(refreshSelection);
  }
  function setActive(active) {
    const next = Boolean(active);
    if (next && !state.active && document.activeElement instanceof HTMLElement) state.returnFocus = document.activeElement;
    state.active = next;
    if (!ui) return;
    state.drag = null; state.selected = null; setGuides(false, false);
    document.documentElement.classList.toggle("thomas-ui-editing", state.active);
    ui.editor.hidden = !state.active; ui.entry.hidden = state.active;
    sync();
    if (state.active) { ui.toolbar.querySelector('[data-action="done"]').focus(); flash(`Editing ${L.currentPoint()} layout`); }
    else if (state.returnFocus && state.returnFocus.isConnected) state.returnFocus.focus({ preventScroll: true });
  }
  function select(node) {
    state.selected = node; sync();
    if (L.policy(node).protected) flash(`${label(node)} is protected`);
  }
  function boundary(node) {
    const p = L.policy(node);
    if (p.containment === "parent" && node.parentElement) return node.parentElement.getBoundingClientRect();
  ...
  function preview(node, next) {
    node.style.translate = `${next.x || 0}px ${next.y || 0}px`;
    if (Number.isFinite(next.width)) node.style.width = `${next.width}px`;
    if (Number.isFinite(next.height)) node.style.height = `${next.height}px`;
  ...
  function collides(node, next, natural) {
    ...
    return Array.from(document.querySelectorAll("[data-ui-id]")).some((other) => {
      if (other === node || node.contains(other) || other.contains(node) || L.policy(other).protected) return false;
      const r = other.getBoundingClientRect(); return trial.left < r.right && trial.right > r.left && trial.top < r.bottom && trial.bottom > r.top;
    });
  }
  ...
  function moveDrag(event) {
    ...
    const dx = event.clientX - d.startX, dy = event.clientY - d.startY, p = L.policy(d.node), box = boundary(d.node);
    if (d.type === "move") {
      let x = d.item.x + dx, y = d.item.y + dy;
      const centerX = d.natural.left + x + d.rect.width / 2, centerY = d.natural.top + y + d.rect.height / 2;
      const guideX = Math.abs(centerX - (box.left + box.right) / 2) <= 8, guideY = Math.abs(centerY - (box.top + box.bottom) / 2) <= 8;
      if (guideX && state.snap) x += (box.left + box.right) / 2 - centerX;
      if (guideY && state.snap) y += (box.top + box.bottom) / 2 - centerY;
      ...
      const next = Object.assign({}, d.item, { x, y }); setGuides(guideX, guideY);
      ...
      next.x = Math.max(box.left - d.natural.left, Math.min(box.right - d.natural.left - width, next.x));
      next.y = Math.max(box.top - d.natural.top, Math.min(box.bottom - d.natural.top - height, next.y));
      if (!collides(d.node, next, d.natural)) preview(d.node, next);
    }
  }
  function endDrag() {
    if (!state.drag) return;
    L.set(L.identity(state.drag.node), state.drag.pending); state.drag = null; setGuides(false, false); sync();
  }
  function changeSelected(patch) {
    if (!state.selected) return;
    const current = item(), p = policy(), rect = state.selected.getBoundingClientRect(), box = boundary(state.selected);
    ...
    next.x = Math.max(box.left - naturalLeft, Math.min(box.right - naturalLeft - width, next.x));
    next.y = Math.max(box.top - naturalTop, Math.min(box.bottom - naturalTop - height, next.y));
    const changesGeometry = ["x", "y", "width", "height"].some((key) => Object.prototype.hasOwnProperty.call(patch, key));
    if (changesGeometry && collides(state.selected, next, { left: naturalLeft, top: naturalTop, width: rect.width, height: rect.height })) {
      flash("That change would overlap another region"); return;
    }
    record(); L.set(selectedId(), next); sync();
  }
  function undo() { const previous = state.history.pop(); if (!previous) return; state.future.push(L.currentMap()); L.replaceMap(previous); flash("Undid layout change"); sync(); }
  function redo() { const next = state.future.pop(); if (!next) return; state.history.push(L.currentMap()); L.replaceMap(next); flash("Redid layout change"); sync(); }
  function resetSelected() { if (!state.selected) return; record(); L.remove(selectedId()); flash(`${label(state.selected)} restored`); sync(); }
  function resetView() { record(); L.resetBreakpoint(); state.selected = null; flash(`${L.currentPoint()} layout restored`); sync(); }
  function exportLayout() {
    const blob = new Blob([JSON.stringify({ source: "Thomas UI Edit Mode", layouts: L.exportBook() }, null, 2)], { type: "application/json" });
    const link = document.createElement("a"), url = URL.createObjectURL(blob); link.href = url; link.download = "thomas-ui-layout.json"; link.click(); URL.revokeObjectURL(url); flash("Layout exported");
  }
  function action(name) {
    if (name === "done") setActive(false); else if (name === "undo") undo(); else if (name === "redo") redo();
    else if (name === "snap") { state.snap = !state.snap; sync(); } else if (name === "lock") changeSelected({ locked: !item().locked });
    else if (name === "reset") resetSelected(); else if (name === "reset-view") resetView(); else if (name === "export") exportLayout();
  }
  ...
  function onPointerDown(event) {
    if (!state.active || event.target.closest("[data-editor-ui]")) return;
    const node = event.target.closest("[data-ui-id]"); if (!node) return;
    event.preventDefault(); event.stopImmediatePropagation(); select(node); beginDrag(event, node, "move");
  }
  function onKeyDown(event) {
    if (event.key.toLowerCase() === "e" && event.ctrlKey && event.shiftKey) { event.preventDefault(); state.chord = false; setActive(!state.active); return; }
    if (event.key === "Tab" && event.shiftKey && !typing(event.target)) { event.preventDefault(); state.chord = false; setActive(!state.active); return; }
    if ((event.key === "Shift" && event.ctrlKey) || (event.key === "Control" && event.shiftKey)) { state.chord = true; return; }
    if (!["Shift", "Control"].includes(event.key)) state.chord = false;
  ...
  function start() {
    ...
    ui.toolbar.addEventListener("click", (event) => { const button = event.target.closest("[data-action]"); if (button && button.dataset.action !== "move-toolbar") action(button.dataset.action); });
    ...
    addEventListener("keydown", onKeyDown); addEventListener("keyup", (event) => { if (["Shift", "Control"].includes(event.key) && state.chord) { state.chord = false; setActive(!state.active); } });
    addEventListener("scroll", refreshSelection, true); addEventListener("resize", () => { L.applyAll(); sync(); });
    sync();
  }
  window.ThomasUiEditMode = { setActive, toggle: () => setActive(!state.active), isActive: () => state.active, selectedId };
