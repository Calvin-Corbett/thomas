function initModuleWorkspace() {
    ...
    const readChannelsRuntimeFieldValue = (field) => {
    ...
    const mergeChannelsRuntimePatch = (channelsState, patch) => {
        if (!channelsState || !patch || typeof patch !== 'object') return;
        const currentStatus = channelsState.status && typeof channelsState.status === 'object' ? channelsState.status : {};
        const currentRuntime = currentStatus.runtime && typeof currentStatus.runtime === 'object' ? currentStatus.runtime : {};
    ...
    const queueChannelsRuntimeApply = (patch, delayMs = 250) => {
        const channelsState = moduleChannelsState();
        if (!channelsState || !patch || !Object.keys(patch).length) return;
        channelsState.liveRuntimePatch = {
            ...(channelsState.liveRuntimePatch && typeof channelsState.liveRuntimePatch === 'object' ? channelsState.liveRuntimePatch : {}),
            ...patch,
        };
        if (channelsState.liveRuntimeTimer) {
            window.clearTimeout(channelsState.liveRuntimeTimer);
        }
        channelsState.liveRuntimeTimer = window.setTimeout(() => {
            channelsState.liveRuntimeTimer = 0;
            const payload = { ...(channelsState.liveRuntimePatch || {}) };
            channelsState.liveRuntimePatch = {};
            ...
            const request = async () => {
                ...
                mergeChannelsRuntimePatch(channelsState, payload);
            };
            const prior = channelsState.liveRuntimeRequest || Promise.resolve();
            channelsState.liveRuntimeRequest = prior
                .catch(() => {})
                .then(request)
                .catch((error) => {
                    notifyUser(safeString(error?.message) || 'Discord runtime update failed.', {
                        tone: 'error',
                        durationMs: 2200,
                        debugKind: 'channels-runtime-live',
                    });
                    throw error;
                });
    ...
    if (moduleQueueList) {
        ...
        moduleQueueList.addEventListener('input', (event) => {
            const slider = event.target instanceof Element ? event.target.closest('input[type="range"][data-channels-slider-format]') : null;
            if (!(slider instanceof HTMLInputElement)) return;
            const format = safeString(slider.dataset.channelsSliderFormat) || 'int';
            const suffix = safeString(slider.dataset.channelsSliderSuffix);
            const valueNode = slider.closest('.module-channels-slider-row')?.querySelector('[data-channels-slider-value]');
            if (valueNode instanceof HTMLOutputElement || valueNode instanceof HTMLElement) {
                valueNode.textContent = moduleChannelsFormatSliderValue(slider.value, format, suffix);
            }
            const runtimeKey = safeString(slider.getAttribute('data-channels-runtime-field'));
            if (runtimeKey) {
                queueChannelsRuntimeApply({ [runtimeKey]: readChannelsRuntimeFieldValue(slider) }, 120);
            }
        });
        moduleQueueList.addEventListener('input', (event) => {
            const field = event.target instanceof Element ? event.target.closest('[data-channels-runtime-field]') : null;
            if (!(field instanceof HTMLInputElement || field instanceof HTMLTextAreaElement || field instanceof HTMLSelectElement)) return;
            if (field instanceof HTMLInputElement && (field.type === 'range' || field.type === 'checkbox')) return;
            const runtimeKey = safeString(field.getAttribute('data-channels-runtime-field'));
            if (!runtimeKey) return;
            queueChannelsRuntimeApply({ [runtimeKey]: readChannelsRuntimeFieldValue(field) }, 350);
        });
        moduleQueueList.addEventListener('change', (event) => {
            const field = event.target instanceof Element ? event.target.closest('[data-channels-runtime-field]') : null;
            if (!(field instanceof HTMLInputElement || field instanceof HTMLTextAreaElement || field instanceof HTMLSelectElement)) return;
            const runtimeKey = safeString(field.getAttribute('data-channels-runtime-field'));
            if (!runtimeKey) return;
            queueChannelsRuntimeApply({ [runtimeKey]: readChannelsRuntimeFieldValue(field) }, field instanceof HTMLInputElement && field.type === 'checkbox' ? 0 : 150);
        });
        moduleQueueList.addEventListener('click', (event) => {
            ...
            try {
                ...
                if (actionId === 'save_voice_settings') {
                    ...
                    moduleQueueList.querySelectorAll('[data-channels-voice-config-field]').forEach((field) => {
                        ...
                        if (!key) return;
                        if (field instanceof HTMLInputElement && field.type === 'checkbox') {
                            voiceConfigPayload[key] = Boolean(field.checked);
                            return;
                        }
                        const value = field instanceof HTMLInputElement || field instanceof HTMLTextAreaElement || field instanceof HTMLSelectElement
                            ? safeString(field.value)
    ...
    state.channelsRefreshId = window.setInterval(() => {
        if (safeString(sidebarNavMode) !== 'channels') return;
        const channelsState = moduleChannelsState();
        if (!channelsState || channelsState.loading || channelsState.requestPromise || channelsState.voiceProbeRunning) return;
        const active = document.activeElement instanceof HTMLElement ? document.activeElement : null;
        if (active && active.closest('[data-channels-config-form], [data-channels-search], [data-channels-runtime-field], [data-channels-voice-config-field], textarea, select')) {
            return;
        }
        void moduleRefreshChannels({ force: true });
    }, 4000);
