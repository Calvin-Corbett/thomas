...
let composerEvolvePanelOpen = false;
let composerEvolvePanelPlaceholder = '';
let composerEvolveWizardState = {
    step: 1,
    strategy: '',
    duration: '',
    scope: '',
...
let setupBootstrapSnapshot = null;
...
function missionLiveStatusOrder(statusRaw) {
    ...
    if (status === 'running' || status === 'active' || status === 'executing' || status === 'in_progress') return 0;
    if (status === 'awaiting_approval' || status === 'blocked' || status === 'awaiting_proof' || status === 'verified') return 1;
    if (status === 'queued' || status === 'pending' || status === 'requested' || status === 'classified' || status === 'claimed') return 2;
    return 3;
}
