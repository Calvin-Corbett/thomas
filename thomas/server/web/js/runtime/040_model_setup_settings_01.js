/* Shared theme, guardrails posture, and runtime-help helpers live in
 * 039a_model_setup_settings_shared.js to keep this file below the monolith cap. */
...
function initModelSetup() {
    ...
    const openModelSetupModal = () => {
        ...
        activeGuardrailsPosture = getCurrentGuardrailsPosture();
        setSegmentedControlSelection('setupGuardrailsGroup', activeGuardrailsPosture);
        wireRuntimeHelpTargets(modelSetupModal);
    ...
    bindSegmentedControl('setupGuardrailsGroup', val => {
        activeGuardrailsPosture = normalizeGuardrailsPosture(val);
        renderChatComposerSubbar();
    });
    ...
    applyModelSetupBtn.addEventListener('click', async () => {
        ...
        const desiredGuardrailsPosture = normalizeGuardrailsPosture(activeGuardrailsPosture);
        const currentGuardrailsPosture = getCurrentGuardrailsPosture();
        const patch = {
        ...
        try {
            ...
            if (desiredGuardrailsPosture !== currentGuardrailsPosture) {
                await applyGuardrailsPostureChange(desiredGuardrailsPosture, { suppressSuccess: true });
            }
...
/* Shared settings modal and utility helpers live in
 * 039b_model_setup_settings_helpers.js. */
...
/* Advanced chat physics and Install Guard settings helpers are shared from
 * 039b_model_setup_settings_helpers.js and 006b_install_guard_shared.js. */

async function loadSettings() {
    try {
        const [prefsRes, codexRes, setupRes] = await Promise.all([
        ...
        setupBootstrapSnapshot = setupRes.ok ? (setupRes.data || null) : setupBootstrapSnapshot;
        applyTheme(safeString(currentPreferences?.appearance?.theme) || 'auto');
        applyFontSize(toInt(currentPreferences?.appearance?.font_size, 16, 12, 28));
        applyInterfaceMotionPreference();
        ...
        syncSettingsInstallGuardUi(setupBootstrapSnapshot);
        ...
        activeGuardrailsPosture = normalizeGuardrailsPosture(currentPreferences?.advanced?.security?.guardrails_posture);
...
async function saveSettings() {
    try {
        ...
        const desiredInstallGuardProfile = normalizeInstallGuardProfile(settingSecurityProfile?.value || setupBootstrapSnapshot?.security_profile || 'balanced');
        const currentInstallGuardProfile = normalizeInstallGuardProfile(setupBootstrapSnapshot?.security_profile || 'balanced');
        ...
        const desiredGuardrailsPosture = normalizeGuardrailsPosture(settingAdvGuardrailsPosture?.value || activeGuardrailsPosture);
        const currentGuardrailsPosture = getCurrentGuardrailsPosture();
        const patch = {
            profile: {
        ...
        currentPreferences = await res.json();
        const desiredDiscordWakeWord = Boolean(patch?.voice?.wake_word_enabled);
        try {
            const discordRuntimeRes = await fetch('/api/channels/discord/runtime', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    voice_require_wake_word: desiredDiscordWakeWord,
                    restart_if_running: true,
                }),
            });
            if (!discordRuntimeRes.ok) {
                const details = await discordRuntimeRes.text();
                throw new Error(details || `HTTP ${discordRuntimeRes.status}`);
            }
        } catch (discordSyncError) {
            console.warn('Saved preferences, but Discord wake-word sync failed', discordSyncError);
            notifyUser('Settings saved, but Discord wake-word sync failed.', {
                tone: 'warning',
                durationMs: 2600,
                debugKind: 'settings-discord-sync',
            });
        }
        if (desiredInstallGuardProfile !== currentInstallGuardProfile) {
            const securityRes = await persistInstallGuardProfile(desiredInstallGuardProfile);
            setupBootstrapSnapshot = {
                ...(setupBootstrapSnapshot || {}),
                security_profile: normalizeInstallGuardProfile(securityRes?.security_profile || desiredInstallGuardProfile),
                security_capabilities: securityRes?.security_capabilities || setupBootstrapSnapshot?.security_capabilities || {},
                security_profiles: Array.isArray(securityRes?.security_profiles) ? securityRes.security_profiles : (setupBootstrapSnapshot?.security_profiles || []),
            };
        }
        if (desiredGuardrailsPosture !== currentGuardrailsPosture) {
            await applyGuardrailsPostureChange(desiredGuardrailsPosture, { suppressSuccess: true });
        ...
        updateDebugDockSnapshot();
        wireRuntimeHelpTargets(settingsModal instanceof HTMLElement ? settingsModal : document);
