...
function chatTaskStatusTone(statusRaw) {
    ...
    if (['defining', 'spawning', 'starting', 'pending', 'queued', 'requested', 'classified'].includes(status)) return 'starting';
...
function chatTaskStatusLabel(statusRaw) {
    const status = safeString(statusRaw).toLowerCase();
    if (status === 'defining') return 'Defining task';
    if (['queued', 'pending', 'requested', 'classified'].includes(status)) return 'Queued';
...
function chatTaskIsLive(statusRaw) {
    const status = safeString(statusRaw).toLowerCase();
    return Boolean(status) && !chatTaskIsTerminal(status) && !['quiet', 'waiting', 'paused', 'idle'].includes(status);
}
...
function chatShouldSeedTaskUi(promptRaw) {
    return chatPromptLooksTaskLike(promptRaw);
}
    ...
    if (!(strip instanceof HTMLElement)) {
        ...
        strip.innerHTML = `
            <div class="message-task-strip-head">
                ...
                <button type="button" class="message-task-strip-open" data-role="open">Open Mission Control</button>
            </div>
...
function updateMessageTaskStrip(messageId, patch = {}) {
    ...
    syncComposerBackgroundTaskBadge();
    chatTaskRuntimeEnsureTick();
    return state;
}

function syncComposerBackgroundTaskBadge() {
    if (!(composerStatusBar instanceof HTMLElement)) return;
    let badge = composerStatusBar.querySelector('[data-role="background-task"]');
    const states = Array.from(chatTaskStripStateByMessageId.values()).filter((state) => state && chatTaskIsLive(state.status));
    if (states.length === 0) {
        if (badge instanceof HTMLElement) badge.remove();
        composerStatusBar.classList.remove('has-background-task');
        return;
    }
    const primary = states[states.length - 1];
    if (!(badge instanceof HTMLElement)) {
        badge = document.createElement('span');
        badge.className = 'composer-status-badge composer-status-badge-background';
        badge.dataset.role = 'background-task';
        composerStatusBar.prepend(badge);
    }
    const count = states.length;
    const summary = chatTaskCheckpointText(primary.summary || primary.latestCheckpoint || 'Background task active.');
    badge.textContent = count > 1
        ? `${count} background tasks active`
        : `${chatTaskStatusLabel(primary.status)} in background`;
    badge.title = summary;
    composerStatusBar.classList.add('has-background-task');
}
...
function openMissionForTaskContext(context = {}) {
    const sessionToken = safeString(context?.sessionId || activeChatId || sessionId || taskContinuityLatestSessionId);
    if (sessionToken) {
        taskContinuityLatestSessionId = sessionToken;
    }
