/* Canvas scene graph, editing engine, persistence, API, and code generation.
 * Chrome and app_builder integration live in canvas_workspace_runtime.js;
 * semantic shared-edit registration lives in canvas_workspace_contract.js. */

(function () {
    'use strict';

    var canvasContract = window.ThomasCanvasWorkspaceContract;
    var workspaceRuntime = window.ThomasCanvasWorkspaceRuntime;
    ...
    function notify(message, opts) {
        ...
        try { console.log('[Canvas] ' + message); } catch (_e2) { /* ignore */ }
    }
    ...
    function specToReact(spec) {
        var lines = [];
        lines.push('// Auto-generated by Thomas Canvas — deterministic spec -> React + Tailwind');
    ...
    function mountCanvas(container, options) {
        ...
        if (!workspaceRuntime || typeof workspaceRuntime.buildChrome !== 'function') {
        ...
        function renderLayers() {
            ...
            nodes.slice().reverse().forEach(function (n) {
                ...
                if (canvasContract) canvasContract.annotate(row, 'layer', { instanceKey: n.id, label: n.name + ' layer' });
                ...
                lock.type = 'button';
                if (canvasContract) canvasContract.annotate(lock, 'action', { instanceKey: 'layer-lock-' + n.id, label: 'Lock or unlock ' + n.name });
        ...
        function aiTemplate() {
            ...
            if (api.offline) {
                offlineAiTimer = setTimeout(function () {
                    offlineAiTimer = null;
                    if (destroyed) return;
                    applyAiSpec(mockTemplateSpec(prompt), 'mock'); done();
                }, 350);
                return;
            }
            postJson(api.apiBase + '/template', { prompt: prompt, spec: api.spec, selection_id: selId }).then(function (res) {
                if (destroyed) return;
                var payload = responseData(res);
                if (payload && payload.spec) applyAiSpec(payload.spec, 'ai');
                else if (payload && payload.nodes) applyAiNodes(payload.nodes);
                else notify('AI returned no layout.', { tone: 'warn' });
                done();
            }).catch(function () { if (destroyed) return; notify('AI route unavailable — showing a sample layout.', { tone: 'warn' }); applyAiSpec(mockTemplateSpec(prompt), 'mock'); done(); });
        }
        ...
        function handleDrop(file) {
            ...
            var reader = new FileReader(); activeReader = reader;
            reader.onload = function () {
                ...
                }).catch(function () { if (destroyed) return; notify('Vision route unavailable — showing a sample.', { tone: 'warn' }); applyAiSpec(mockSketchSpec(), 'mock'); });
            };
        ...
        function onSketchChange() { var f = ui.sketchInput.files && ui.sketchInput.files[0]; ui.sketchInput.value = ''; if (f) handleDrop(f); }
        function onStageDragOver(e) { e.preventDefault(); ui.stageWrap.classList.add('is-drop-active'); }
        function onStageDragLeave(e) { if (e.target === ui.stageWrap) ui.stageWrap.classList.remove('is-drop-active'); }
        ...
        ui.sketchInput.addEventListener('change', onSketchChange);
        ui.stageWrap.addEventListener('dragover', onStageDragOver);
        ui.stageWrap.addEventListener('dragleave', onStageDragLeave);
        ui.stageWrap.addEventListener('drop', onStageDrop);
        ...
        var initialFrame = requestAnimationFrame(function () { initialFrame = null; if (!destroyed) { resize(); refreshPanels(); } });
        var initialResizeTimer = setTimeout(function () { initialResizeTimer = null; resize(); }, 60);
        ...
        var controller = {
            ...
            destroy: function () {
                ...
                if (autosaveTimer) clearTimeout(autosaveTimer);
                if (offlineAiTimer) clearTimeout(offlineAiTimer);
                if (activeReader && activeReader.readyState === FileReader.LOADING) { try { activeReader.abort(); } catch (_error) {} }
                activeReader = null;
                ...
                ui.sketchInput.removeEventListener('change', onSketchChange);
        ...
        function openCodeModal(files) {
            ...
            keys.forEach(function (k) {
                var b = document.createElement('button'); b.type = 'button'; b.className = 'ui-studio-btn'; b.textContent = k;
                if (canvasContract) canvasContract.annotate(b, 'action', { instanceKey: 'preview-tab-' + k, label: 'Show ' + k });
    ...
    function responseData(res) {
        if (canvasContract && typeof canvasContract.responseData === 'function') return canvasContract.responseData(res);
    ...
    if (workspaceRuntime && typeof workspaceRuntime.install === 'function') workspaceRuntime.install(window.UI_STUDIO);
})();
