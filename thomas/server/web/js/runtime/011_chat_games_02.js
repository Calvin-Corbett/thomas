...
async function runEvolveMissionJob(sendJob, { profile = '', modelId = '', passes = 1 } = {}) {
    const goal = safeString(sendJob?.text).trim();
    const chatSessionId = safeString(sessionId || activeChatId);
    const summaryLines = Array.isArray(sendJob?.wizardSummaryLines) ? sendJob.wizardSummaryLines : buildEvolveWizardSummaryLines();
    const payload = {
        kind: 'evolve_session',
        ...
        passes: Math.max(1, Number(passes) || 1),
    ...
    const assistantContent = `${jobName} is running in the background.${summaryText}
...
async function launchEvolveMissionFromComposer() {
    const text = buildEvolveWizardGoal();
    const wizardSummaryLines = buildEvolveWizardSummaryLines();
    const docsToSend = [...pendingDocs];
    const imagesToSend = [...pendingImages];
    const selectedProfile = modelSelector.value || setupProviderSelector.value;
    const studioChatContext = typeof moduleStudioBuildChatContext === 'function'
        ? moduleStudioBuildChatContext()
        : { enabled: false, preferredProfile: '', context: {} };
    ...
    const passCount = Math.max(1, Number(duration?.passes) || 1);
    ...
    if (docsToSend.length > 0 || imagesToSend.length > 0) {
        notifyUser('Evolve launches ignore file attachments. The current attachments were cleared before launch.', {
            tone: 'warning',
            durationMs: 2600,
            debugKind: 'mission-job',
        });
    ...
    try {
        await runEvolveMissionJob({
            text,
            docsToSend,
            imagesToSend,
            composerModeId: 'evolve',
            composerModeLabel: 'Evolve',
            wizardSummaryLines,
        }, {
            profile: resolvedProfile,
            modelId: resolveActiveModelIdForProfile(resolvedProfile) || '',
            passes: passCount,
        });
    } catch (error) {
...
async function handleSend() {
    ...
    if (composerEvolvePanelOpen) return;
