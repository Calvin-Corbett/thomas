...
async function missionRefresh({ force = false, silent = false } = {}) {
    ...
    try {
        ...
        const jobsParams = new URLSearchParams({ limit: String(MISSION_JOBS_LIMIT) });
        if (preferredSessionId) {
            jobsParams.set('session_id', preferredSessionId);
        }
        const [res, jobsRes] = await Promise.all([
            fetch('/api/mission/control'),
            fetch(`/api/mission/jobs?${jobsParams.toString()}`),
        ]);
        if (!res.ok) {
            const details = await res.text();
...
async function spendRefresh({ force = false } = {}) {
    if (!force && spendState.lastFetchedAt && (Date.now() - spendState.lastFetchedAt < 5000)) return;
    try {
