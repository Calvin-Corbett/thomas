async function applyOnboardingCompletion({ skippedInterview = false } = {}) {
    ...
    const onboardingAnswers = {
        ...
        install_guard_mode: normalizeInstallGuardProfile(easySetupState.securityProfile),
    };
...
function ensureChatComposerSubbar() {
    ...
    if (!(style instanceof HTMLStyleElement)) {
        ...
        style.textContent = `
            ...
            .chat-composer-control-label-wrap {
                display: inline-flex;
                align-items: center;
                gap: 6px;
            }
...
function renderChatComposerSubbar() {
    ...
    const labelWithHelp = (label, title, body) => (
        `<span class="chat-composer-control-label" data-runtime-help-title="${escapeHtml(title)}" data-runtime-help-body="${escapeHtml(body)}">${escapeHtml(label)}</span>`
    );
    ...
    root.innerHTML = `
        <div class="chat-composer-control" data-control="reasoning"${reasoningControl?.supported ? '' : ' hidden'}>
            ${labelWithHelp(
                safeString(reasoningControl?.label) || 'Reasoning',
                'Reasoning Effort',
                'Controls how much deliberate thinking Thomas spends before answering. Higher settings usually improve hard tasks, but they also take longer and use more tokens.'
            )}
        ...
        <div class="chat-composer-control" data-control="autonomy">
            ${labelWithHelp(
                safeString(autonomyControl?.label) || 'Autonomy',
                'Autonomy',
                'Controls how independently Thomas can work. Higher autonomy usually means more multi-step execution, delegated work, and more token usage.'
            )}
        ...
        <div class="chat-composer-control" data-control="token_economy">
            ${labelWithHelp(
                safeString(tokenControl?.label) || 'Token Economy',
                'Token Economy',
                'Controls how aggressively Thomas spends retries, context, and pass budget. Cheap lowers overhead; Maximum spends more for better coverage.'
            )}
        ...
        <div class="chat-composer-control" data-control="guardrails">
            ${labelWithHelp(
                'Guardrails',
                'Guardrails',
                'Controls trust and safety posture separately from token economy. Stronger guardrails can add overhead from approvals, verification, and extra checks, so they may increase token usage.'
            )}
            <select id="chatComposerGuardrailsSelect" class="chat-composer-control-select">
                ...
                ], safeString(activeGuardrailsPosture) || 'standard')}
            </select>
        </div>
    `;
    ...
    if (guardrailsSelect instanceof HTMLSelectElement) {
        guardrailsSelect.value = safeString(activeGuardrailsPosture) || 'standard';
        guardrailsSelect.addEventListener('change', async (event) => {
            const nextPosture = normalizeGuardrailsPosture(event.target.value);
            const previousPosture = normalizeGuardrailsPosture(activeGuardrailsPosture);
            if (nextPosture === previousPosture) return;
            try {
                await applyGuardrailsPostureChange(nextPosture, { suppressSuccess: true });
                activeGuardrailsPosture = nextPosture;
                if (settingAdvGuardrailsPosture) settingAdvGuardrailsPosture.value = nextPosture;
            } catch (error) {
                guardrailsSelect.value = previousPosture;
                activeGuardrailsPosture = previousPosture;
                notifyUser(
                    `Could not change guardrails: ${safeString(error?.message) || 'unknown error'}`,
                    {
                        tone: 'error',
                        durationMs: 3200,
                        debugKind: 'error',
