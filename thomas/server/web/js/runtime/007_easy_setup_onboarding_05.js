...
function updateEasySetupReadyList() {
    ...
    const installGuard = getInstallGuardOption(easySetupState.securityProfile);
    ...
    const rows = [
        ...
        `Install Guard: ${safeString(installGuard?.title) || 'Balanced'}`,
        depMissing > 0
...
async function loadEasySetupBootstrap() {
    ...
    setupBootstrapSnapshot = res.data;
    easySetupState.securityProfile = normalizeInstallGuardProfile(res.data?.security_profile);
    easySetupState.securityProfiles = Array.isArray(res.data?.security_profiles) ? res.data.security_profiles : [];
    return res.data;
}
...
async function persistOnboardingPrefs(overrides = {}) {
    ...
    const mergedAnswers = {
        ...
        install_guard_mode: normalizeInstallGuardProfile(
            Object.prototype.hasOwnProperty.call(overrides.answers || {}, 'install_guard_mode')
                ? overrides.answers.install_guard_mode
                : easySetupState.securityProfile
        ),
...
async function openEasySetup({ source = 'manual', force = false, restart = false } = {}) {
    ...
    try {
        ...
        easySetupState.securityProfile = normalizeInstallGuardProfile(
            onboarding?.answers?.install_guard_mode || bootstrap?.security_profile || easySetupState.securityProfile
        );
        easySetupState.securityProfiles = Array.isArray(bootstrap?.security_profiles) ? bootstrap.security_profiles : easySetupState.securityProfiles;
...
function initEasySetup() {
    ...
    if (easySetupSecurityProfileGrid) {
        easySetupSecurityProfileGrid.addEventListener('click', (event) => {
            const button = event.target.closest('[data-security-profile]');
            if (!(button instanceof HTMLElement)) return;
            easySetupState.securityProfile = normalizeInstallGuardProfile(button.dataset.securityProfile);
            renderEasySetupSecurityProfiles();
            updateEasySetupReadyList();
        });
    }
