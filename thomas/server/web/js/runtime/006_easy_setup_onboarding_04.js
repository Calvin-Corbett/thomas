...
function hideAssistantSuggestions({ force = false } = {}) {
    ...
    composerEvolvePanelOpen = false;
    composerEvolveWizardState = {
        step: 1,
        strategy: '',
        duration: '',
        scope: '',
    ...
    if (composerTextarea && composerEvolvePanelPlaceholder) {
        composerTextarea.placeholder = composerEvolvePanelPlaceholder;
    }
    composerEvolvePanelPlaceholder = '';
...
const EVOLVE_WIZARD_STRATEGIES = [
    {
        id: 'refactor',
        label: 'Refactor + simplify',
        summary: 'Improve structure, reduce debt, and make the codebase easier to maintain.',
        prompt: 'Prefer a structural code-quality upgrade. Refactor tangled code, reduce duplication, simplify control flow, and improve maintainability without inventing random features.',
    },
    {
        id: 'reliability',
        label: 'Reliability + hardening',
        summary: 'Look for brittle behavior, weak error handling, and verification gaps.',
        prompt: 'Prefer a reliability-focused upgrade. Strengthen error handling, reduce brittle behavior, improve guardrails, and prioritize practical correctness over aesthetics.',
    },
    {
        id: 'capability',
        label: 'Capability upgrade',
        summary: 'Add a meaningful product or workflow improvement if verification supports it.',
        prompt: 'Prefer a product capability upgrade. Choose a useful improvement that adds real leverage to Thomas while still keeping the change coherent and verifiable.',
    },
    {
        id: 'custom',
        label: 'Custom goal',
        summary: 'You define the evolve target in the composer before continuing.',
        prompt: '',
    },
];

const EVOLVE_WIZARD_DURATIONS = [
    {
        id: 'quick',
        label: 'Quick',
        summary: 'One pass. Smallest useful upgrade.',
        passes: 1,
        prompt: 'Keep the run short and decisive. Favor the smallest worthwhile upgrade.',
    },
    {
        id: 'standard',
        label: 'Standard',
        summary: 'Two passes. Default balance between depth and speed.',
        passes: 2,
        prompt: 'Take a balanced amount of time. Explore a bit, but stay practical.',
    },
    {
        id: 'deep',
        label: 'Deep',
        summary: 'Three passes. More exploration and a higher token budget.',
        passes: 3,
        prompt: 'Use a deeper evolve run. Spend more time exploring alternatives before settling on the strongest upgrade.',
    },
];

const EVOLVE_WIZARD_SCOPES = [
    {
        id: 'surgical',
        label: 'Surgical',
        summary: 'Keep the change tight and low-risk.',
        prompt: 'Keep scope surgical. Favor a compact, low-risk patch over broad churn.',
    },
    {
        id: 'focused',
        label: 'Focused subsystem',
        summary: 'Allow one coherent subsystem-level change.',
        prompt: 'Scope the change to one coherent subsystem if needed, but keep the work intentional and bounded.',
    },
    {
        id: 'broad',
        label: 'Broad sweep',
        summary: 'Allow a wider multi-file improvement if it clearly pays off.',
        prompt: 'A broader multi-file change is acceptable if it materially improves the product and still verifies cleanly.',
    },
];

function getEvolveWizardStrategy() {
    return EVOLVE_WIZARD_STRATEGIES.find((option) => option.id === safeString(composerEvolveWizardState.strategy)) || null;
}

function getEvolveWizardDuration() {
    return EVOLVE_WIZARD_DURATIONS.find((option) => option.id === safeString(composerEvolveWizardState.duration)) || null;
}

function getEvolveWizardScope() {
    return EVOLVE_WIZARD_SCOPES.find((option) => option.id === safeString(composerEvolveWizardState.scope)) || null;
}

function setEvolveWizardPlaceholderForCurrentStep() {
    if (!(composerTextarea instanceof HTMLTextAreaElement)) return;
    if (!composerEvolvePanelPlaceholder) {
        composerEvolvePanelPlaceholder = composerTextarea.placeholder || '';
    }
    if (safeString(composerEvolveWizardState.strategy) === 'custom' && Number(composerEvolveWizardState.step) === 1) {
        composerTextarea.placeholder = 'Describe the custom evolve goal you want Thomas to pursue.';
        return;
    }
    composerTextarea.placeholder = 'The evolve wizard is driving this flow. Use the buttons above.';
}

function buildEvolveWizardGoal() {
    const strategy = getEvolveWizardStrategy();
    const duration = getEvolveWizardDuration();
    const scope = getEvolveWizardScope();
    const customText = safeString(composerTextarea?.value);
    const promptParts = [];
    if (strategy?.id === 'custom') {
        promptParts.push(customText || 'Choose the highest-leverage self-improvement available and explain the reasoning through the resulting change.');
    } else if (strategy?.prompt) {
        promptParts.push(strategy.prompt);
    }
    if (duration?.prompt) promptParts.push(duration.prompt);
    if (scope?.prompt) promptParts.push(scope.prompt);
    if (customText && strategy?.id !== 'custom') {
        promptParts.push(`Additional operator guidance: ${customText}`);
    }
    return promptParts.filter(Boolean).join('\n\n');
}

function buildEvolveWizardSummaryLines() {
    const strategy = getEvolveWizardStrategy();
    const duration = getEvolveWizardDuration();
    const scope = getEvolveWizardScope();
    return [
        strategy ? `Strategy: ${strategy.label}` : '',
        duration ? `Run depth: ${duration.label}` : '',
        scope ? `Change scope: ${scope.label}` : '',
    ].filter(Boolean);
}

function evolveWizardProgressMarkup(step) {
    return `
        <div class="assistant-evolve-progress" aria-label="Evolve wizard progress">
            ${[1, 2, 3, 4].map((value) => `
                <span class="assistant-evolve-progress-dot${value === step ? ' active' : ''}${value < step ? ' done' : ''}">${value}</span>
            `).join('')}
        </div>
    `;
}

function renderEvolveWizardStep() {
    if (!assistantSuggestionRail || !assistantSuggestionBubbles) return;
    const step = Math.max(1, Math.min(4, Number(composerEvolveWizardState.step) || 1));
    composerEvolveWizardState.step = step;
    composerEvolvePanelOpen = true;
    setEvolveWizardPlaceholderForCurrentStep();

    suggestionRailVersion += 1;
    suggestionContext = 'evolve_launch';
    suggestionDismissible = true;
    assistantSuggestionRail.classList.remove('hidden');
    assistantSuggestionRail.dataset.context = suggestionContext;
    if (assistantSuggestionTitle) assistantSuggestionTitle.textContent = 'Evolve Thomas';
    if (assistantSuggestionDismissBtn) assistantSuggestionDismissBtn.classList.remove('hidden');

    let bodyMarkup = '';

    if (step === 1) {
        const customSelected = safeString(composerEvolveWizardState.strategy) === 'custom';
        bodyMarkup = `
            <section class="assistant-evolve-panel assistant-evolve-step-panel" aria-label="Evolve wizard step 1">
                ${evolveWizardProgressMarkup(1)}
                <div class="assistant-evolve-stage-head">
                    <span class="assistant-evolve-step-label">Step 1</span>
                    <h3>How should Thomas evolve?</h3>
                    <p>Pick the kind of upgrade Thomas should hunt for. This choice becomes the core evolve prompt.</p>
                </div>
                <div class="assistant-evolve-choice-grid">
                    ${EVOLVE_WIZARD_STRATEGIES.map((option) => `
                        <button
                            type="button"
                            class="assistant-evolve-choice${safeString(composerEvolveWizardState.strategy) === option.id ? ' selected' : ''}"
                            data-evolve-strategy="${option.id}">
                            <strong>${escapeHtml(option.label)}</strong>
                            <span>${escapeHtml(option.summary)}</span>
                        </button>
                    `).join('')}
                </div>
                <div class="assistant-evolve-footer">
                    ${customSelected ? '<p class="assistant-evolve-footnote">Write the custom evolve goal in the composer below, then continue.</p>' : '<p class="assistant-evolve-footnote">You can leave the composer blank unless you want to add extra guidance.</p>'}
                    <div class="assistant-evolve-actions">
                        <button type="button" class="assistant-evolve-btn" data-evolve-action="cancel">Cancel</button>
                        <button type="button" class="assistant-evolve-btn primary" data-evolve-action="next-step-1">Continue</button>
                    </div>
                </div>
            </section>
        `;
        ...
        bodyMarkup = `
            <section class="assistant-evolve-panel assistant-evolve-step-panel" aria-label="Evolve wizard step 2">
                ${evolveWizardProgressMarkup(2)}
                <div class="assistant-evolve-stage-head">
                    <span class="assistant-evolve-step-label">Step 2</span>
                    <h3>How long should the run spend evolving?</h3>
                    <p>Choose how many evolve passes Thomas should spend before stopping to verify.</p>
                </div>
                <div class="assistant-evolve-choice-grid compact">
                    ${EVOLVE_WIZARD_DURATIONS.map((option) => `
                        <button
                            type="button"
                            class="assistant-evolve-choice${safeString(composerEvolveWizardState.duration) === option.id ? ' selected' : ''}"
                            data-evolve-duration="${option.id}">
                            <strong>${escapeHtml(option.label)}</strong>
                            <span>${escapeHtml(option.summary)}</span>
                        </button>
                    `).join('')}
                ...
                <div class="assistant-evolve-actions">
                    <button type="button" class="assistant-evolve-btn" data-evolve-action="back">Back</button>
                </div>
            </section>
        `;
    } else if (step === 3) {
        bodyMarkup = `
            <section class="assistant-evolve-panel assistant-evolve-step-panel" aria-label="Evolve wizard step 3">
                ${evolveWizardProgressMarkup(3)}
                <div class="assistant-evolve-stage-head">
                    <span class="assistant-evolve-step-label">Step 3</span>
                    <h3>How wide can the change reach?</h3>
                    <p>Set the scope Thomas is allowed to take if it finds a worthwhile upgrade.</p>
                </div>
                <div class="assistant-evolve-choice-grid compact">
                    ${EVOLVE_WIZARD_SCOPES.map((option) => `
                        <button
                            type="button"
                            class="assistant-evolve-choice${safeString(composerEvolveWizardState.scope) === option.id ? ' selected' : ''}"
                            data-evolve-scope="${option.id}">
                            <strong>${escapeHtml(option.label)}</strong>
                            <span>${escapeHtml(option.summary)}</span>
                        </button>
                    `).join('')}
        ...
        const summaryLines = buildEvolveWizardSummaryLines();
        bodyMarkup = `
            <section class="assistant-evolve-panel assistant-evolve-step-panel" aria-label="Evolve wizard step 4">
                ${evolveWizardProgressMarkup(4)}
                <div class="assistant-evolve-stage-head">
                    <span class="assistant-evolve-step-label">Step 4</span>
                    <h3>Final warning</h3>
                    <p>Review the run shape, then explicitly approve the evolve mission.</p>
                </div>
                <div class="assistant-evolve-summary-card">
                    ${summaryLines.map((line) => `<div class="assistant-evolve-summary-line">${escapeHtml(line)}</div>`).join('')}
                </div>
                <div class="assistant-evolve-card warning compact-warning">
                    <span class="assistant-evolve-kicker">Warning</span>
                    <strong>This may produce unknown or unprecedented results.</strong>
                    <p>Thomas will evolve inside the green mirror, verify there, and only then report back. Review the output before any promotion.</p>
                </div>
                <div class="assistant-evolve-actions">
                    <button type="button" class="assistant-evolve-btn" data-evolve-action="back">Back</button>
                    <button type="button" class="assistant-evolve-btn primary" data-evolve-action="accept">I accept. Run evolve.</button>
                </div>
            </section>
        `;
    }

    assistantSuggestionBubbles.innerHTML = bodyMarkup;
    assistantSuggestionBubbles.dataset.mode = 'wizard';
    assistantSuggestionBubbles.dataset.ready = 'true';

    assistantSuggestionBubbles.querySelectorAll('[data-evolve-strategy]').forEach((button) => {
        if (!(button instanceof HTMLButtonElement)) return;
        button.addEventListener('click', () => {
            composerEvolveWizardState.strategy = safeString(button.dataset.evolveStrategy);
            renderEvolveWizardStep();
            if (safeString(composerEvolveWizardState.strategy) === 'custom' && composerTextarea) {
                composerTextarea.focus();
            }
        });
    });

    assistantSuggestionBubbles.querySelectorAll('[data-evolve-duration]').forEach((button) => {
        if (!(button instanceof HTMLButtonElement)) return;
        button.addEventListener('click', () => {
            composerEvolveWizardState.duration = safeString(button.dataset.evolveDuration);
            composerEvolveWizardState.step = 3;
            renderEvolveWizardStep();
        });
    });

    assistantSuggestionBubbles.querySelectorAll('[data-evolve-scope]').forEach((button) => {
        if (!(button instanceof HTMLButtonElement)) return;
        button.addEventListener('click', () => {
            composerEvolveWizardState.scope = safeString(button.dataset.evolveScope);
            composerEvolveWizardState.step = 4;
            renderEvolveWizardStep();
        });
    });

    assistantSuggestionBubbles.querySelectorAll('[data-evolve-action]').forEach((button) => {
        if (!(button instanceof HTMLButtonElement)) return;
        button.addEventListener('click', async () => {
            const action = safeString(button.dataset.evolveAction);
            if (action === 'cancel') {
                hideAssistantSuggestions({ force: true });
            ...
            if (action === 'back') {
                composerEvolveWizardState.step = Math.max(1, step - 1);
                renderEvolveWizardStep();
                return;
            }
            if (action === 'next-step-1') {
                if (!safeString(composerEvolveWizardState.strategy)) {
                    notifyUser('Pick how Thomas should evolve first.', {
                        tone: 'warning',
                        durationMs: 1800,
                ...
                if (safeString(composerEvolveWizardState.strategy) === 'custom' && !safeString(composerTextarea?.value)) {
                    notifyUser('Write the custom evolve goal in the composer before continuing.', {
                        tone: 'warning',
                        durationMs: 2200,
                        debugKind: 'mission-job',
                    });
                ...
                composerEvolveWizardState.step = 2;
                renderEvolveWizardStep();
                return;
            }
            if (action === 'accept') {
                ...
                try {
                    await launchEvolveMissionFromComposer();
                } finally {
                    if (composerEvolvePanelOpen) {
...
function showEvolveLaunchPanel() {
    if (!(composerTextarea instanceof HTMLTextAreaElement)) return;
    if (!composerEvolvePanelPlaceholder) {
        composerEvolvePanelPlaceholder = composerTextarea.placeholder || '';
    }
