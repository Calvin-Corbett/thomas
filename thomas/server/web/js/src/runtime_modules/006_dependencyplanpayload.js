...
function blockEasySetupClose(source = 'close_request') {
    const message = withAgentName(
        'Finish Easy Setup before {{agent}} can use memory, tools, or automation safely.'
    );
...
async function openEasySetup({ source = 'manual', force = false, restart = false } = {}) {
    ...
    try {
        ...
        if (easySetupRecommendedHint) {
            ...
            if (recommendedPath) {
                easySetupRecommendedHint.textContent = `Recommended first path: ${recommendedPath}. ${quickStartReason}`;
            } else {
                easySetupRecommendedHint.textContent =
                    quickStartReason || 'Pick the simplest path that works for you. Thomas verifies it before applying anything.';
            }
        }
    } catch (err) {
        if (easySetupRecommendedHint) {
    ...
    setEasySetupStatus(easySetupConnectionStatus, 'Verify the connection to continue.');
