...
from thomas.preferences._utils import normalize_profile_type
from thomas.preferences.guardrails_policy import guardrails_patch_weakening_fields
...
def register_preferences_routes(
    ...
    *,
    ...
    async def api_preferences_patch(request: web.Request) -> web.Response:
        ...
        thread_id = _parse_thread_id(request)
        user_id = _get_user_id(request)
        current = store.get(user_id=user_id, thread_id=thread_id)
        current_profile_type = normalize_profile_type(
            getattr(current.profile, "profile_type", None), default="adaptive"
        )
        incoming_profile_type = (
            normalize_profile_type(getattr(patch.profile, "profile_type", None), default=current_profile_type)
            if patch.profile is not None and "profile_type" in patch.profile.model_fields_set
            else current_profile_type
        )
        violations = guardrails_patch_weakening_fields(
            current_advanced=current.advanced.model_dump(),
            incoming_advanced_patch=patch.advanced.model_dump(exclude_unset=True) if patch.advanced is not None else {},
            ignore_runtime_fields=incoming_profile_type == "non_coder",
        )
        if violations:
            raise web.HTTPBadRequest(
                text=(
                    "guardrail weakening must be changed via dedicated /api/security/guardrails-posture route: "
                    + ", ".join(violations)
                )
            )
        try:
