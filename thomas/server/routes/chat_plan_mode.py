...
_WEB_SLASH_SPECS = (
    ...
    ("/model", "Switch or list model profiles", "/model [name|id:...]"),
    ("/status", "Show session status", "/status"),
    ("/plan", "Enter plan mode", "/plan [prompt]"),
)
...
def normalize_conversation_mode(raw: Any) -> str:
    ...
    if mode in {"plan", "default", "production"}:
        return mode
    return "default"
...
def serialize_web_slash_specs() -> list[dict[str, Any]]:
    ...
    for command, summary, usage in _WEB_SLASH_SPECS:
        specs.append(
            {
                "cmd": command,
                "desc": summary,
