...
from thomas.server.app_keys import APP_APPROVALS_BROKER, APP_CONFIG
...
def build_mission_control_routes(
    ...
    *,
    ...
    def _task_bot_room_and_summary(row: dict[str, Any]) -> tuple[str, str]:
        state = str(row.get("state") or "").strip().lower()
        summary = _trim_summary(row.get("summary") or row.get("progress_summary") or row.get("task_id") or "Task")
        room = {
            "requested": "inbox",
            "classified": "planning",
            "queued": "inbox",
    ...
    def _task_bot_status_label(row: dict[str, Any]) -> str:
        return str(row.get("state") or "").strip().lower() or "unknown"

    def _task_bot_agents_and_events() -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        agents: list[dict[str, Any]] = []
        events: list[dict[str, Any]] = []
        if app.get(APP_CONFIG) is None:
            return agents, events
        try:
            rows = list(task_bot_runtime.list_executions(refresh=True) or [])
        except OSError:
            rows = []
        active_statuses = {"requested", "classified", "queued", "claimed", "executing", "awaiting_proof", "verified", "blocked"}
        active_rows = [row for row in rows if str(row.get("state") or "").strip().lower() in active_statuses]
        ended_rows = [row for row in rows if str(row.get("state") or "").strip().lower() not in active_statuses]
        selected_rows = active_rows[:32] + ended_rows[:16]
        ...
        for row in selected_rows:
            ...
            if isinstance(full, dict) and full:
                row = full
            room, summary = _task_bot_room_and_summary(row)
            status = _task_bot_status_label(row)
            updated_at = _coerce_iso(row.get("updated_at") or row.get("created_at"))
            ...
            agents.append(
                {
                    "id": f"taskbot:{execution_id}",
                    "source": "task_bot_execution",
                    "kind": "task_execution",
                    "name": _trim_summary(row.get("summary") or task_id or execution_id, 84),
                    "room": room,
                    "status": status,
                    "summary": summary,
                    ...
                    "task_id": task_id,
                    "backend_type": str(row.get("backend_type") or "task_manager"),
            ...
            transition_summary = str((last_transition or {}).get("summary") or summary).strip()
            events.append(
                {
                    "id": f"evt:taskbot:{execution_id}:{status}",
                    "source": "task_bot_execution",
                    "agent_id": f"taskbot:{execution_id}",
                    "run_id": "",
                    "ts": _coerce_iso((last_transition or {}).get("occurred_at") or updated_at),
                    "type": status or "task",
                    "text": _trim_summary(transition_summary or summary, 160),
                }
            )
        return agents, events
    ...
    def _build_mission_control_payload() -> dict[str, Any]:
        ...
        task_bot_agents, task_bot_events = _task_bot_agents_and_events()
        if task_bot_agents:
            agents.extend(task_bot_agents)
        if task_bot_events:
            events.extend(task_bot_events)
