    ...
    *,
    ...
    async def api_chat(request: web.Request) -> web.StreamResponse:
        ...
        if not await deps.begin_session_run(sid):
            ...
            if not wants_fork:
                raise web.HTTPConflict(text=f"session is already processing another request (busy_strategy={busy_strategy})")
    ...
    async def _api_chat_inner(
        ...
        if fallback_cfgs:
            ...
            for fb_cfg in fallback_cfgs:
                fb_key = (
                    f"{getattr(fb_cfg, 'name', '')}|{getattr(fb_cfg, 'provider', '')}|{getattr(fb_cfg, 'model', '')}"
                )
        ...
        if advanced_tools is not None:
            if runtime_prefs_saved:
                ...
                allow_shell = bool(config.tools.allow_shell)
            ...
            blocked_commands = [
                s.strip().lower()
                for s in raw_blocked.replace("\r", "\n").replace("\n", ",").split(",")
                if s.strip()
            ]
