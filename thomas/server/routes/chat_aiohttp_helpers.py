"""Helper utilities and dependencies for chat aiohttp routes."""
...
def _clone_conversation_fallback(value: Any) -> Any:
    """Safely clone conversation data with fallback for complex types."""
...
def _create_parallel_fork_session(
    ...
    """Create a parallel fork session from a parent session."""
    import secrets
...
def _appkey_identity(key: Any) -> str:
    """Extract AppKey identity from repr string."""
    rep = repr(key)
...
def _resolve_app_value(
    ...
    *,
    ...
    """Resolve app value by key, with type checking and fallback to identity lookup."""
    value = app.get(key)
...
_SESSION_MSG_QUEUES: dict[str, Any] = {}
