...
_DISCUSSION_ONLY_RISK_REQUEST = re.compile(
    r"^\s*(?:"
    r"explain|describe|document|discuss|review|audit|compare|"
    r"show\s+me\s+how|tell\s+me\s+how|"
    r"how\s+(?:do|does|would|could|can|should)|"
    r"what\s+(?:would|could|does|happens)|"
    r"plan\s+(?:how|a\s+safe\s+way)"
    r")\b",
    re.IGNORECASE,
)
_NEGATED_RISK_ACTION = re.compile(
    r"\b(?:do\s+not|don't|never|without)\s+(?:actually\s+)?"
    r"(?:deploy(?:ing)?|publish(?:ing)?|releas(?:e|ing)|ship(?:ping)?|push(?:ing)?|"
    r"overwrit(?:e|ing)|discard(?:ing)?|eras(?:e|ing)|delet(?:e|ing)|remov(?:e|ing)|"
    r"wip(?:e|ing)|destroy(?:ing)?)\b",
    re.IGNORECASE,
)
_DISCUSSION_THEN_ACTION = re.compile(
    r"(?:\b(?:and|then|also|afterward|afterwards)\b|[.;]\s*)"
    r"[^\r\n]{0,40}\b(?:deploy|publish|release|ship|push|overwrite|discard|erase|"
    r"delete|remove|wipe|destroy|make|take|put|go)\b",
    re.IGNORECASE,
)

# Each row names the semantic risk shown to the user. Patterns deliberately
# cover shell syntax *and* ordinary language so changing a verb (for example,
# "make it live" instead of "deploy") cannot bypass the approval boundary.
_RISKY_CODE_INTENTS: tuple[tuple[str, tuple[re.Pattern[str], ...]], ...] = (
    (
        "deploy, publish, or release externally",
        (
            re.compile(
                r"^\s*(?:please\s+)?(?:deploy|publish|ship)\b|"
                r"\b(?:deploy|publish|ship|promote|launch)\b[^\r\n]{0,100}"
                r"\b(?:production|prod|staging|public|live|customers?|users?)\b|"
                r"\brelease\s+(?:(?:this|the|our|my|current|latest)\s+){0,3}"
                r"(?:build|app|application|site|service|version|product)\b|"
                r"\b(?:make|take|put)\b[^\r\n]{0,80}\b(?:go\s+)?live\b|"
                r"\bgo\s+live\b",
                re.IGNORECASE,
            ),
    ...
    (
        ...
        (
            re.compile(
                r"(?:\b(?:git\s+)?push\b[^\r\n]{0,100}?(?:--force(?:-with-lease)?|(?<!\S)-f(?:\s|$))|"
                r"\bforce[- ]push\b)",
                re.IGNORECASE,
            ),
            re.compile(
                r"\b(?:overwrite|replace|rewrite|clobber)\b[^\r\n]{0,100}"
                r"\b(?:remote|origin|upstream|remote\s+branch|remote\s+history)\b|"
                r"\bpush\s+(?:this|the|my|our)?\s*(?:code|changes|branch|commit)?\s*"
                r"(?:to\s+)?(?:github|origin|upstream|the\s+remote)\b",
                re.IGNORECASE,
            ),
        ),
    ),
    (
        ...
        (
            re.compile(r"\bgit\s+reset\b[^\r\n]{0,80}?--hard\b", re.IGNORECASE),
            re.compile(
                r"\bgit\s+(?:clean\b[^\r\n]{0,80}?-[a-z]*f|checkout\s+--\s+\.|restore\s+\.)",
                re.IGNORECASE,
            ),
            re.compile(
                r"\b(?:discard|drop|throw\s+away|erase|wipe|delete|remove|revert)\b"
                r"[^\r\n]{0,100}\b(?:uncommitted|unstaged|untracked|local|working[- ]tree)\b"
                r"[^\r\n]{0,60}\b(?:changes?|edits?|files?|work)\b|"
                r"\b(?:discard|drop|throw\s+away|erase|wipe|delete|remove|revert)\b"
                r"[^\r\n]{0,40}\b(?:all|every)\b[^\r\n]{0,40}\b(?:local\s+changes|working[- ]tree\s+changes)\b",
                re.IGNORECASE,
            ),
        ),
    ),
    (
        "erase tracked files",
        (
            re.compile(r"\bgit\s+rm\b[^\r\n]{0,100}(?:-[a-z]*r|--recursive)", re.IGNORECASE),
            re.compile(
                r"\b(?:erase|delete|remove|wipe|destroy)\b[^\r\n]{0,100}"
                r"\b(?:tracked|version[- ]controlled)\b[^\r\n]{0,60}\b(?:files|content|tree)\b|"
                r"\b(?:erase|delete|remove|wipe|destroy)\b[^\r\n]{0,40}\b(?:all|every)\b"
                r"[^\r\n]{0,60}\btracked\s+files\b",
                re.IGNORECASE,
            ),
        ),
    ),
    (
        "recursively delete repository content",
        (
            re.compile(
                r"\brm\b(?=[^\r\n]{0,80}(?:--recursive|-[a-z]*r[a-z]*))"
                r"(?=[^\r\n]{0,80}(?:--force|-[a-z]*f[a-z]*))[^\r\n]{0,80}",
                re.IGNORECASE,
            ),
            re.compile(
                r"\b(?:rmdir\s+/s(?:\s+/q)?|del\s+/s(?:\s+/q)?|"
                r"remove-item\b[^\r\n]{0,100}?(?=[^\r\n]*-recurse\b)(?=[^\r\n]*-force\b))",
                re.IGNORECASE,
            ),
            re.compile(
                r"\b(?:delete|remove|wipe|erase|destroy)\s+(?:(?:the|this|entire|whole)\s+)?"
                r"(?:git\s+)?(?:repo(?:sitory)?|codebase)(?:\s+(?:folder|directory|tree))?\b|"
                r"\b(?:destructive|recursive|complete|total)\s+(?:repo(?:sitory)?|codebase)\s+"
                r"(?:delete|deletion|removal)\b|\b(?:repo(?:sitory)?|codebase)\s+"
                r"(?:delete|deletion|removal)\b",
                re.IGNORECASE,
            ),
        ),
    ),
    (
        "perform an external or system-wide side effect",
        (
            re.compile(
                r"\b(?:send\s+(?:an?\s+)?email|delete\s+(?:all|the\s+database|production)|"
                r"drop\s+(?:database|table)|format\s+(?:disk|drive)|"
                r"install\s+(?:globally|system[- ]wide))\b",
                re.IGNORECASE,
            ),
        ),
    ),
...
def _risky_code_action(message: str) -> str:
    text = str(message or "").strip()
    if not text:
        return ""
    # Remove only the explicitly negated action phrase. Returning early on any
    # negation would let a later compound command bypass the boundary (for
    # example, "do not deploy; overwrite the remote instead").
    actionable_text = _NEGATED_RISK_ACTION.sub("", text)
    if _DISCUSSION_ONLY_RISK_REQUEST.search(text) and not _DISCUSSION_THEN_ACTION.search(actionable_text):
        return ""
    for risk, patterns in _RISKY_CODE_INTENTS:
        if any(pattern.search(actionable_text) for pattern in patterns):
            return risk
    return ""
