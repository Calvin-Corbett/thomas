...
from .discord_channels_support import (
...
def _should_start_task_manager_dispatch(dispatch_decision: Any, payload: dict[str, Any]) -> bool:
    _ = payload
    return str(getattr(dispatch_decision, "action", "") or "").strip().lower() == "dispatch"
...
async def execute_chat_request(
    ...
    discord_command_reply = await maybe_handle_discord_chat_command(
        request=request,
        session=session,
        sid=sid,
        text=text,
    ...
    if discord_command_reply is not None:
        return discord_command_reply
    ...
    # ─── Dispatch routing ────────────────────────────────────────────────────
    dispatch_decision: Any = None
    try:
        from thomas.agent.dispatch import should_dispatch

        dispatch_decision = should_dispatch(text)
    except ImportError:
        log.debug("Dispatch classifier not available, using inline agent loop")
    except Exception as dispatch_exc:
        log.warning("Dispatch classification failed, using inline agent loop: %s", dispatch_exc)
    ...
    if dispatch_decision is not None and _should_start_task_manager_dispatch(dispatch_decision, payload):
        ...
        task_manager_ready = is_task_manager_dispatch_ready()

    if dispatch_decision is not None and _should_start_task_manager_dispatch(dispatch_decision, payload) and task_manager_ready:
        ...
        try:
            ack_text = await stream_task_start_acknowledgment(
                llm,
                user_text=text,
                emit_text=lambda chunk: dispatch_send({"type": "text", "text": chunk}),
            )
            async with session_lock:
            ...
            await dispatch_send({"type": "error", "error": str(dispatch_exc)})
        ...
        if dispatch_result.ok:
            ...
            await dispatch_send(
                {
                    "type": "error",
                    "error": dispatch_result.error or "Task-manager dispatch failed.",
                }
            )
        ...
        with contextlib.suppress(Exception):
            await dispatch_resp.write_eof()
        return dispatch_resp
    if dispatch_decision is not None and _should_start_task_manager_dispatch(dispatch_decision, payload) and not task_manager_ready:
        log.info("Task-manager lane unavailable for chat session %s; falling back to inline agent loop.", sid[:12])
