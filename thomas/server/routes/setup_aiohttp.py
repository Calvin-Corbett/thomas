...
from thomas.core.config import AppConfig, normalize_security_profile, security_profile_options
...
def _config_path_for_security_profile(cfg: AppConfig) -> Path:
    configured = getattr(cfg, "config_path", None)
    if configured:
        return Path(str(configured)).resolve()
    return Path(__file__).resolve().parents[3] / "thomas.toml"


def _write_security_profile(path: Path, profile: str) -> None:
    profile_line = f'profile = "{profile}"'
    content = path.read_text(encoding="utf-8") if path.exists() else ""
    if re.search(r"(?ms)^\[security\]\s*(?P<body>.*?)(?=^\[|\Z)", content):

        def _replace_section(match: re.Match[str]) -> str:
            body = str(match.group("body") or "")
            if re.search(r'(?m)^\s*profile\s*=\s*"[^"]*"\s*$', body):
                next_body = re.sub(r'(?m)^\s*profile\s*=\s*"[^"]*"\s*$', profile_line, body)
            else:
                suffix = "" if body.endswith(("\n", "\r")) or not body else "\n"
                next_body = f"{body}{suffix}{profile_line}\n"
            return f"[security]\n{next_body}"

        updated = re.sub(r"(?ms)^\[security\]\s*(?P<body>.*?)(?=^\[|\Z)", _replace_section, content, count=1)
    else:
        trimmed = content.rstrip()
        updated = f"{trimmed}\n\n[security]\n{profile_line}\n" if trimmed else f"[security]\n{profile_line}\n"
...
def register_setup_routes(
    ...
    *,
        ...
        return web.json_response(
            {
                ...
                "security_capabilities": cfg.security_capabilities(),
                "security_profiles": security_profile_options(),
                "quick_start": {
    ...
    async def api_setup_repair(request: web.Request) -> web.Response:
        ...
        auto_install = _as_bool(payload.get("auto_install_tools"), False)
        skip_install = _as_bool(payload.get("skip_install"), False)
        skip_doctor = _as_bool(payload.get("skip_doctor"), False)
        capabilities = cfg.security_capabilities()
        if auto_install and not bool(capabilities.get("tool_install_allowed")):
            raise web.HTTPBadRequest(
                text=str(
                    capabilities.get("blocked_actions", {}).get("tool_install")
                    or "Tool installation is blocked by the active security profile."
                )
            )
        ...
        if not auto_install:
            ...
            cmd.append("-ConfirmedToolInstall")
    ...
    async def api_setup_security_profile(request: web.Request) -> web.Response:
        """Update the repo-backed install guard mode for this Thomas checkout."""
        require_api_access(request)
        require_loopback(request)
        if request.method.upper() != "POST":
            raise web.HTTPMethodNotAllowed(method=request.method, allowed_methods=["POST"])
        ...
        if raw_profile is None:
            raise web.HTTPBadRequest(text="profile is required")
        try:
            normalized = normalize_security_profile(str(raw_profile))
        except ValueError as exc:
            raise web.HTTPBadRequest(text=str(exc)) from exc

        cfg: AppConfig = request.app[APP_CONFIG]
        config_path = _config_path_for_security_profile(cfg)
        try:
            await asyncio.to_thread(_write_security_profile, config_path, normalized)
        except Exception as exc:
            raise web.HTTPInternalServerError(
                text=f"failed to persist security profile: {type(exc).__name__}: {exc}"
            ) from exc

        cfg.security.profile = normalized
        return web.json_response(
            {
                "ok": True,
                "security_profile": cfg.security_profile,
    ...
    async def api_local_pull(request: web.Request) -> web.StreamResponse:
        ...
        try:
            ...
            async with (
                ...
                client.stream("POST", url, json={"name": model_id}) as r,
            ):
    ...
    app.router.add_post("/api/setup/security-profile", api_setup_security_profile)
