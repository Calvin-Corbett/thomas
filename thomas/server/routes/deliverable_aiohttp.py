...
# The listing helpers live in deliverable_listing.py (size guard, 2026-09-05); every
# name is re-exported here because chat_delegation_session and the tests import
# them from this module and patch them on it.
from thomas.server.routes.deliverable_listing import (  # noqa: F401
    _BUILD_FILENAMES,
...
class DeliverablePreviewService:
    ...
    async def preview_directory_url(
        ...
        *,
        ...
        # A page whose root-absolute links resolve nowhere in the tree is an
        # application shell: its scripts exist only once a server mounts them.
        # Served from disk it is a blank page with a script error (Thomas's own
        # chat.html, offered as a self-edit demo, was exactly that), so it is
        # refused here, before an origin is started, the way the offline smoke
        # and web.playtest already refuse it.
        unresolved = first_unresolved_root_link(root, target)
        if unresolved:
            raise ValueError(
                f"{entry} links {unresolved}, which only a server provides; "
                "it cannot be previewed from disk, run it on a server instead"
            )
    ...
    def active_grants(self) -> int:
        """How many preview origins are alive right now."""
    ...
    def _target_path(workspace: Path, tail: str, allowed_files: frozenset[str] | None) -> Path:
        ...
        if allowed_files is not None and normalized not in allowed_files and not _is_package_web_asset(rel):
            raise web.HTTPNotFound(text="preview file not found")
