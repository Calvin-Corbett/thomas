...
def register_companion_routes(
    ...
    *,
    ...
    async def api_companion_devices(request: web.Request) -> web.Response:
        require_api_access(request)
        kernel = _kernel_from_config(config)
        _require_control_plane_identity(request, kernel)
    ...
    async def api_companion_device_register(request: web.Request) -> web.Response:
        ...
        runtime_capability_set = (
            _string_list(payload.get("runtime_capability_set")) or capabilities
        )
        row = DeviceRegistry(kernel).upsert(
            ...
            installed_modules=_installed_modules_from_payload(payload),
    ...
    async def api_companion_device_heartbeat(request: web.Request) -> web.Response:
        ...
        distribution_channel = str(
            payload.get("distribution_channel") or current.distribution_channel
        ).strip()
        storefront_region = str(payload.get("storefront_region") or current.storefront_region).strip()
        runtime_capability_set = (
            _string_list(payload.get("runtime_capability_set")) or list(current.runtime_capability_set)
        )
        capabilities = [
            str(x) for x in list(payload.get("capabilities") or current.capabilities) if str(x).strip()
        ]
        policy_profile_id = resolve_policy_profile(
            platform=platform,
            distribution_channel=distribution_channel,
        ...
        row = reg.upsert(
            ...
            runtime_capability_set=runtime_capability_set,
            installed_modules=(
                _installed_modules_from_payload(payload) or dict(current.installed_modules)
            ),
    ...
    async def api_companion_device_pin_release(request: web.Request) -> web.Response:
        ...
        row = DeviceRegistry(kernel).set_pinned_release(device_id, release_id, timestamp=_now_iso())
        if row is None:
            return web.json_response({"ok": False, "error": f"device not found: {device_id}"}, status=404)
        _audit(kernel).append(
            "device.pin_release",
    ...
    async def api_companion_device_unpin_release(request: web.Request) -> web.Response:
        ...
        if not device_id:
            raise web.HTTPBadRequest(text="missing device_id")
        row = DeviceRegistry(kernel).set_pinned_release(device_id, "", timestamp=_now_iso())
        if row is None:
            return web.json_response({"ok": False, "error": f"device not found: {device_id}"}, status=404)
        _audit(kernel).append(
    ...
    async def api_companion_device_updates_check(request: web.Request) -> web.Response:
        ...
        installed_modules = _installed_modules_from_payload(payload) or dict(device.installed_modules)
    ...
    async def api_companion_release_download(request: web.Request) -> web.Response:
        ...
        bundle_dir = _release_bundle_dir(release_payload)
        body = _zip_bundle(bundle_dir)
    ...
    async def api_companion_release_publish(request: web.Request) -> web.Response:
        ...
        result = reg.publish_from_bundle(
            ...
            compliance_status=(
                "pass"
                if bool(compliance_payload.get("ok"))
                else "block"
            ),
            compliance_violations=(
                _int_or_none(
                    ((compliance_payload.get("report") or {}).get("counts") or {}).get(
                        "blocking_violations"
                    )
                )
                or 0
            ...
            compliance_warnings=(
                _int_or_none(
                    ((compliance_payload.get("report") or {}).get("counts") or {}).get("warnings")
                )
                or 0
            ),
            compliance_checked_at=str(
                ((compliance_payload.get("report") or {}).get("checked_at") or "")
            ),
            timestamp=_now_iso(),
        )
        ...
        _audit(kernel).append(
            ...
            details={
                ...
                "compliance_report_id": str(
                    ((compliance_payload.get("report") or {}).get("report_id") or "")
                ),
            },
        )
        status = 200 if bool(result.get("ok")) else 400
    ...
    async def api_companion_release_rollout(request: web.Request) -> web.Response:
        ...
        row = ReleaseRegistry(kernel).set_rollout(
            release_id,
            rollout_pct=(
                _int_or_none(payload.get("rollout_pct"))
                if "rollout_pct" in payload
                else None
            ),
            target_devices=(
                _string_list(payload.get("target_devices"))
                if "target_devices" in payload
                else None
            ),
            exclude_devices=(
                _string_list(payload.get("exclude_devices"))
                if "exclude_devices" in payload
                else None
            ),
            min_app_version=(
                str(payload.get("min_app_version") or "").strip()
                if "min_app_version" in payload
                else None
            ),
            required_capabilities=(
                _string_list(payload.get("required_capabilities"))
    ...
    async def api_companion_audit_events(request: web.Request) -> web.Response:
        ...
        rows = _audit(kernel).list(limit=limit, event_type=event_type)
    ...
    app.router.add_post(
        "/api/companion/v1/devices/{device_id}/pin-release",
        api_companion_device_pin_release,
    )
    app.router.add_post(
        "/api/companion/v1/devices/{device_id}/unpin-release",
        api_companion_device_unpin_release,
    )
    app.router.add_post(
        "/api/companion/v1/devices/{device_id}/updates/check",
        api_companion_device_updates_check,
    )
    ...
    app.router.add_get(
        "/api/companion/v1/releases/{release_id}/manifest",
        api_companion_release_manifest,
    )
    app.router.add_get(
        "/api/companion/v1/releases/{release_id}/download",
        api_companion_release_download,
    )
    app.router.add_post("/api/companion/v1/releases/publish", api_companion_release_publish)
    app.router.add_post(
        "/api/companion/v1/releases/{release_id}/rollout",
        api_companion_release_rollout,
    )
    app.router.add_post(
        "/api/companion/v1/releases/{release_id}/promote",
        api_companion_release_promote,
    )
    app.router.add_post(
        "/api/companion/v1/releases/{release_id}/rollback",
        api_companion_release_rollback,
    )
