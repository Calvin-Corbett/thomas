...
from thomas.marketplace.orchestrator.brain_helpers import specialist_timeout_seconds
...
def _load_dispatch():
    """Load the pure chat dispatch module without entering the agent lane."""
    try:
        return import_module("thomas.chat.dispatch")
...
class OrchestratorBrainV3:
    ...
    async def _casual_reply(
        ...
        try:
            ...
            log.debug("Casual memory capture skipped: %s", exc)
    ...
    async def _dispatch_to_bot(
        ...
        contract = DelegationContract(
            ...
            task_description=prompt[:500],
            allowed_tools=specialist.capabilities,
            timeout_seconds=specialist_timeout_seconds(mode, ""),
            max_iterations=10,
        ...
        try:
            ...
            log.debug("Delegation memory capture skipped: %s", exc)
