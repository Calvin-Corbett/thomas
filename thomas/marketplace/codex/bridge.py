...
class CodexBridge:
    ...
    async def login_chatgpt(self) -> CodexAccount:
        ...
        def on_login_completed(params: dict[str, Any]) -> None:
            nonlocal login_id
            if params.get("loginId") == login_id and not login_done.done():
        ...
        try:
            ...
            raise CodexBridgeError("Login timed out (5 minutes). Please try again.") from None
        finally:
        ...
        *,
        ...
        def on_turn_completed(params: dict[str, Any]) -> None:
            if not turn_done.done():
                ...
                usage = _extract_usage_payload(turn) or _extract_usage_payload(params)
                if usage:
                    events_queue.put_nowait({"type": "usage", "usage": usage})
        ...
        def on_thread_token_usage_updated(params: dict[str, Any]) -> None:
            usage = _extract_usage_payload(params)
            if usage:
                events_queue.put_nowait({"type": "usage", "usage": usage})
        ...
        self._on("thread/tokenUsage/updated", on_thread_token_usage_updated)
        ...
        try:
            ...
            self._off("thread/tokenUsage/updated", on_thread_token_usage_updated)
    ...
    async def _request(self, method: str, params: dict[str, Any], timeout: float = _REQUEST_TIMEOUT) -> dict[str, Any]:
        ...
        try:
            ...
            raise CodexBridgeError(f"Request {method} timed out after {timeout}s") from None
