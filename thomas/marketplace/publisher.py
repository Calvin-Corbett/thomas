...
def _extension_catalog_runtime() -> Any:
    return import_module("thomas.plugins.extension_catalog_runtime")


def _desktop_plugins_manifest() -> Any:
    return import_module("thomas.server.desktop_plugins_manifest")


def _desktop_plugins_runtime() -> Any:
    return import_module("thomas.server.desktop_plugins_runtime")
...
def _catalog_by_id(extensions_root: Path) -> dict[str, dict[str, Any]]:
    loaded = _extension_catalog_runtime().load_extension_catalog(root=extensions_root)
...
def _validation_by_id(extensions_root: Path) -> dict[str, dict[str, Any]]:
    payload = _extension_catalog_runtime().validate_extension_catalog(root=extensions_root)
    out: dict[str, dict[str, Any]] = {}
    ...
    *,
    ...
    for plugin_id in selected_ids:
        ...
        if not bool(validation_row.get("valid")):
            ...
            continue
        manifest = _desktop_plugins_manifest().maybe_load_desktop_plugin_manifest(plugin_dir, require_signature=False)
        if manifest is None:
            report["skipped"].append(
                {
                    **entry,
        ...
        bundle_bytes = _desktop_plugins_manifest().build_plugin_bundle_bytes(plugin_dir, plugin_id)
        hosted_manifest = _desktop_plugins_runtime().create_official_hosted_manifest(
