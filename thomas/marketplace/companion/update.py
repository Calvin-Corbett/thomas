...
class VerifyReport:
    ...
    module: ModuleContract | None
    manifest: UpdateBundleManifest | None
...
class BundleVerifier:
    ...
    def verify_bundle(self, bundle_dir: Path) -> VerifyReport:
        ...
        if not _semver_gte(KERNEL_VERSION, manifest.min_kernel_version):
            errors.append(f"kernel {KERNEL_VERSION} is below required min_kernel_version {manifest.min_kernel_version}")
        ...
        for file_entry in manifest.files:
            ...
            if not rel.startswith(module_prefix):
                errors.append(f"bundle file path must stay within module namespace '{module_prefix}': {rel}")
