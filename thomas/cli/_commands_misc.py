...
def security_audit_cmd(repo_root: str, as_json: bool, strict: bool) -> None:
    ...
    if as_json:
        muted_logger = logging.getLogger("thomas.server.app_core")
        previous_disabled = muted_logger.disabled
        previous_level = muted_logger.level
        muted_logger.disabled = True
        muted_logger.setLevel(logging.CRITICAL + 1)
        try:
            with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
                report = run_security_audit(resolved_root, include_incident_drill=False)
        finally:
            muted_logger.disabled = previous_disabled
            muted_logger.setLevel(previous_level)
    else:
        report = run_security_audit(resolved_root, include_incident_drill=False)
    payload = {
