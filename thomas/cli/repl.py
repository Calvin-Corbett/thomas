...
from thomas.cli.repl_activity import ThomasREPLActivityMixin
...
def _should_open_slash_popup(text: str, cursor_position: int) -> bool:
    ...
    return bool(stripped.startswith("/") and " " not in stripped and "\n" not in stripped and stripped != "//")
...
class ThomasREPL(ThomasREPLActivityMixin, ThomasREPLRuntimeMixin, ThomasREPLAgentMixin):
    ...
    # Activity tracking, logging, tool-event processing, and panel rendering
    # methods live in ThomasREPLActivityMixin (repl_activity.py).
...
def _get_version() -> str:
    try:
        ...
        return "?"
from thomas.cli.repl_command import repl as repl  # noqa: E402,F401
