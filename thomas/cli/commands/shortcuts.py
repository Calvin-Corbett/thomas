...
def _install_windows(mode: str) -> str | None:
    ...
    if mode == "serve":
        cmd = f"{python} -m thomas serve"
        filename = "Thomas AI (Web).bat"
    else:
        cmd = f"{python} -m thomas repl"
        filename = "Thomas AI (Terminal).bat"

    bat_content = f"@echo off\ntitle Thomas AI\n{cmd}\npause\n"
    path = desktop / filename
    path.write_text(bat_content)

    logger.info("Created Windows shortcut: %s", path)
    return str(path)
...
def remove_shortcuts() -> list:
    ...
    if platform == "linux":
        ...
        desktop = Path.home() / "Desktop"
        for name in ["Thomas AI (Web).bat", "Thomas AI (Terminal).bat"]:
            p = desktop / name
            if p.exists():
                p.unlink()
                removed.append(str(p))
