...
PROVIDERS: list[dict[str, str]] = [
    ...
    },
    {
        ...
        "desc": "Any provider with OpenAI-compatible API",
        "base_url": "",
        "key_required": "true",
    },
]
...
def _banner() -> None:
    ...
    click.echo(click.style("  ║      Thomas Setup Wizard             ║", fg="cyan"))
    click.echo(click.style("  ╚══════════════════════════════════════╝", fg="cyan"))
    click.echo("")
...
def _step(n: int, title: str) -> None:
    """Print a step header."""
    click.echo(click.style(f"\n  Step {n}: {title}", fg="green", bold=True))
...
def run_wizard() -> str | None:
    ...
    if ollama_running:
        click.echo(click.style("  ✓ Ollama detected running locally!", fg="green"))

    for i, p in enumerate(PROVIDERS, 1):
        marker = " ←" if p["id"] == "ollama" and ollama_running else ""
        click.echo(f"  {i}. {p['name']:30s} {p['desc']}{marker}")
    ...
    if provider["key_required"] == "true":
        ...
        if not api_key.strip():
            click.echo(click.style("  ⚠ No API key provided — connection may fail", fg="yellow"))
    else:
    ...
    if ok:
        click.echo(click.style(f'  ✓ Connected! Response: "{reply[:60]}"', fg="green"))
    else:
        click.echo(click.style(f"  ✗ Connection failed: {reply[:100]}", fg="red"))
    ...
    click.echo(click.style(f"  ✓ Config saved to {config_path.resolve()}", fg="green"))

    # ── Step 8: Done ─────────────────────────────────────────
    click.echo("")
    click.echo(click.style("  ╔══════════════════════════════════════╗", fg="cyan"))
    click.echo(click.style("  ║      Setup Complete!                 ║", fg="cyan"))
    click.echo(click.style("  ╚══════════════════════════════════════╝", fg="cyan"))
    click.echo("")
