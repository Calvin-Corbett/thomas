...
def _memory_emit_search_output(payload: dict[str, Any], *, as_json: bool, query: str) -> None:
    ...
    for row in payload.get("results", []):
        if isinstance(row, dict):
            click.echo(f"  - {row.get('file', row.get('id', '?'))}: {str(row.get('preview', row.get('text', '')))[:120]}")
...
@_memory_json_option
@_memory_run_mode_option
def memory_list(as_json: bool, run_mode: bool) -> None:
    """List memory snapshots."""
    if not run_mode:
        _memory_emit_describe("list", as_json=as_json)
        return
    _memory_emit_result(_memory_unimplemented_payload("list"), as_json=as_json)
