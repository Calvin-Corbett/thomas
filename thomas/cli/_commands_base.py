...
from thomas.cli.repl import repl


# Delay import of _resolve_model_profile_name to avoid circular imports
# It will be imported when needed
...
def cli(
    ...
    if ctx.invoked_subcommand not in ("setup", "quickstart") and not getattr(cli, "_first_run_checked", False):
    ...
    # Background update check (silent, once per day; never auto-applies)
    if ctx.invoked_subcommand not in ("update", "version") and not getattr(cli, "_update_checked", False):
