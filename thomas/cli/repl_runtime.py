...
class ThomasREPLRuntimeMixin:
    def _named_sessions_dir(self) -> Path:
        path = Path(self.config.memory.root_path) / "repl_sessions"
        path.mkdir(parents=True, exist_ok=True)
        return path

    @staticmethod
    def _normalize_session_name(raw: str) -> str:
    ...
    def _session_file_path(self, name: str) -> Path | None:
        normalized = self._normalize_session_name(name)
        if not normalized:
            return None
        return self._named_sessions_dir() / f"{normalized}.json"

    def _safe_conversation_rows(self) -> list[dict[str, str]]:
    ...
    def _list_named_sessions(self) -> list[str]:
        try:
            files = sorted(
                self._named_sessions_dir().glob("*.json"),
    ...
    def _save_named_session(self, name: str) -> Path | None:
        target = self._session_file_path(name)
        if target is None:
            return None
        payload = {
            "name": target.stem,
            "saved_at": int(time.time()),
            "session_id": str(getattr(self, "_repl_session_id", "") or ""),
            "memory_thread_id": str(getattr(self, "_chat_thread_id", "") or ""),
            "model": str(getattr(self, "_current_model", "") or ""),
            "autonomy_level": int(getattr(self, "_autonomy_level", 3) or 3),
            "tools_policy": str(getattr(self, "_tools_policy", "auto") or "auto"),
            "show_system_logs": bool(getattr(self, "_show_system_logs", False)),
            "activity_verbosity": str(getattr(self, "_activity_verbosity", "normal") or "normal"),
            "conversation": self._safe_conversation_rows(),
        }
        try:
    ...
    def _load_named_session(self, name: str) -> tuple[bool, str]:
        target = self._session_file_path(name)
        if target is None:
            return False, "Invalid session name. Use letters, numbers, '.', '_' or '-'."
        if not target.exists():
            return False, f"Session '{target.stem}' was not found."
        try:
            payload = json.loads(target.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            return False, f"Session '{target.stem}' could not be read."
        if not isinstance(payload, dict):
            return False, f"Session '{target.stem}' is invalid."

        raw_conversation = payload.get("conversation")
        if not isinstance(raw_conversation, list):
            return False, f"Session '{target.stem}' has no valid conversation payload."
        restored: list[dict[str, str]] = []
        for row in raw_conversation:
        ...
        saved_model = str(payload.get("model") or "").strip()
        if saved_model and saved_model in self.config.models:
            self._current_model = saved_model
            reset_context_window = getattr(self, "_reset_context_window_cache", None)
            if callable(reset_context_window):
                reset_context_window()

        autonomy_raw = payload.get("autonomy_level")
        try:
            autonomy_level = int(autonomy_raw)
            if 1 <= autonomy_level <= 4:
                self._autonomy_level = autonomy_level
        except (TypeError, ValueError):
            pass

        tools_policy = str(payload.get("tools_policy") or "").strip().lower()
        if tools_policy in {"auto", "always", "never"}:
            self._tools_policy = tools_policy

        self._show_system_logs = bool(payload.get("show_system_logs", self._show_system_logs))
        activity_verbosity = str(payload.get("activity_verbosity") or "").strip().lower()
        if activity_verbosity in {"minimal", "normal", "debug"}:
            self._activity_verbosity = activity_verbosity

        self._conversation = restored
        self._session_turns = sum(1 for row in restored if row.get("role") == "user")

        rotate_chat_thread = getattr(self, "_rotate_chat_thread", None)
        if callable(rotate_chat_thread):
            rotate_chat_thread()

        self._persist_conversation_state()
        self._persist_repl_runtime_settings()
        return True, f"Loaded session '{target.stem}' with {len(restored)} messages."

    def _persist_repl_runtime_settings(self) -> None:
        persist_fn = getattr(self, "_persist_runtime_settings", None)
        if not callable(persist_fn):
            return
        try:
            persist_fn()
        except Exception as exc:
            log.warning("Failed to persist REPL runtime settings: %s", f"{type(exc).__name__}: {exc}")
    ...
    async def _persist_repl_model_selection(self, *, profile: str, model_id: str | None) -> None:
        ...
        try:
            ...
            log.warning(
                "Failed to persist REPL model selection profile=%s model_id=%s: %s",
                resolved_profile,
    ...
    async def _handle_slash(self, cmd: str) -> tuple[bool, bool]:
        ...
        if command == "/exit":
            ...
            if callable(rotate_chat_thread):
                rotate_chat_thread()
            self._persist_conversation_state()
            self._console.print("[dim]Conversation cleared. Started a new memory thread.[/dim]")
            ...
            profile_lookup = {str(name).strip().lower(): str(name).strip() for name in self.config.models}
            resolved_profile = profile_lookup.get(str(arg).strip().lower())
            if resolved_profile:
                self._current_model = resolved_profile
                ...
                await self._persist_repl_model_selection(profile=resolved_profile, model_id=None)
            ...
            if str(arg).strip().lower() in {"auto", "default", "l0"}:
                ...
                level = clamp_autonomy_level(1, default=self._autonomy_level)
            ...
            tool_count = 0
            repl_session_id = str(getattr(self, "_repl_session_id", "") or "").strip()
            chat_thread_id = str(getattr(self, "_chat_thread_id", "") or "").strip()
            ...
            if repl_session_id:
                self._console.print(f"[dim]Session: {repl_session_id}[/dim]")
            if chat_thread_id:
                self._console.print(f"[dim]Memory thread: {chat_thread_id}[/dim]")
            self._console.print(
            ...
            if not sub or sub in {"info", "status"}:
                user_turns = sum(1 for m in self._conversation if m.get("role") == "user")
                asst_turns = sum(1 for m in self._conversation if m.get("role") == "assistant")
                self._console.print(f"[dim]Session: {repl_session_id or '(none)'}[/dim]")
                self._console.print(f"[dim]Memory thread: {chat_thread_id or '(none)'}[/dim]")
                self._console.print(
                    f"[dim]Conversation: {user_turns} user, {asst_turns} assistant, "
                    f"{len(self._conversation)} total messages[/dim]"
                )
            ...
            if sub in {"new", "reset"}:
                ...
                self._persist_conversation_state()
                self._console.print(
                    f"[dim]Started new session: {str(getattr(self, '_repl_session_id', '') or '(none)')}[/dim]"
                )
                return False, True

            if sub in {"list", "ls"}:
                names = self._list_named_sessions()
                if not names:
                    self._console.print("[dim]No named sessions saved yet.[/dim]")
                else:
                    self._console.print(f"[dim]Saved sessions ({len(names)}):[/dim]")
                    for name in names[:30]:
                        self._console.print(f"  [cyan]{name}[/cyan]")
                return False, True

            if sub.startswith("save"):
                name = raw_sub[4:].strip()
                if not name:
                    self._console.print("[yellow]Usage: /session save <name>[/yellow]")
                    return False, True
                saved = self._save_named_session(name)
                if saved is None:
                    self._console.print("[yellow]Failed to save session. Check the name/path.[/yellow]")
                    return False, True
                self._console.print(f"[dim]Saved session '{saved.stem}' to {saved}[/dim]")
                return False, True

            if sub.startswith("load"):
                name = raw_sub[4:].strip()
                if not name:
                    self._console.print("[yellow]Usage: /session load <name>[/yellow]")
                    return False, True
                ok, message = self._load_named_session(name)
                if not ok:
            ...
            self._console.print("[yellow]Usage: /session [info|new|list|save <name>|load <name>][/yellow]")
            return False, True
            ...
            if sub in {"clear", "new", "reset", "rotate"}:
                rotate_chat_thread = getattr(self, "_rotate_chat_thread", None)
                if callable(rotate_chat_thread):
                    rotate_chat_thread()
                    new_thread_id = str(getattr(self, "_chat_thread_id", "") or "").strip()
                    if new_thread_id:
                        self._console.print(f"[dim]Started new memory thread: {new_thread_id}[/dim]")
                    else:
                        self._console.print("[dim]Started new memory thread.[/dim]")
                else:
                    self._console.print("[yellow]Memory thread rotation is unavailable in this runtime.[/yellow]")
                return False, True

            if sub.startswith("query"):
                query_text = raw_sub[5:].strip() if len(raw_sub) >= 5 else ""
                if query_text.startswith(":"):
                    query_text = query_text[1:].strip()
                if not query_text:
                    self._console.print("[yellow]Usage: /memory query <text>[/yellow]")
                    return False, True
                if self._memory and self._memory.started:
                    try:
                        result = self._memory.retrieve(query=query_text, thread=chat_thread_id or None)
                        result_text = str(getattr(result, "text", "") or "").strip()
                        if result_text:
                            self._console.print(Panel(result_text, title="Memory Query", border_style="dim"))
                        else:
                            self._console.print("[dim]No memory context matched that query.[/dim]")
                    except Exception as e:
                        self._console.print(f"[yellow]Memory query failed: {type(e).__name__}[/yellow]")
                else:
                    self._console.print("[yellow]Memory engine not active[/yellow]")
                return False, True

            if sub in {"thread", "scope"}:
                if repl_session_id:
                    self._console.print(f"[dim]Session: {repl_session_id}[/dim]")
                if chat_thread_id:
                    self._console.print(f"[dim]Memory thread: {chat_thread_id}[/dim]")
                if self._memory and self._memory.started and chat_thread_id:
                    policy_getter = getattr(self._memory, "thread_memory_policy", None)
                    if callable(policy_getter):
                        try:
                            policy = dict(policy_getter(chat_thread_id))
                            include_global = bool(policy.get("include_global", True))
                            include_profile = bool(policy.get("include_profile", True))
                            max_results = int(policy.get("max_results", 12) or 12)
                            pins_only = bool(policy.get("pins_only", False))
                            self._console.print(
                                "[dim]Policy: "
                                f"include_global={include_global}, include_profile={include_profile}, "
                                f"max_results={max_results}, pins_only={pins_only}[/dim]"
                            )
                        except Exception:
                            self._console.print("[dim]Policy: unavailable[/dim]")
                return False, True

            if sub and sub not in {"stats", "status"}:
                self._console.print("[yellow]Usage: /memory [stats|thread|new|query <text>][/yellow]")
                return False, True
            ...
            if chat_thread_id:
                self._console.print(f"[dim]Active memory thread: {chat_thread_id}[/dim]")
            if self._llm:
                usage = self._llm.session_usage
                self._console.print(
            ...
            if arg:
                ...
                if not tasks:
                    ...
                    for t in tasks[:20]:
                        elapsed_seconds = (
                            (t.finished_at - t.started_at) if t.finished_at else (_time.time() - t.started_at)
                        )
                        elapsed = f" ({max(0.0, elapsed_seconds):.1f}s)"
