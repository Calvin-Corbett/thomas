...
def is_globally_approved(prompt_text: str) -> bool:
    ...
    # R5 (praxis-unbypassable-2026-05-29): a negated prompt like "I do NOT
    # approve risky skills" contains the substring "approve risky skills" — a
    # naive match treated that as global approval. Reject when the prompt
    # negates risky-skill approval.
    if prompt_negates_risky_approval(prompt_text):
        return False
    return any(
        phrase in src
        for phrase in (
...
# R5 (praxis-unbypassable-2026-05-29): a negation near the approval verb means
# the user did NOT approve (e.g. "I do not approve risky skills"). Treat such
# phrasing as non-approval rather than letting the bare verb match.
_APPROVAL_NEGATION_RE = re.compile(
    r"\b(?:no|not|never|don'?t|do not|cannot|can'?t|without|deny|denied|refuse[ds]?|disapprove[ds]?)\b"
)


def has_skill_approval_phrase(prompt_text: str, skill_name: str) -> bool:
    # Normalize hyphens/underscores to spaces in BOTH the prompt and the skill
    # name so "cloudflare-deploy" and "cloudflare deploy" match the same way.
    src = re.sub(r"[-_]", " ", str(prompt_text or "").lower())
    if not src:
        return False
    normalized = _normalize_skill_name(skill_name).replace("_", " ").replace("-", " ")
    escaped = re.escape(normalized)
    approve_kw = r"(?:approve[sd]?|allow(?:ed|s)?|authoriz(?:e|ed|es))"
    pattern = rf"\b{approve_kw}\b[^\n]{{0,40}}\b{escaped}\b"
    for match in re.finditer(pattern, src):
        # Reject when a negation immediately precedes the approval verb, e.g.
        # "do not approve <skill>" / "never allow <skill>".
        window = src[max(0, match.start() - 24) : match.start()]
        if _APPROVAL_NEGATION_RE.search(window):
            continue
        return True
    return False


# R5 (praxis-unbypassable-2026-05-29): detect a prompt that explicitly DECLINES
# risky-skill / deployment approval, e.g. "I do not approve risky skills or
# deployment actions". When present, a bare explicit mention of a high-risk
# skill must NOT be treated as approval — the user said no in the same breath.
_RISKY_APPROVAL_NEGATION_RE = re.compile(
    r"\b(?:no|not|never|don'?t|do not|cannot|can'?t|without|deny|denied|refuse[ds]?|disapprove[ds]?)\b"
    r"[^\n]{0,30}\b(?:approv\w*|risky|risk|deploy\w*|dangerous|high[ -]?risk)\b"
)


def prompt_negates_risky_approval(prompt_text: str) -> bool:
    """True if the prompt contains an explicit refusal of risky-skill approval."""
    src = re.sub(r"[-_]", " ", str(prompt_text or "").lower())
    if not src:
        return False
    return bool(_RISKY_APPROVAL_NEGATION_RE.search(src))
