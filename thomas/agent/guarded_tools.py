...
def _needs_native_auth_in_allow_mode(tool_name: str, decision_reason: str) -> bool:
    tool = str(tool_name or "").strip().lower()
    reason = str(decision_reason or "").strip().lower()
    destructive_tool = any(token in tool for token in ("delete", "remove", "destroy"))
    destructive_reason = "destructive" in reason
    return destructive_tool or destructive_reason
    ...
    async def run(
        ...
        *,
        ...
        if decision.type == PolicyDecisionType.REQUIRE_APPROVAL:
            ...
            requires_native_auth = bool(decision.meta.get("native_auth"))
            auto_allow_needs_native_auth = effective_no_human_mode == "allow" and _needs_native_auth_in_allow_mode(
                tool_name,
                decision.reason,
            )

            if requires_native_auth or auto_allow_needs_native_auth:
                await emit_event(
                ...
                approval_resolution = {
                    "auto": effective_no_human_mode == "allow",
                    "no_human_mode": effective_no_human_mode,
                    "decision": "NATIVE_AUTH_APPROVED" if approved else "NATIVE_AUTH_DENIED",
                    "source": "native_auth",
                }
            elif effective_no_human_mode == "allow":
                approved = True
                approval_resolution = {
                    "auto": True,
                    "no_human_mode": effective_no_human_mode,
