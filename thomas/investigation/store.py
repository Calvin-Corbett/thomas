...
class Document:
    ...
    analyzed_at_ms: Optional[int] = None
...
class Claim:
    ...
    date_referenced: Optional[str] = None
...
class InvestigationStore:
    ...
    def create_case(
        self, name: str, source_path: str, description: str = ""
    ) -> Case:
    ...
    def list_cases(self) -> List[Case]:
        with self._lock:
            rows = self._conn.execute(
                "SELECT * FROM cases ORDER BY created_at_ms DESC"
            ).fetchall()
        return [Case(**dict(r)) for r in rows]

    def get_latest_case(self) -> Optional[Case]:
        with self._lock:
            row = self._conn.execute(
                "SELECT * FROM cases ORDER BY created_at_ms DESC LIMIT 1"
            ).fetchone()
        if row is None:
            return None
        return Case(**dict(row))

    def find_case_by_source(self, source_path: str) -> Optional[Case]:
        with self._lock:
            row = self._conn.execute(
    ...
    def add_document(self, doc: Document) -> int:
        ...
        with self._lock:
            cur = self._conn.execute(
                ...
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    doc.case_id, doc.file_path, doc.file_name, doc.file_type,
                    doc.file_size_bytes, doc.file_hash, doc.extracted_text,
                    doc.extraction_method, doc.page_count, now, "pending",
                ),
            )
    ...
    def get_pending_documents(self, case_id: str) -> List[Document]:
        with self._lock:
            rows = self._conn.execute(
                "SELECT * FROM documents WHERE case_id = ? AND analysis_status = 'pending' "
                "ORDER BY id",
                (case_id,),
            ).fetchall()
    ...
    def update_document_text(
        self, doc_id: int, text: str, extraction_method: str = "transcription"
    ) -> None:
    ...
    def get_image_documents(
        self, case_id: str, *, only_untranscribed: bool = False
    ) -> List[Document]:
    ...
    def add_claim(self, claim: Claim) -> int:
        ...
        with self._lock:
            cur = self._conn.execute(
                ...
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    claim.case_id, claim.doc_id, claim.claim_text, claim.category,
                    claim.subcategory, claim.date_referenced, claim.date_precision,
                    people_json, claim.sentiment, claim.severity, claim.confidence,
                    claim.quote_excerpt, now,
                ),
            )
            claim_id = cur.lastrowid
            # Update FTS
            self._conn.execute(
                "INSERT INTO claims_fts(rowid, claim_text, category, quote_excerpt) "
                "VALUES (?, ?, ?, ?)",
    ...
    def get_claims(
        ...
        *,
        ...
        with self._lock:
            if category:
                rows = self._conn.execute(
                    "SELECT * FROM claims WHERE case_id = ? AND category = ? "
                    "ORDER BY date_referenced, id LIMIT ?",
    ...
    def search_claims(
        self, query: str, case_id: str, *, category: Optional[str] = None, limit: int = 20
    ) -> List[Dict[str, Any]]:
        with self._lock:
            if category:
                rows = self._conn.execute(
    ...
    def search_documents(
        self, query: str, case_id: str, *, limit: int = 10
    ) -> List[Dict[str, Any]]:
        with self._lock:
            rows = self._conn.execute(
    ...
    def add_pattern(self, pattern: Pattern) -> int:
        ...
        with self._lock:
            cur = self._conn.execute(
                ...
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    pattern.case_id, pattern.pattern_text, pattern.category,
                    pattern.evidence_count, json.dumps(pattern.claim_ids),
                    pattern.first_seen, pattern.last_seen, pattern.strength,
                    now, now,
                ),
            )
            self._conn.commit()
    ...
    def get_patterns(
        self, case_id: str, *, min_strength: float = 0.0, category: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        with self._lock:
    ...
    def add_timeline_event(self, event: TimelineEvent) -> int:
        ...
        with self._lock:
            cur = self._conn.execute(
                ...
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    event.case_id, event.event_date, event.event_summary,
                    event.category, json.dumps(event.doc_ids),
                    json.dumps(event.claim_ids), event.severity, now,
                ),
            )
            self._conn.commit()
        return cur.lastrowid  # type: ignore[return-value]
    ...
    def clear_timeline(self, case_id: str) -> None:
        with self._lock:
            self._conn.execute(
                "DELETE FROM timeline_events WHERE case_id = ?", (case_id,)
            )
            self._conn.commit()
    ...
    def get_case_summary(self, case_id: str) -> Dict[str, Any]:
        with self._lock:
            ...
            claim_count = self._conn.execute(
                "SELECT COUNT(*) FROM claims WHERE case_id = ?", (case_id,)
            ).fetchone()[0]
