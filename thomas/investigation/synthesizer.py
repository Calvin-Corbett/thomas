...
from thomas.investigation.store import InvestigationStore, Pattern, TimelineEvent
...
class InvestigationSynthesizer:
    ...
    async def _synthesize_patterns(
        self, case_id: str, llm_call: Callable
    ) -> int:
        ...
        for rp in raw_patterns:
            ...
            avg_severity = (
                sum(c.get("severity", 0) for c in evidence_claims) / len(evidence_claims)
                if evidence_claims else 0
            )
            avg_confidence = (
                sum(c.get("confidence", 0.7) for c in evidence_claims) / len(evidence_claims)
                if evidence_claims else 0.7
            )

            # Date span
            dates = [
                c.get("date_referenced")
                for c in evidence_claims
                if c.get("date_referenced")
            ]
    ...
    async def _build_timeline(
        ...
        dated_claims = [
            c for c in all_claims if c.get("date_referenced")
        ]
    ...
    @staticmethod
    def _format_claims_by_category(
        by_category: Dict[str, List[Dict[str, Any]]]
    ) -> str:
        parts: List[str] = []
        for cat, claims in sorted(by_category.items()):
            ...
            for c in claims:
                ...
                parts.append(
                    f"  ID={c['id']}{date_str} severity={c.get('severity', 0)}: "
    ...
    @staticmethod
    def _date_span_days(dates: List[str]) -> Optional[int]:
