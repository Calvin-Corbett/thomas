...
from thomas.investigation.store import Document, InvestigationStore
...
ALL_SUPPORTED = (
    _TEXT_EXTS | _PDF_EXTS | _HTML_EXTS | _JSON_EXTS | _CSV_EXTS
    | _IMAGE_EXTS | _DOCX_EXTS | _EMAIL_EXTS
)
...
class DocumentIngester:
    ...
    def _extract_text(self, fp: Path) -> Tuple[Optional[str], str, Optional[int]]:
    ...
    @staticmethod
    def _extract_pdf(fp: Path) -> Tuple[str, str, Optional[int]]:
        try:
            import pdfplumber
        except ImportError:
            # Try basic fallback
    ...
    @staticmethod
    def _extract_docx(fp: Path) -> Tuple[str, str, Optional[int]]:
        try:
