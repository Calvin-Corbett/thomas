...
def _moltbook_module() -> Any:
    return import_module("thomas.integrations.moltbook")


def _get_client() -> Any:
    """Lazily construct and cache a MoltbookClient."""
    moltbook = _moltbook_module()
    ...
    client = moltbook.MoltbookClient(moltbook.MoltbookConfig(
        api_key=api_key,
...
def handle_moltbook_register(
    ...
    **_: Any,
    ...
    client = _get_client()
    auth = _moltbook_module().MoltbookAuthFlow(client)
