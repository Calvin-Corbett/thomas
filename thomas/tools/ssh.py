...
class SSHConnectionPool:
    ...
    async def _connect_asyncssh(
        ...
        try:
            ...
            raise SSHConnectionError(f"Connection refused: {e}") from e
        except asyncssh.PermissionDenied as e:
            raise SSHAuthenticationError(f"Authentication failed: {e}") from e
        except asyncssh.HostKeyNotVerifiable as e:
            raise SSHConnectionError(f"Host key verification failed: {e}") from e
    ...
    async def _connect_paramiko(
        ...
        def _connect():
            ...
            # Reject unknown hosts instead of silently accepting them
            # (AutoAddPolicy enables man-in-the-middle attacks).
            known_hosts = os.path.expanduser("~/.ssh/known_hosts")
    ...
    async def disconnect(self, host: str, port: int = 22, username: str = "root") -> None:
        ...
        if conn and self._backend in {"asyncssh", "paramiko"}:
            conn.close()
    ...
    async def _execute_asyncssh(
        ...
        try:
            ...
            raise SSHTimeoutError(f"Command timed out after {timeout}s") from None
        except Exception as e:
    ...
    async def _upload_asyncssh(
        ...
        try:
            ...
            raise SSHFileError(f"File upload failed: {e}") from e
    ...
    async def _download_asyncssh(
        ...
        try:
            ...
            raise SSHFileError(f"File download failed: {e}") from e
    ...
    async def _list_asyncssh(self, conn: Any, remote_path: str) -> list:
        ...
        try:
            ...
            raise SSHFileError(f"Directory listing failed: {e}") from e
    ...
    async def _read_asyncssh(self, conn: Any, remote_path: str) -> str:
        ...
        try:
            ...
            raise SSHFileError(f"File read failed: {e}") from e
    ...
    async def _write_asyncssh(
        ...
        try:
            ...
            raise SSHFileError(f"File write failed: {e}") from e
