...
async def _run_thomas_api_task(
    *,
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        ...
        async with client.stream("POST", f"{base}/api/v2/chat", headers=headers, json=payload) as resp:
            ...
            async for raw_line in resp.aiter_lines():
                ...
                elif et == "task_dispatched":
                    dispatched_task_id = str(evt.get("task_id") or "").strip()
                    dispatched_execution_id = str(evt.get("execution_id") or "").strip()
                    _watch_line(
                        watch,
                        f"\n{watch_prefix} task_dispatched task_id={dispatched_task_id} execution_id={dispatched_execution_id}",
                    )
                elif et == "error":
                    error = str(evt.get("error") or "unknown error")
                    _watch_line(watch, f"\n{watch_prefix} error: {error}")
                elif et == "done":
                    final_text = str(evt.get("text") or evt.get("response") or "")
        ...
        if dispatched_task_id or dispatched_execution_id:
            terminal_states = {"completed", "failed", "abandoned", "cancelled", "canceled"}
            poll_started = time.monotonic()
            last_state = ""
            last_progress = ""
            matched_row: dict[str, Any] = {}
            while time.monotonic() - poll_started < 300.0:
                await asyncio.sleep(0.75)
                delegations_resp = await client.get(f"{base}/api/v2/chat/session/{sid}/delegations", headers=headers)
                delegations_resp.raise_for_status()
                payload = delegations_resp.json() or {}
                rows = list(payload.get("delegations") or [])
                match = None
                for row in rows:
                    row_exec = str((row or {}).get("execution_id") or "").strip()
                    row_task = str((row or {}).get("task_id") or "").strip()
                    if dispatched_execution_id and row_exec == dispatched_execution_id:
                        match = dict(row or {})
                        break
                    if dispatched_task_id and row_task == dispatched_task_id:
                        match = dict(row or {})
                        break
                if match is None and rows:
                    match = dict(rows[0] or {})
                if match is None:
                    continue
                matched_row = match
                state = str(match.get("state") or "").strip().lower()
                progress = str(match.get("last_progress") or match.get("summary") or "").strip()
                if state != last_state or progress != last_progress:
                    _watch_line(
                        watch,
                        f"\n{watch_prefix} delegation_state={state or 'unknown'} progress={progress[:120]}",
                    )
                    last_state = state
                ...
                if state in terminal_states:
                    background_meta = {
                        ...
                        "last_progress": progress,
                        "poll_elapsed_seconds": round(max(0.0, time.monotonic() - poll_started), 3),
                    }
                    if state != "completed":
                        error = (
                            error
                            or f"background_task_{state}: {progress or background_meta['summary'] or 'no details'}"
                        )
                    break
            else:
                background_meta = {
                    "state": "timeout",
                    "execution_id": str(matched_row.get("execution_id") or ""),
                    "task_id": str(matched_row.get("task_id") or ""),
                    "summary": str(matched_row.get("summary") or ""),
                    "last_progress": str(matched_row.get("last_progress") or ""),
                    "poll_elapsed_seconds": round(max(0.0, time.monotonic() - poll_started), 3),
                }
                error = error or "background_task_timeout"
            token_report["background_task"] = background_meta
