...
def classify_failure_event(*, channel: Optional[str], event: Mapping[str, Any]) -> FailureClassification:
    ...
    if http_status == 401 or _contains_any(message_l, ("unauthorized", "authentication", "invalid token", "wrong token")):
    ...
    if http_status == 404 or _contains_any(message_l, ("chat not found", "not found", "peer id invalid", "user not found")):
...
def _extract_retry_after_from_message(message_l: str) -> Optional[int]:
...
def _taxonomy_for_channel(channel: Optional[str]) -> Sequence[FailureCategoryInfo]:
...
def _validate_probe_inputs(*, channel: Optional[str], config: Optional[Mapping[str, Any]]) -> None:
    if channel is None:
