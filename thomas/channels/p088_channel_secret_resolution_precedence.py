...
def _parse_spec(spec: str) -> tuple[SecretSpecType, dict[str, Any]]:
    ...
    if s.startswith("env:"):
        ...
        if not name:
            raise InvalidSecretRequestError(
                "env: secret spec must include an environment variable name."
            )
    ...
    if s.startswith("${") and s.endswith("}") and len(s) > 3:
        ...
        if not name:
            raise InvalidSecretRequestError(
                "${...} secret spec must include an environment variable name."
            )
        return (SecretSpecType.ENV, {"env_var": name})
...
def resolve_channel_secret(
    ...
    *,
    ...
    env_names = (
        tuple(request.env_var_names)
        if request.env_var_names
        else _default_env_var_names(provider, secret_key)
    )
