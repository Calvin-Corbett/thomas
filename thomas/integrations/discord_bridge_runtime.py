...
class DiscordBridgeRuntime(
    ...
    def __init__(self, config: AppConfig, *, bridge_root: Path | None = None):
        ...
        self._secret_store = SecretStore(_default_shared_secret_root())
    ...
    def _migrate_legacy_secret_store(self) -> None:
        with contextlib.suppress(Exception):
            if self._legacy_secret_root.resolve() == Path(self._secret_store.storage_info.path).parent.resolve():
                return
        legacy_store = SecretStore(self._legacy_secret_root)
    ...
    def runtime_settings(self) -> dict[str, Any]:
        ...
        return {
            "requireMention": bool(candidate.get("requireMention", defaults["requireMention"])),
            "voiceRequireWakeWord": bool(candidate.get("voiceRequireWakeWord", defaults["voiceRequireWakeWord"])),
    ...
    def update_runtime_settings(self, updates: dict[str, Any], *, restart_if_running: bool = True) -> dict[str, Any]:
        ...
        if "require_mention" in updates:
            next_settings["requireMention"] = bool(updates.get("require_mention"))
        if "voice_require_wake_word" in updates:
            next_settings["voiceRequireWakeWord"] = bool(updates.get("voice_require_wake_word"))
        if "voice_wake_words" in updates:
            next_settings["voiceWakeWords"] = _normalize_string_list(updates.get("voice_wake_words")) or list(
                next_settings["voiceWakeWords"] or list(_DEFAULT_VOICE_WAKE_WORDS)
            )
        if "voice_media_volume" in updates:
            next_settings["voiceMediaVolume"] = _clamp_int(
                updates.get("voice_media_volume"),
                default=int(next_settings["voiceMediaVolume"]),
                minimum=0,
                maximum=200,
            )
        if "voice_profile" in updates:
            next_settings["voiceProfile"] = _safe_text(updates.get("voice_profile"))
