...
test("parseNativeCommand detects play requests and sound effects", () => {
  ...
  assert.deepEqual(
    parseNativeCommand("Okay, anyways, can you play music?"),
    { type: "play_music" },
  );
  assert.deepEqual(
...
test("parseNativeCommand detects playback control requests", () => {
  ...
  assert.deepEqual(parseNativeCommand("Thomas, pause music."), { type: "pause" });
  assert.deepEqual(parseNativeCommand("pause the song"), { type: "pause" });
  assert.deepEqual(parseNativeCommand("skip this song"), { type: "skip" });
  assert.deepEqual(parseNativeCommand("resume playback"), { type: "resume" });
  assert.deepEqual(parseNativeCommand("resume the song"), { type: "resume" });
  assert.deepEqual(parseNativeCommand("presume music"), { type: "resume" });
  ...
  assert.deepEqual(parseNativeCommand("what song is playing"), { type: "status" });
  assert.deepEqual(parseNativeCommand("set volume to 135 percent"), {
    type: "volume",
    volumePercent: 135,
  });
  assert.deepEqual(parseNativeCommand("volume up"), {
    type: "volume_relative",
    deltaPercent: 10,
  });
  assert.deepEqual(parseNativeCommand("turn the music down"), {
    type: "volume_relative",
    deltaPercent: -10,
  });
  assert.deepEqual(parseNativeCommand("turn the music down a bit"), {
    type: "volume_relative",
    deltaPercent: -10,
  });
  ...
  assert.deepEqual(parseNativeCommand("Thomas, play, pain remains."), {
    type: "play",
    query: "pain remains",
  });
  assert.deepEqual(parseNativeCommand("queue pain remains by lorna shore"), {
    type: "queue_track",
    query: "pain remains by lorna shore",
  });
});
