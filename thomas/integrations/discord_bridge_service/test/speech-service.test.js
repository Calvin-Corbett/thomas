...
test("buildSpeechEnv prepends local CUDA runtime directories when present", () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "thomas-speech-env-"));
  const cudaDir = path.join(
    tempRoot,
    "runtime",
    ".thomas",
    "discord_bridge",
  ...
  fs.writeFileSync(path.join(cudaDir, "cublas64_12.dll"), "");

  const env = buildSpeechEnv({
    PATH: "C:\\Windows\\System32",
    THOMAS_PROJECT_ROOT: tempRoot,
  });
  const pathValue = env.PATH;

  assert.match(pathValue, /cuda-libs[\\/]+cuda12_v3/i);
});
