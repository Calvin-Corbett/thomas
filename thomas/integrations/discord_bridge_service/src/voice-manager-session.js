...
export const voiceSessionMethods = {
  buildVoiceJoinAnnouncement() {
    return "Thomas is online.";
  },
    ...
    for (const guildId of targetGuildIds) {
      try {
        ...
        await this.joinChannel(channel, {
          ...
          announceText: this.buildVoiceJoinAnnouncement(),
        });
  ...
  async applyRuntimeSettings(nextSettingsRaw = null, previousSettingsRaw = null) {
    const nextSettings = nextSettingsRaw || this.getRuntimeSettings();
    const previousSettings = previousSettingsRaw || {};
    const nextVolume = clampVolumePercent(nextSettings.voiceMediaVolume, this.config.voiceMediaVolume);
    const previousWakeRequired = Boolean(previousSettings.voiceRequireWakeWord);
    const nextWakeRequired = Boolean(nextSettings.voiceRequireWakeWord);
    const wakeWordsChanged = JSON.stringify(previousSettings.voiceWakeWords || []) !== JSON.stringify(nextSettings.voiceWakeWords || []);

    this.config.voiceMediaVolume = nextVolume;
    this.config.voiceRequireWakeWord = nextWakeRequired;
    this.config.voiceWakeWords = Array.isArray(nextSettings.voiceWakeWords)
      ? [...nextSettings.voiceWakeWords]
      : [...this.config.voiceWakeWords];
    this.config.requireMention = Boolean(nextSettings.requireMention);

    for (const state of this.sessions.values()) {
      if (state.volumePercent !== nextVolume) {
        state.volumePercent = nextVolume;
        state.currentAudioResource?.volume?.setVolume(nextVolume / 100);
      }
      if ((!previousWakeRequired && !nextWakeRequired) || wakeWordsChanged || !nextWakeRequired) {
        state.ignoredUntilByUser.clear();
      }
    }

    const nextProfile = String(nextSettings.voiceProfile || "").trim();
    if (
      nextProfile
      && nextProfile !== this.ttsRuntime.model
      && String(this.ttsRuntime.backend || "").toLowerCase() === "piper"
    ) {
      try {
        await this.switchVoiceProfile(nextProfile, { persist: false });
      } catch (error) {
        console.warn(`[thomas-discord] Voice profile apply failed: ${error.message}`);
      }
    }
  },

  async switchVoiceProfile(profileInput, { persist = true } = {}) {
    ...
    this.config.voiceTtsModel = profile.id;
  ...
  async setVoiceProfile(profileInput) {
    return this.switchVoiceProfile(profileInput, { persist: true });
  },
