...
import {
  ...
  VOICE_IGNORE_DURING_BOT_SPEECH_MS,
  VOICE_REPEAT_WINDOW_MS,
  VOICE_STALE_PLAYBACK_MS,
  ...
  async handleSpeakingStart(state, userId) {
    ...
    if (state.player.state.status !== AudioPlayerStatus.Idle) {
      if (state.playbackKind === "speech") {
        ...
        if (msSinceSpoken < VOICE_IGNORE_DURING_BOT_SPEECH_MS) {
          console.log(
            `[thomas-discord] Voice ignored start in ${state.voiceChannelName}: bot recently spoke ${msSinceSpoken}ms ago`,
          );
          return;
        }

        if (msSinceSpoken > VOICE_STALE_PLAYBACK_MS) {
          console.warn(
            `[thomas-discord] Voice resetting stale playback state in ${state.voiceChannelName}: ${state.player.state.status}`,
          );
          this.stopPlayer(state);
        } else {
          console.log(
            `[thomas-discord] Voice proceeding despite non-idle speech playback in ${state.voiceChannelName}: ${state.player.state.status}`,
          );
        }
      } else {
        console.log(
    ...
    try {
      ...
      const pcm = await this.captureUserSpeech(state, userId, {
      ...
      try {
        ...
        rawTranscript = await this.speech.transcribeWav(wavPath, this.getVoiceTranscriptionOptions());
        console.log(
