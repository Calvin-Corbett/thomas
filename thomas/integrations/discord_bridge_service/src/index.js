...
import { createNativeMessageCommandHandler } from "./index_native_commands.js";
...
export async function main() {
  ...
  settingsStore.on("change", (nextSettings, previousSettings) => {
    void voiceManager.applyRuntimeSettings(nextSettings, previousSettings).catch((error) => {
      console.warn(`[thomas-discord] Runtime settings apply failed: ${error.message}`);
    });
  });
  ...
  const handleNativeMessageCommand = createNativeMessageCommandHandler({
    config,
    voiceManager,
    sendLongReply,
    formatDelegatedAccessList,
    ...
    if (interaction.commandName === "voice") {
      ...
      try {
        ...
        if (subcommand === "resume") {
          const resumed = await voiceManager.resumeOrRestartPlayback(interaction.guildId, {
            requestedBy: interaction.member?.displayName || interaction.user?.globalName || interaction.user?.username || "someone",
