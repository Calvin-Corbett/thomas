...
function normalizeCommandText(value) {
  return normalizeWhitespace(
    stripFillerWords(value)
      .replace(/[.!?]+$/g, ""),
  );
}

function normalizeAsrCommandText(value) {
  return normalizeWhitespace(value)
    .replace(/\bpresume\b/gi, "resume")
    .replace(/\bresume it\b/gi, "resume")
    .replace(/\bskip this\b/gi, "skip")
    .replace(/\bturn it up\b/gi, "volume up")
    .replace(/\bturn it down\b/gi, "volume down")
    .replace(/\bmake it louder\b/gi, "volume up")
    .replace(/\bmake it quieter\b/gi, "volume down")
    .replace(/\bmake it softer\b/gi, "volume down");
}
...
function stripFillerWords(value) {
  let text = normalizeAsrCommandText(value);
  let changed = true;

  while (changed && text) {
    changed = false;
    const next = text
      .replace(/^(?:uh|um|hey|yo|okay|ok|please|all\s+right|alright|right|well|so|anyway|anyways|man|dude|buddy)[\s,.:;!?-]+/i, "")
      .replace(/^thomas[\s,.:;!?-]+/i, "")
...
function isGenericPlayQuery(value) {
  const normalized = normalizeWhitespace(value)
    .toLowerCase()
    .replace(/^(?:some|the|a)\s+/i, "")
    .replace(/[.!?]+$/g, "");
  return [
    "music",
    "song",
...
export function parseNativeCommand(text) {
  ...
  if (/^(?:stop|turn off)\s+(?:the\s+)?(?:music|playback|song|track)$/i.test(normalized)) {
    return { type: "stop" };
  }

  if (/^(?:pause|pause (?:the )?(?:music|playback|song|track)|pause it|hold (?:the )?(?:music|playback)|hold it)$/i.test(normalized)) {
    return { type: "pause" };
  }

  if (/^(?:resume|resume (?:the )?(?:music|playback|song|track)|unpause|continue|continue (?:the )?(?:music|song|track)|keep playing|keep (?:the )?(?:music|song|track) going|play it)$/i.test(normalized)) {
    return { type: "resume" };
  }

  if (/^(?:skip|skip song|skip track|skip this|skip this song|skip this track|next song|next track|next one)$/i.test(normalized)) {
    return { type: "skip" };
  }
  ...
  if (/^(?:status|voice status|what(?:'| i)s playing|what is playing|now playing|what(?:'| i)s this song|what is this song|what song is playing)$/i.test(normalized)) {
    return { type: "status" };
  }

  const relativeVolumeMatch = normalized.match(
    /^(?:volume\s+)?(?<direction>up|down|louder|quieter|softer)$/i,
  ) || normalized.match(
    /^(?:turn|make|bring)\s+(?:the\s+)?(?:volume|music|audio|sound)\s+(?<direction>up|down|louder|quieter|softer)(?:\s+(?:a\s+)?(?:bit|little))?$/i,
  ) || normalized.match(
    /^(?:turn|make|bring)\s+(?:it)\s+(?<direction>up|down|louder|quieter|softer)(?:\s+(?:a\s+)?(?:bit|little))?$/i,
  );
  if (relativeVolumeMatch?.groups?.direction) {
    const direction = String(relativeVolumeMatch.groups.direction).toLowerCase();
    return {
      type: "volume_relative",
      deltaPercent: ["up", "louder"].includes(direction) ? 10 : -10,
    };
  }
  ...
  const playMatch = buildCommandMatch(normalized, [
    /^(?<verb>play|queue)[\s,.:;-]+(?<query>.+)$/i,
    /^(?<verb>put on|start)[\s,.:;-]+(?<query>.+)$/i,
  ]);
  if (playMatch?.groups?.query) {
    const query = normalizeWhitespace(playMatch.groups.query.replace(/[.!?]+$/g, ""));
    const verb = normalizeWhitespace(playMatch.groups.verb).toLowerCase();
    if (isGenericPlayQuery(query)) {
      return {
        type: "play_music",
      };
    }
