...
_DEFAULT_MAX_FILES_PER_COMMIT = 50
...
_DEFAULT_SCOPED_COMMIT_AGENT = "workspace-sync"
...
def _scope_key(rel_path: str) -> str:
    parts = Path(_normalize_rel_path(rel_path)).parts
    if not parts:
        return "<root>"
    if len(parts) == 1:
        return parts[0]
...
class WorkspaceSyncEngine:
    ...
    def _run_cycle(self, *, reason: str, force: bool) -> dict[str, Any]:
        ...
        files = self._select_commit_batch(stable_files)
        ...
        try:
            ...
            commit_result = self._commit_with_scoped_helper(files, reason=reason)
            if not bool(commit_result.get("ok")):
                skip_reason = str(commit_result.get("skip_reason") or "scoped_commit_failed")
                return self._finish(
                    reason=reason,
                    committed=False,
            ...
            committed_files = list(commit_result.get("selected_paths") or files)
            title, _body = self._build_commit_message(committed_files, reason=reason)
            commit_sha = str(commit_result.get("commit_sha") or self._git_text("rev-parse", "--short", "HEAD") or "").strip()
            push = self._push_latest()
            details: dict[str, Any] = {
                "files": committed_files[:50],
    ...
    def _select_candidate_files(self, status_rows: list[str]) -> list[dict[str, Any]]:
        ...
        for raw in status_rows:
            ...
            if not deleted and full.is_dir():
                for nested in sorted(path for path in full.rglob("*") if path.is_file()):
                    nested_rel = _normalize_rel_path(str(nested.relative_to(self._repo_root)))
                    if not nested_rel or self._is_excluded(nested_rel):
                        continue
                    out.append({"path": nested_rel, "deleted": False, "code": code})
                continue
    ...
    def _is_excluded(self, rel_path: str) -> bool:
        ...
        if not normalized:
            return True
        return any(fnmatch(normalized, pat) for pat in self._exclude_globs)
    ...
    def _select_commit_batch(self, files: list[str]) -> list[str]:
        max_files = max(1, int(self._max_files_per_commit))
        if len(files) <= max_files:
            return list(files)
        grouped: dict[str, list[str]] = {}
        for rel in files:
            grouped.setdefault(_scope_key(rel), []).append(rel)
        for _scope, scope_files in grouped.items():
            if scope_files:
                return list(scope_files[:max_files])
        return list(files[:max_files])

    def _commit_with_scoped_helper(self, files: list[str], *, reason: str) -> dict[str, Any]:
        workboard_path = self._repo_root / "plans" / "thomas" / "WORKBOARD.md"
        if not workboard_path.exists():
            return self._commit_with_legacy_git(files, reason=reason)
        title, body = self._build_commit_message(files, reason=reason)
        message = f"{title}\n\n{body}"
        fallback_reason = f"{self._coordination_note}: {reason or 'workspace_sync'}"
        cmd = [
            os.environ.get("PYTHON", sys.executable),
            "scripts/agent_commit.py",
            "--json",
            "--agent",
            str(os.environ.get("THOMAS_WORKSPACE_SYNC_AGENT_ID") or _DEFAULT_SCOPED_COMMIT_AGENT),
            "--message",
            message,
        ...
        if ok:
            return {
                ...
                "selected_paths": list(payload.get("selected_paths") or files),
                "message": str(payload.get("message") or "scoped agent commit created"),
                "blocker_class": None,
        ...
        blocker_class = str(payload.get("blocker_class") or "").strip().lower()
        skip_reason = "scoped_commit_failed"
        if blocker_class == "needs_refactor":
            skip_reason = "needs_refactor"
        elif str(payload.get("gate_name") or "").strip().lower() == "protected_files":
            skip_reason = "protected_files_blocked"
        ...
        return {
            ...
            "commit_sha": "",
            "selected_paths": list(payload.get("selected_paths") or files),
            "message": str(payload.get("message") or f"scoped commit helper failed with exit code {proc.returncode}"),
            "blocker_class": blocker_class or "local_gate_failed",
            "gate_name": str(payload.get("gate_name") or ""),
            "next_step": str(payload.get("next_step") or ""),
            "suggested_command": str(payload.get("suggested_command") or ""),
            "stdout": _truncate(stdout),
    ...
    def _commit_with_legacy_git(self, files: list[str], *, reason: str) -> dict[str, Any]:
        ...
        if commit_result.returncode != 0:
            ...
            return {
                ...
                "skip_reason": skip_reason,
                "commit_sha": "",
                "selected_paths": staged_after_add,
                "message": "legacy git commit failed",
                "blocker_class": "local_gate_failed",
                "gate_name": "git_commit",
                "next_step": "",
                "suggested_command": None,
        ...
        return {
            ...
            "commit_sha": commit_sha,
            "selected_paths": staged_after_add,
            "message": "legacy git commit created",
            "blocker_class": None,
            "gate_name": "",
            "next_step": "",
            "suggested_command": None,
