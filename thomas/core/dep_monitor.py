...
def _get_deps_scan():
    """Deferred import: core must not import from thomas.tools at module level."""
...
class DepMonitor:
    ...
    def _scan_project(self) -> dict[str, Any]:
        ...
        if pyproject.exists():
            try:
                report["python"] = _get_deps_scan()({"target": str(pyproject), "ecosystem": "python"})
            except Exception as e:
                report["python"] = {"error": str(e)}
        elif reqs.exists():
            try:
                report["python"] = _get_deps_scan()({"target": str(reqs), "ecosystem": "python"})
            except Exception as e:
                report["python"] = {"error": str(e)}
        ...
        if npm_target is not None:
            try:
                report["npm"] = _get_deps_scan()({"target": str(npm_target), "ecosystem": "npm"})
            except Exception as e:
