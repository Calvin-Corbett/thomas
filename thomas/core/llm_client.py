...
class LLMClient:
    ...
    def _record_usage_to_cost_tracker(self, usage: Any) -> None:
        if usage is None:
            return
        try:
            from thomas.core.cost_tracker import extract_token_usage, get_cost_tracker

            prompt_tokens, completion_tokens = extract_token_usage(usage)
            if prompt_tokens <= 0 and completion_tokens <= 0:
                return
            get_cost_tracker().record(
                model=str(self.config.model or self.config.name or "unknown"),
                provider=str(self.config.provider or ""),
                prompt_tokens=prompt_tokens,
                completion_tokens=completion_tokens,
            )
        except Exception as exc:
            log.debug("Cost tracker usage recording skipped: %s", exc)
    ...
    async def _stream_current_provider(
        ...
        if self.config.provider == "codex":
            ...
            async for event in stream_obj:
                if event.type == "usage":
                    last_usage = event.data.get("usage")
                yield event
        else:
            stream_obj = await _coerce_async_iterator(
            ...
            async for event in stream_obj:
                if event.type == "usage":
                    last_usage = event.data.get("usage")
                yield event
        self._record_usage_to_cost_tracker(last_usage)
