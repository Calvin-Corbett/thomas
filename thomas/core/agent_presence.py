...
def _apply_file_activity(
    ...
    for row in rows:
        ...
        if not scope:
            continue
        matches = [
            item for item in activity if any(_paths_overlap(str(item.get("path") or ""), claimed) for claimed in scope)
        ]
        if not matches:
            continue
        matches.sort(
        ...
        row["recent_paths"] = _dedupe([*list(row.get("recent_paths") or []), *recent_paths])[:8]
        ...
        row["source_signals"] = _dedupe([*list(row.get("source_signals") or []), "file_activity"])
        family = _family_from_agent_label(str(row.get("agent_id") or row.get("display_name") or ""))
        ...
        if "process_tree" in list(row.get("source_signals") or []) and str(row.get("state") or "") in {
            ...
            row["source_signals"] = _dedupe([*list(row.get("source_signals") or []), "global_agent_process"])
            row["warnings"] = _dedupe(
                [
                    *list(row.get("warnings") or []),
                    f"{_display_name_for_family(family)} has a live process tree and fresh activity in its claimed scope.",
                ]
            )
            row["evidence"] = _dedupe([*list(row.get("evidence") or []), *list(family_presence.get("commands") or [])])[
                :8
            ]
            row["process_pids"] = _dedupe(
...
def _read_inferred_state(repo_root: Path) -> dict[str, Any]:
    payload = _read_json(inferred_state_path(repo_root), {})
    if not isinstance(payload, dict):
        payload = {}
...
def _reduce_inferred_scope(path: str) -> str:
    token = _normalize_relpath(path)
    if not token or _is_noise_activity_path(token):
        return ""
...
def _claim_conflict(
    ...
    for row in claims:
        claim_scope = _normalize_scope(row.get("scope"))
        if not claim_scope or not any(_paths_overlap(scope, item) for item in claim_scope):
            continue
...
def _match_families_for_scope(
    ...
    for row in rows:
        ...
        if row_scope and any(_paths_overlap(scope, item) or _paths_overlap(path, item) for item in row_scope):
            matches.add(family)
            continue
        if recent_paths and any(
            _paths_overlap(path, item) or _paths_overlap(scope, _reduce_inferred_scope(item)) for item in recent_paths
        ):
            matches.add(family)
    return matches


def _infer_claim_candidates(
    *,
    ...
    for item in activity:
        ...
        family_rows = [
            row
            for row in rows
            if _family_from_agent_label(str(row.get("agent_id") or row.get("display_name") or "")) == family
        ]
        preferred = next(
            (
                row
                for row in family_rows
                if str(row.get("state") or "") in {"active", "alive"} and _normalize_scope(row.get("scope"))
            ),
            None,
        )
        ...
        display_name = str((preferred or {}).get("display_name") or _display_name_for_family(family)).strip()
        ...
        candidate["recent_paths"] = _dedupe([*list(candidate.get("recent_paths") or []), path])[:8]
...
def collect_presence(
    *, repo_root: str | Path | None = None, workboard_path: str | Path | None = None
    ...
    _apply_file_activity(rows, activity, global_presence)
    claimed = [
        scope
        for row in rows
    ...
    if uncovered:
        ...
        rows = list(agents.values())
    inferred_candidates, inferred_suppressed_now = _infer_claim_candidates(
        rows=rows,
        activity=activity,
    ...
    inferred_state = _read_inferred_state(repo)
    payload = {
        "success": True,
        "repo_root": str(repo),
