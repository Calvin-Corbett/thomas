...
SECURITY_PROFILE_ORDER = ("balanced", "hands_on", "review_only")
SECURITY_PROFILE_ALIASES = {
    ...
    "hands_on": "hands_on",
    "hands-on": "hands_on",
    "guided_build": "hands_on",
    "locked": "review_only",
    "review_only": "review_only",
    "review-only": "review_only",
    "check_only": "review_only",
}


def normalize_security_profile(profile: str | None) -> str:
    """Normalize the configured security profile."""
    raw = str(profile or "balanced").strip().lower().replace("-", "_") or "balanced"
    normalized = SECURITY_PROFILE_ALIASES.get(raw, raw)
    if normalized not in SECURITY_PROFILE_ORDER:
        raise ValueError(
            "security.profile must be one of: " + ", ".join(f"'{name}'" for name in SECURITY_PROFILE_ORDER)
        )
    return normalized


def security_profile_options() -> list[dict[str, Any]]:
    """Return user-facing security profile metadata."""
    return [
        {
            "id": "balanced",
            "title": "Balanced",
            ...
            "best_for": "Daily use on your own machine.",
            "recommended": True,
        },
        {
            "id": "hands_on",
            "title": "Hands-On",
            "summary": "Same install guardrails as Balanced, but tuned for people actively extending Thomas with assistant-guided changes.",
            "tradeoff": "Best when you want Thomas to help you modify the product, but still only after explicit approval.",
            "best_for": "Builders who want tighter workflow integration without background mutation.",
            "recommended": False,
        },
        {
            "id": "review_only",
            "title": "Review Only",
            "summary": "Check-only mode for setup and updates. Thomas can inspect and explain, but it will not install tools or apply updates.",
            "tradeoff": "Highest friction, lowest surprise.",
            "best_for": "Sensitive machines where you want every environment change to happen manually.",
            "recommended": False,
        },
    ]


def security_capabilities_for_profile(profile: str | None) -> dict[str, Any]:
    """Return derived capabilities for a security profile."""
    normalized = normalize_security_profile(profile)
    tool_install_allowed = normalized in {"balanced", "hands_on"}
    update_apply_allowed = normalized in {"balanced", "hands_on"}
    blocked_actions = {
        "tool_install": "" if tool_install_allowed else "Review Only mode blocks tool installation.",
        "update_apply": "" if update_apply_allowed else "Review Only mode allows update checks only.",
    }
    return {
        "tool_install_allowed": tool_install_allowed,
        "tool_install_requires_confirmation": tool_install_allowed,
        "update_apply_allowed": update_apply_allowed,
        "update_apply_requires_confirmation": update_apply_allowed,
        "startup_update_checks_allowed": True,
        "background_mutation_allowed": False,
        "assistant_guided_installs": normalized == "hands_on",
...
class SecurityConfig:
    """Configuration for dependency/update security posture."""

    profile: str = "balanced"

    def validate(self) -> list[str]:
        try:
            normalize_security_profile(self.profile)
...
class AppConfig:
    ...
    def security_profile(self) -> str:
        """Return the normalized security profile."""
        return normalize_security_profile(self.security.profile)

    def security_capabilities(self) -> dict[str, Any]:
        """Return derived security capabilities for the active profile."""
        return security_capabilities_for_profile(self.security_profile)
    ...
    def failover_chain(self, primary_profile: str) -> list[ModelConfig]:
        ...
        if explicit:
            ...
            ordered_names.extend([name for name in self.models if name != primary])
...
def _env_override(data: dict[str, Any], prefix: str = "THOMAS") -> None:
    ...
    Examples:
      ...
      THOMAS_DATA_DIR=%LOCALAPPDATA%/Thomas
      THOMAS_PROFILE=demo
    ...
    core_section_fields: dict[str, set[str]] = {
        ...
        "security": set(getattr(SecurityConfig, "__dataclass_fields__", {}).keys()),
    }
    extension_sections = {
...
def _auto_discover_bool_fields() -> set:
    ...
    discovered: set = set()
    for cls in (ToolsConfig, MemoryConfig, ServerConfig, QualityConfig, FailoverConfig, SecurityConfig):
...
def _collect_unknown_core_keys(data: dict[str, Any]) -> list[str]:
    ...
    _scan_section("security", _field_names(SecurityConfig))
...
def load_config(
    ...
    *,
    ...
    raw_security_profile = str(security_data.get("profile", "balanced") or "balanced")
    try:
        normalized_security_profile = normalize_security_profile(raw_security_profile)
    except ValueError:
        normalized_security_profile = raw_security_profile
    security = SecurityConfig(profile=normalized_security_profile)
