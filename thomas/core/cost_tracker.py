...
class CostTracker:
    ...
    def __init__(self, spend_path: Path | None = None, toml_path: Path | None = None) -> None:
        ...
        self._session_start_pos = self._current_eof()
    ...
    def session_usd(self) -> float:
        return float(self._session_snapshot()["total_usd"])

    def session_call_count(self) -> int:
        return int(self._session_snapshot()["call_count"])

    def session_tokens(self) -> dict[str, int]:
        return dict(self._session_snapshot()["tokens"])

    def session_by_model_detail(self) -> dict[str, dict[str, Any]]:
        return dict(self._session_snapshot()["by_model_detail"])
    ...
    def _current_eof(self) -> int:
        try:
            return int(self._spend_path.stat().st_size)
        except FileNotFoundError:
            return 0

    def _read_rows_from_offset(self, start_pos: int) -> list[dict[str, Any]]:
        ...
        offset = max(0, min(int(start_pos or 0), size))
        if offset >= size:
            return []
        ...
        text = blob[: last_nl + 1].decode("utf-8", errors="replace")
    ...
    def _session_snapshot(self) -> dict[str, Any]:
        with self._lock:
            start_pos = self._session_start_pos

        if self._current_eof() < start_pos:
            start_pos = 0
        ...
        prompt_tokens = 0
        completion_tokens = 0
        by_model_detail: dict[str, dict[str, Any]] = {}

        for row in self._read_rows_from_offset(start_pos):
            model = str(row.get("model", "unknown")).strip() or "unknown"
            usd_total = float(row.get("usd_total", 0.0) or 0.0)
            ...
            entry = by_model_detail.setdefault(
                model,
                {
                    "usd": 0.0,
                    "calls": 0,
            ...
            entry["usd"] = float(entry["usd"]) + usd_total
            entry["calls"] = int(entry["calls"]) + 1
            entry["prompt_tokens"] = int(entry["prompt_tokens"]) + pt
            entry["completion_tokens"] = int(entry["completion_tokens"]) + ct
        ...
        return {
            ...
            "by_model_detail": dict(
                sorted(
                    by_model_detail.items(),
                    key=lambda kv: (-float(kv[1].get("usd", 0.0)), kv[0].lower()),
                ),
            ),
        }
