...
from .guardrails_policy import (
    apply_guardrails_posture_overlay,
    normalize_guardrails_posture,
    sync_guardrails_security_state,
)
...
class PreferencesStore:
    ...
    def set_third_party_agent_access(
        ...
        *,
        ...
        with self._lock, self._connect() as conn, conn:
            ...
            advanced = sync_guardrails_security_state(advanced)
    ...
    def set_human_breakglass_enabled(
        ...
        *,
        ...
        from scripts.breakglass_auth import consume_breakglass_toggle_receipt

        receipt = consume_breakglass_toggle_receipt(authorization_receipt)
        if receipt is None:
            raise PermissionError("human breakglass preference changes require a fresh Windows authorization receipt")
        actor = str(receipt.actor or "").strip() or str(changed_by or "system")
        with self._lock, self._connect() as conn, conn:
            base = self._get_or_create_base_prefs(conn, user_id)
            advanced = dict(base.get("advanced") or {})
            security = AdvancedSecurityPrefs(**(advanced.get("security") or {})).model_dump()
            ...
            security["human_breakglass_changed_by"] = actor
            advanced["security"] = security
            advanced = sync_guardrails_security_state(advanced)
            base["advanced"] = advanced
            self._save_base_prefs(conn, user_id, base)
    ...
    def set_guardrails_posture(
        ...
        *,
        ...
        with self._lock, self._connect() as conn, conn:
            ...
            advanced = dict(base.get("advanced") or {})
            security = AdvancedSecurityPrefs(**(advanced.get("security") or {})).model_dump()
            security["guardrails_posture"] = normalize_guardrails_posture(posture)
            security["guardrails_posture_changed_at"] = utc_now_iso()
            security["guardrails_posture_changed_by"] = str(changed_by or "system")
            advanced["security"] = security
            advanced = sync_guardrails_security_state(advanced)
            base["advanced"] = advanced
            self._save_base_prefs(conn, user_id, base)
    ...
    def get(self, user_id: str = "default", thread_id: str | None = None) -> PreferencesResponse:
        with self._lock, self._connect() as conn:
            ...
            effective_base = deepcopy(base)
            effective_advanced = apply_guardrails_posture_overlay(effective_base.get("advanced"))
            effective_base["advanced"] = effective_advanced
            ...
            return PreferencesResponse(
                ...
                advanced=AdvancedPrefs(**(effective_base.get("advanced") or {})),
    ...
    def patch(
        ...
        with self._lock, self._connect() as conn, conn:
            ...
            self._enforce_non_coder_runtime_locks(base)
            if isinstance(base.get("advanced"), dict):
                base["advanced"] = sync_guardrails_security_state(base.get("advanced"))
            self._save_base_prefs(conn, user_id, base)
