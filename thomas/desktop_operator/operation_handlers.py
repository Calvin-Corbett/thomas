...
class OperationHandlersMixin:
    ...
    def _resolve_click_target(
        ...
        *,
        ...
        if x is not None and y is not None:
            return (int(x), int(y), 1.0, "coordinates")
        if selector is None:
            return (window.client_bounds.left + 10, window.client_bounds.top + 10, 0.5, "coordinates")
        resolved_selector = dict(selector)
        profile_adapter = self._resolve_allowlisted_adapter(window, preferred_profile=workflow_profile)
        if profile_adapter is not None:
            normalized = profile_adapter.normalize_selector(resolved_selector)
            if isinstance(normalized, dict):
                resolved_selector = normalized
        if hasattr(self.adapter, "snapshot_accessibility"):
            snapshot = self.adapter.snapshot_accessibility(window.handle)
            nodes = snapshot.get("nodes") if isinstance(snapshot, dict) else []
        else:
            nodes = self.adapter.ax_query(window, resolved_selector)
        if not nodes:
            raise not_found("Accessibility selector did not match any nodes.", selector=resolved_selector)
        matched = self._match_accessibility_node(nodes, resolved_selector)
        if matched is None:
            raise not_found(
                "Accessibility selector matched nodes but none satisfied the query.", selector=resolved_selector
            )
        ...
        return (center_x, center_y, 0.98, "semantic")
    ...
    def _match_accessibility_node(self, nodes: list[Any], selector: dict[str, Any]) -> dict[str, Any] | None:
        for node in nodes:
            ...
            node_automation_id = str(node.get("automation_id") or "").strip().lower()
            wanted_role = str(selector.get("role") or "").strip().lower()
            wanted_name = str(selector.get("name") or "").strip().lower()
            wanted_alias = str(selector.get("alias") or "").strip().lower()
            wanted_automation_id = str(selector.get("automation_id") or "").strip().lower()
            if wanted_role and node_role != wanted_role:
                continue
            if wanted_name and wanted_name not in node_name:
                continue
            if wanted_alias and wanted_alias not in node_name:
                continue
            if wanted_automation_id and node_automation_id != wanted_automation_id:
                continue
            return node
        return None
    ...
    def _action_class(self, adapter, action: str) -> str:
        ...
        adapter_classes = getattr(adapter, "action_classes", None)
        if isinstance(adapter_classes, dict):
            resolved = str(adapter_classes.get(action) or "").strip().lower()
            if resolved:
                return resolved
        if isinstance(ACTION_CLASSES, dict):
            resolved = str(ACTION_CLASSES.get(action) or "").strip().lower()
            if resolved:
                return resolved
        return "generic"
    ...
    def _supervisor_preflight(
        ...
        *,
        ...
        effective_action_class = str(action_class or "").strip().lower() or self._action_class(effective_adapter, action)

        if effective_action_class == "sensitive":
            raise session_error(
        ...
        if effective_action_class == "navigation":
            url = payload.get("url") or window.title
            self._require_allowed_domain(session, url)

        if effective_action_class in ("file_dialog", "filesystem"):
    ...
    def _require_allowed_domain(self, session: DesktopSession, url: str) -> None:
        ...
        if allowed and not any(domain.endswith(d.lstrip(".")) for d in allowed):
            self._pause_for_approval(
                session,
                reason="Requested URL is outside the allowlisted domains.",
                approval_reason="domain_not_allowlisted",
            )
            raise session_error(
                "Requested URL is not allowlisted for this session.",
                domain=domain,
                allowlist=allowed,
                session_id=session.session_id,
            )
    ...
    def _detect_secret_zone_hits(self, profile_summary: dict[str, Any], raw_ocr_text: str) -> list[str]:
        hits: list[str] = []
        summary_hits = profile_summary.get("secret_zone_hits") or profile_summary.get("labels") or []
        for item in summary_hits if isinstance(summary_hits, list) else []:
            text = str(item or "").strip().lower()
            if text and text not in hits:
                hits.append(text)
        if raw_ocr_text and self._looks_like_secret(raw_ocr_text):
            if "ocr_sensitive_text" not in hits:
                hits.append("ocr_sensitive_text")
        return hits
    ...
    def _redact_text(self, text: str, *, force: bool = False) -> str:
        ...
        redacted = _HIGH_RISK_TEXT_RE.sub("[REDACTED]", redacted)
        if force and redacted == text and text.strip():
            return "[REDACTED]"
        return redacted
    ...
    def _register_verification_result(self, session: DesktopSession, *, ok: bool, reason: str) -> None:
        if ok:
            ...
            session.verify_failures = int(session.verify_failures or 0) + 1
            if session.verify_failures >= _MAX_VERIFY_FAILURES_BEFORE_TERMINATE:
                self._trip_circuit(
                    session,
                    state="verification_failed",
                    reason=f"Verification failed {session.verify_failures} times.",
                )
    ...
    def _record_low_confidence(self, session: DesktopSession, *, target_confidence: float, action: str) -> None:
        session.low_confidence_events = int(session.low_confidence_events or 0) + 1
        self._last_refusal_reason = f"Low confidence ({target_confidence:.2%}) for action: {action}"
        if session.low_confidence_events >= _LOW_CONFIDENCE_LIMIT:
            self._trip_circuit(
                session,
                state="blocked_by_policy",
                reason=f"Low confidence recorded {session.low_confidence_events} times.",
            )

    def _pause_for_approval(self, session: DesktopSession, *, reason: str, approval_reason: str) -> None:
        self._set_session_state(session, "paused_for_approval", reason=reason, risk_level="high")
