...
class PlannerAgent:
    ...
    async def plan(self, *, goal: str, context: str = "", session_id: Optional[str] = None) -> Dict[str, Any]:
        schema_hint = {
            "type": "object",
            "required": ["actions"],
            "properties": {
        ...
        out = await self.chat.generate_json(system=self.prompts.system, user=user, schema_hint=schema_hint, session_id=session_id)
    ...
    def review(self, plan: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        for a in actions:
            ...
            if kind in ("shell", "exec", "filesystem_write", "network_request"):
                problems.append(f"Action kind '{kind}' is potentially dangerous; route via Guardrails and require approval.")
        ok = len(problems) == 0
        return ok, problems
