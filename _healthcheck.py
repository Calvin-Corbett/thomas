...
import sys
sys.path.insert(0, 'f:/DevHub/Thomas')
...
def t_config():
    ...
    cfg = load_config(Path('f:/DevHub/Thomas/thomas.toml'))
...
def t_bedrock():
    ...
    body = c._build_openai_request([], tools=[
        {'type': 'function', 'function': {'name': 'fs.write_file', 'description': '', 'parameters': {}}}
    ])
...
def t_audit():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        file_audit.record_file_write(run_id='r1', model='m', tool_name='fs.write_file', path='/a.py', action='write', content_before='old', content_after='new')
...
def t_tools():
    import thomas.tools.filesystem
    import thomas.tools.diff
    import thomas.tools.git
    import thomas.tools.shell
    import thomas.tools.code_search
    import thomas.tools.registry
...
def t_loop():
    from thomas.agent.loop import AgentLoop, SYSTEM_PROMPT
...
def t_inspector_js():
    src = open('f:/DevHub/Thomas/thomas/server/web/js/inspector.js', encoding='utf-8').read()
...
def t_audit_js():
    src = open('f:/DevHub/Thomas/thomas/server/web/js/audit.js', encoding='utf-8').read()
...
def t_audit_css():
    src = open('f:/DevHub/Thomas/thomas/server/web/css/layout.css', encoding='utf-8').read()
