...
def test_gate_prefers_explicit_fallback_scope_even_with_active_claim(tmp_path: Path, monkeypatch, capsys) -> None:
    workboard = _write_workboard(
        tmp_path,
        "- agent=Codex 2; scope=plans/thomas; task=planner lane",
        active_tasks_block="- task_id=planner-lane; agent=Codex 2; scope=plans/thomas; summary=planner lane; status=active",
    )
    monkeypatch.setenv("AGENT_ID", "Codex 2")
    monkeypatch.setenv("THOMAS_WORKBOARD_SCOPE_FALLBACK", "tests/test_agent_commit.py,tests/test_agent_maintenance.py")
    monkeypatch.setenv("THOMAS_WORKBOARD_SCOPE_FALLBACK_REASON", "maintenance checkpoint fallback")
    monkeypatch.setattr(mod, "_staged_files", lambda: ["tests/test_agent_commit.py", "tests/test_agent_maintenance.py"])
    ...
    assert payload["scope_source"] == "explicit_fallback"
    assert payload["matching_claim_count"] == 1
    assert payload["scopes"] == ["tests/test_agent_commit.py", "tests/test_agent_maintenance.py"]
