...
from check_protected_files_gate import (
...
class TestProtectedFilesGate:
    ...
    def test_agents_md_is_protected(self):
        ...
        assert "AGENTS.md" in IMMUTABLE_POLICY_FILES

    def test_guardrails_are_protected(self):
        """All GUARDRAILS.md files should be in the protected list."""
        assert "GUARDRAILS.md" in IMMUTABLE_POLICY_FILES
        assert "thomas/server/GUARDRAILS.md" in IMMUTABLE_POLICY_FILES
        assert "thomas/memory/GUARDRAILS.md" in IMMUTABLE_POLICY_FILES
        assert "thomas/agent/GUARDRAILS.md" in IMMUTABLE_POLICY_FILES

    def test_test_architecture_is_protected(self):
        """test_architecture.py should be protected."""
        assert "tests/test_architecture.py" in IMMUTABLE_POLICY_FILES

    def test_architecture_module_is_protected(self):
        """_architecture.py should be protected."""
        assert "thomas/_architecture.py" in IMMUTABLE_POLICY_FILES

    def test_runtime_and_release_sensitive_files_remain_in_protected_union(self):
        """Scoped commits may allow them, but they should remain visible to protected-file audits."""
        assert "thomas/agent/dispatch.py" in RUNTIME_SENSITIVE_FILES
        assert "pyproject.toml" in RELEASE_SENSITIVE_FILES
        assert "thomas/agent/dispatch.py" in PROTECTED_FILES
        assert "pyproject.toml" in PROTECTED_FILES
    ...
    def test_run_blocks_runtime_sensitive_files_without_scoped_commit_class(self, monkeypatch, capsys):
        """Runtime-sensitive files should still block raw/direct commits."""
        monkeypatch.delenv("THOMAS_COMMIT_CLASS", raising=False)
        monkeypatch.setattr(protected_gate, "_staged_files", lambda: ["thomas/agent/dispatch.py"])

        rc = protected_gate.run([])
        out = capsys.readouterr().out

        assert rc == 1
        assert "thomas/agent/dispatch.py" in out
        assert "runtime-sensitive path" in out

    def test_run_blocks_runtime_sensitive_files_for_private_checkpoint_commit_class(self, monkeypatch, capsys):
        """Commit class labels must not weaken runtime-sensitive protection."""
        monkeypatch.setenv("THOMAS_COMMIT_CLASS", "private-checkpoint")
        monkeypatch.setattr(protected_gate, "_staged_files", lambda: ["thomas/agent/dispatch.py"])
    ...
    def test_run_blocks_release_sensitive_files_for_private_checkpoint_commit_class(self, monkeypatch, capsys):
        """Commit class labels must not weaken release-sensitive protection."""
        monkeypatch.setenv("THOMAS_COMMIT_CLASS", "private-checkpoint")
        monkeypatch.setattr(protected_gate, "_staged_files", lambda: ["pyproject.toml"])
        ...
        assert "release-sensitive path" in out
