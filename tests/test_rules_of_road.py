...
def test_coding_write_with_failed_monolith_guard_fails_required_gate():
    report = evaluate_rules(
        route_path="coding_task",
        prompt_text="refactor module boundaries",
        response_text="Patched.",
    ...
    assert "Split or extract" in prompt or "Split/refactor" in prompt


def test_coding_write_to_oversized_file_requires_refactor(tmp_path) -> None:
    oversized = tmp_path / "ui.js"
    oversized.write_text("\n".join(f"const line_{i} = {i};" for i in range(1305)) + "\n", encoding="utf-8")

    report = evaluate_rules(
        route_path="coding_task",
        prompt_text="add another UI control",
        response_text="Patched and validated.",
        tool_events=[
            {
                "name": "diff.create",
    ...
    failed_ids = {c["id"] for c in report["checks"] if c["required"] and not c["passed"]}
    assert "coding_large_file_refactor" in failed_ids
    oversized_signal = (report.get("signals") or {}).get("oversized_written_files") or []
    assert oversized_signal
    assert oversized_signal[0]["path"] == "ui.js"
    prompt = build_remediation_prompt(report)
    assert "ui.js" in prompt
    assert "Do not keep appending" in prompt
