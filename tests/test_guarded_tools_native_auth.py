...
class _RequireNativeAuthPolicy:
    def evaluate(self, _ctx: Any) -> PolicyDecision:
        return PolicyDecision(
            type=PolicyDecisionType.REQUIRE_APPROVAL,
            reason="editing protected Thomas internals",
            meta={"native_auth": True},
        )
...
async def test_guarded_tool_runner_requires_native_auth_in_no_human_allow(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = await runner.run(
        ...
        iteration=1,
        cwd="D:/Workspaces/Thomas",
        sandbox_root="D:/Workspaces/Thomas",
        runtime_root="D:/Workspaces/Thomas/runtime",
...
@pytest.mark.asyncio
async def test_guarded_tool_runner_requires_native_auth_even_in_human_mode(
    ...
    runner = GuardedToolRunner(
        policy=_RequireNativeAuthPolicy(),
        approvals=ApprovalBroker(),
        redactor=Redactor(),
        no_human_mode="human",
    )
    ...
    result = await runner.run(
        executor=lambda _tool_call: {"ok": True, "result": "done"},
        tool_call={"id": "tool-1", "name": "fs.write_file", "args": {"path": "agent_safety.toml"}},
        run_id="run-1",
        session_id="session-1",
        iteration=1,
        cwd="D:/Workspaces/Thomas",
        sandbox_root="D:/Workspaces/Thomas",
        runtime_root="D:/Workspaces/Thomas/runtime",
        conversation_summary="edit protected config",
        emit_event=_emit,
    )
    ...
    resolved = [payload for kind, payload in events if kind == "TOOL_APPROVAL_RESOLVED"]
    assert required[0]["decision"] == "NATIVE_AUTH_REQUIRED"
    assert required[0]["source"] == "native_auth"
    assert resolved[0]["decision"] == "NATIVE_AUTH_APPROVED"
