...
def test_runtime_skills_selection_prefers_explicit_then_pinned(tmp_path: Path, monkeypatch) -> None:
    ...
    _write_skill(home_root / '.thomas', 'explicit-skill', 'Always use the explicit flow.', '- Keep changes deterministic.')
    _write_skill(home_root / '.thomas', 'pinned-skill', 'Always include the pinned behavior.', '- Preserve the pinned workflow.')
...
def test_runtime_skills_context_format_includes_selected_instructions(tmp_path: Path, monkeypatch) -> None:
    ...
    _write_skill(home_root / '.thomas', 'robot-ui', 'Use live screenshots and verify before finishing.', '- Verify before finishing.')
    selection = resolve_runtime_skills(
        cfg,
