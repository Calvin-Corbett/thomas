...
class TestEnvOverride(unittest.TestCase):
    ...
    def test_security_section_override(self) -> None:
        os.environ["THOMAS_SECURITY_PROFILE"] = "locked"
        cfg = load_config(Path("__does_not_exist__.toml"))
        self.assertEqual(cfg.security.profile, "review_only")
        self.assertEqual(cfg.security_profile, "review_only")
    ...
    def test_unknown_core_keys_are_reported(self) -> None:
        ...
        try:
            ...
            self.assertTrue(any("Unknown core config key 'security.profiel'" in e for e in errs))
        finally:
            if cfg_path.exists():
                cfg_path.unlink()
