...
import scripts.workboard_message as mod
...
def test_concurrent_sends_preserve_all_messages_and_board_sections(tmp_path: Path) -> None:
    workboard = _write_workboard(tmp_path)

    def send(index: int) -> tuple[bool, dict[str, object]]:
        return mod.send_message(
            workboard,
            sender=f"Codex {index}",
            recipient="task-manager-agent",
            summary=f"lane {index} online",
            task_id="swarm-target",
            kind="status",
            priority="p0",
            requested_action="none",
            decision="pending",
            msg_id=f"msg-concurrent-{index:02d}",
        )

    with ThreadPoolExecutor(max_workers=10) as pool:
        results = list(pool.map(send, range(25)))

    assert all(ok for ok, _payload in results), results
    text = workboard.read_text(encoding="utf-8")
    assert "## Agent Claims" in text
    assert "## Active Tasks" in text
    assert "## Up For Grabs" in text
    assert "## Issues / Blockers" in text
    assert text.count("msg_id=msg-concurrent-") == 25
    assert gate.evaluate(workboard) == []
