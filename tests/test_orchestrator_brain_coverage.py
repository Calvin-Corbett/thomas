...
from thomas.marketplace.specialists.reasoning import _history_recall_response
...
@pytest.mark.asyncio
async def test_classify_and_route_prefers_tools_for_explicit_tool_requests() -> None:
    brain = OrchestratorBrain(
        config=None,
        llm=_ChatDictLLM(),
        memory_engine=None,
        registry=_Registry(["reasoning", "coding", "tools"]),
    )

    routed = await brain._classify_and_route(
        "Use your tools to create a file in the current repo.",
        ConversationManager(),
        MemoryContext(),
    )
    assert routed.specialists == ["tools"]
    assert routed.parallel is False
    ...
    assert web_read.specialists == ["tools"]
    assert web_read.parallel is False
    assert "explicit file or tool request" in web_read.reasoning
...
@pytest.mark.asyncio
async def test_handle_actionable_fast_tools_route_skips_memory_and_synthesis(monkeypatch: pytest.MonkeyPatch) -> None:
    created_coordinators: list[_MemoryCoordinator] = []
    ...
    monkeypatch.setattr("thomas.marketplace.orchestrator.brain.MemoryCoordinator", _fake_memory_coordinator)
    brain = OrchestratorBrain(config=None, llm=None, memory_engine=None, registry=_Registry(["tools"]))
    dispatcher = _Dispatcher()

    async def _streaming_dispatch_single(**kwargs: object) -> DelegationResult:
        assert kwargs["stream_text_events"] is True
        memory_ctx = kwargs["memory_ctx"]
        assert isinstance(memory_ctx, MemoryContext)
        assert memory_ctx.working == ""
        assert memory_ctx.episodic == ""
        assert memory_ctx.semantic == ""
        await kwargs["dispatcher"].emit_text("direct tool answer")
        return DelegationResult(
            specialist_id="tools",
            content="direct tool answer",
            tool_calls=[{"type": "tool_result", "name": "write_file"}],
            iterations=1,
            tokens_used=3,
        )

    async def _unexpected_synthesise(**kwargs: object) -> str:
        raise AssertionError("fast tools route should not synthesise")

    monkeypatch.setattr(brain, "_dispatch_single", _streaming_dispatch_single)
    monkeypatch.setattr(brain, "_synthesise", _unexpected_synthesise)

    actionable = await brain._handle_actionable(
        session_id="sess-fast-tools",
        conversation=ConversationManager(),
        prompt="Use your tools to create a file in the current repo.",
        dispatcher=dispatcher,
        mode="auto",
        autonomy_level=3,
        token_economy="optimal",
    ...
    assert not dispatcher.memory_refreshes
    assert "".join(dispatcher.text_parts).startswith("Working on that")
    assert "".join(dispatcher.text_parts).count("direct tool answer") == 1
    assert (actionable.last_assistant_message() or "").endswith("direct tool answer")
    assert dispatcher.done_payloads[-1]["tool_calls"] == 1


@pytest.mark.asyncio
async def test_handle_actionable_strict_output_fast_tools_route_suppresses_ack(monkeypatch: pytest.MonkeyPatch) -> None:
    created_coordinators: list[_MemoryCoordinator] = []
    ...
    async def _streaming_dispatch_single(**kwargs: object) -> DelegationResult:
        assert kwargs["stream_text_events"] is True
        await kwargs["dispatcher"].emit_text("D:\\Desktop\\probe.txt\nSPEED_OK")
        return DelegationResult(
            specialist_id="tools",
            content="D:\\Desktop\\probe.txt\nSPEED_OK",
            tool_calls=[{"type": "tool_result", "name": "write_file"}],
            iterations=1,
            tokens_used=3,
        )
    ...
    async def _unexpected_synthesise(**kwargs: object) -> str:
        raise AssertionError("strict fast tools route should not synthesise")

    monkeypatch.setattr(brain, "_dispatch_single", _streaming_dispatch_single)
    monkeypatch.setattr(brain, "_synthesise", _unexpected_synthesise)

    actionable = await brain._handle_actionable(
        session_id="sess-fast-tools-strict",
        conversation=ConversationManager(),
        prompt=(
            "Use your tools to create the file D:\\Desktop\\probe.txt containing exactly SPEED_OK, "
    ...
    assert created_coordinators
    assert not dispatcher.memory_refreshes
    assert "".join(dispatcher.text_parts) == "D:\\Desktop\\probe.txt\nSPEED_OK"
    assert actionable.last_assistant_message() == "D:\\Desktop\\probe.txt\nSPEED_OK"
    assert dispatcher.done_payloads[-1]["tool_calls"] == 1


@pytest.mark.asyncio
async def test_handle_casual_fast_tools_route_uses_tools_specialist(monkeypatch: pytest.MonkeyPatch) -> None:
    created_coordinators: list[_MemoryCoordinator] = []
    ...
    async def _streaming_dispatch_single(**kwargs: object) -> DelegationResult:
        assert kwargs["specialist_id"] == "tools"
        assert kwargs["stream_text_events"] is True
        memory_ctx = kwargs["memory_ctx"]
        assert isinstance(memory_ctx, MemoryContext)
        assert memory_ctx.working == ""
...
def test_history_recall_response_prefers_substantive_prior_user_request() -> None:
    conversation = [
        ...
        {
            "role": "user",
            "content": "Research and make a PDF quiz pack with the answer key at the bottom.",
        },
        {"role": "assistant", "content": "I can do that."},
        {"role": "user", "content": "okay do it"},
    ]
    ...
    assert "Earlier in this chat, you asked me to" in reply
    assert "PDF quiz pack" in reply
    assert "okay do it" not in reply
...
def test_history_recall_response_handles_two_things_wording() -> None:
    ...
    reply = _history_recall_response(
        "What were the two things I asked right before this?",
        conversation,
    )
