...
class _FakeHttpxAsyncClient:
    def __init__(
        ...
        *,
        ...
        stream_lines: list[str] | None = None,
        get_json_payloads: list[dict] | None = None,
        status_code: int = 200,
    ) -> None:
        self._session_payload = dict(session_payload or {})
        self._stream_lines = list(stream_lines or [])
        self._get_json_payloads = list(get_json_payloads or [])
    ...
    async def get(self, url: str, headers: dict | None = None):
        ...
        payload = self._get_json_payloads.pop(0) if self._get_json_payloads else {}
        return _FakeHttpxResponse(status_code=self._status_code, json_payload=payload)
...
class TestAgenticBenchmark(unittest.TestCase):
    ...
    def test_should_use_coding_pipeline_for_job_type_or_prompt_signal(self):
        with patch.dict(os.environ, {"THOMAS_BENCHMARK_RUN_ID": ""}, clear=False):
            self.assertFalse(_should_use_coding_pipeline(job_type="benchmark", prompt="hello"))
        ...
        with patch.dict(os.environ, {"THOMAS_BENCHMARK_RUN_ID": "bench-run"}, clear=False):
            self.assertFalse(_should_use_coding_pipeline(job_type="coding", prompt="fix bug in app.py"))
        with patch.dict(
            os.environ,
            {
                "THOMAS_BENCHMARK_RUN_ID": "bench-run",
                "THOMAS_BENCHMARK_ALLOW_CODING_PIPELINE": "1",
            },
            clear=False,
        ):
            self.assertTrue(_should_use_coding_pipeline(job_type="coding", prompt="fix bug in app.py"))
    ...
    def test_embedded_task_skips_reviewer_when_decision_not_required(self):
        ...
        with (
            patch.dict(os.environ, {"THOMAS_BENCHMARK_RUN_ID": ""}, clear=False),
            patch("thomas.demo.agentic_benchmark_runners._run_single_agent_lane", new=_fake_single_lane),
            patch("thomas.demo.agentic_benchmark_runners._chat_json_lane", new=_unexpected_reviewer),
        ):
            result = asyncio.run(
                _run_thomas_embedded_task(
    ...
    def test_embedded_task_triggers_reviewer_on_low_confidence_candidate(self):
        ...
        with (
            patch.dict(os.environ, {"THOMAS_BENCHMARK_RUN_ID": ""}, clear=False),
            patch("thomas.demo.agentic_benchmark_runners._run_single_agent_lane", new=_fake_single_lane),
            patch("thomas.demo.agentic_benchmark_runners._chat_json_lane", new=_fake_chat_lane),
        ):
            result = asyncio.run(
                _run_thomas_embedded_task(
                    config=None,  # type: ignore[arg-type]
    ...
    def test_run_thomas_api_task_uses_reported_elapsed_and_usage_fallback(self):
        ...
        with patch("thomas.demo.agentic_benchmark_runners.httpx.AsyncClient", new=lambda *args, **kwargs: fake_client):
            result = asyncio.run(
                _run_thomas_api_task(
        ...
        self.assertTrue(fake_client.stream_calls[0]["url"].endswith("/api/v2/chat"))
    ...
    def test_run_thomas_api_task_waits_for_background_dispatch_completion(self):
        stream_lines = [
            json.dumps({"type": "text", "text": "I'm on it now. I'll keep it moving in the background."}),
            json.dumps({"type": "task_dispatched", "task_id": "task-123", "execution_id": "exec-123"}),
            json.dumps({"type": "done", "text": "I'm on it now. I'll keep it moving in the background."}),
        ]
        fake_client = _FakeHttpxAsyncClient(
            session_payload={"session_id": "sess-789"},
            stream_lines=stream_lines,
            get_json_payloads=[
                {
                    "session_id": "sess-789",
        ...
        with patch("thomas.demo.agentic_benchmark_runners.httpx.AsyncClient", new=lambda *args, **kwargs: fake_client):
            result = asyncio.run(
                _run_thomas_api_task(
                    ...
                    api_token="",
                    profile="local",
                    prompt="Use your tools to create a file.",
                    mode="auto",
                    token_economy="optimal",
                    max_iterations=None,
                )
        ...
        self.assertTrue(result["ok"])
        self.assertEqual(result["token_report"]["background_task"]["state"], "completed")
        self.assertEqual(
            fake_client.get_calls[0]["url"], "https://thomas.local/api/v2/chat/session/sess-789/delegations"
        )

    def test_run_task_track_entry_accepts_background_completion_when_only_ack_text_is_missing(self):
        artifact_root_rel = Path("runtime/agentic_bench/test-run")
        target_dir = self.tmp_path / artifact_root_rel / "thomas_os"
        target_dir.mkdir(parents=True, exist_ok=True)
        (target_dir / "task01.json").write_text('{"ok": true}\n', encoding="utf-8")
        ...
        task = {
            ...
            "success": {
                "response_contains": ["task01.json"],
                "required_files": ["{{artifact_dir}}/task01.json"],
                "required_file_contains": {"{{artifact_dir}}/task01.json": '"ok"'},
            },
        }
        args = type(
            "Args",
        ...
        async def _fake_api_task(**kwargs):
            _ = kwargs
            return {
                "ok": True,
                "text": "I'm on it now. I'll keep it moving in the background.",
                "error": "",
                ...
                "elapsed_seconds": 1.5,
                "setup_elapsed_seconds": 0.1,
                "first_token_seconds": 0.2,
                "first_text_delta_seconds": 0.2,
                "first_stream_event_seconds": 0.2,
                "stream_event_count": 5,
                "text_event_count": 1,
            }

        with patch("thomas.demo.agentic_benchmark._run_thomas_api_task", new=_fake_api_task):
            row = asyncio.run(
                _run_task_track_entry(
        ...
        self.assertTrue(row["record"]["success"])
        self.assertIn("background_completion_override=response_ack_only", row["record"]["notes"])
