...
def test_task_manager_bootstrap_starts_worker_when_missing(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setenv("THOMAS_CHAT_WORKER_POOL_SIZE", "2")
    bootstrap = SimpleNamespace(
        DEFAULT_TASK_MANAGER_AGENT="thomas",
        _is_task_manager_claimed=lambda workboard_path, agent: True,
        _spawn_task_manager_loop=lambda **kwargs: (True, {"pid": 111, "agent": kwargs["task_manager_agent"]}, None),
        _spawn_worker_loop=lambda **kwargs: (
            True,
            {
                "pid": 222 if kwargs["worker_agent"] == app_core._TASK_MANAGER_CHAT_WORKER else 333,
                "agent": kwargs["worker_agent"],
            },
            None,
        ),
    )
    monkeypatch.setattr(app_core, "_task_manager_loop_enabled", lambda: True)
    monkeypatch.setattr(app_core, "_find_task_manager_loop_pids", lambda: [])
    monkeypatch.setattr(app_core, "_find_task_manager_worker_pids", lambda agent=app_core._TASK_MANAGER_CHAT_WORKER: [])
    monkeypatch.setitem(sys.modules, "scripts.agent_bootstrap_claim", bootstrap)

    payload = app_core._ensure_task_manager_loop_started(tmp_path)

    assert payload["ok"] is True
    assert payload["reason"] == "spawned"
    assert payload["loop"]["pid"] == 111
    assert payload["worker_agents"] == [app_core._TASK_MANAGER_CHAT_WORKER, f"{app_core._TASK_MANAGER_CHAT_WORKER}-2"]
    assert [row["pid"] for row in payload["workers"]] == [222, 333]
    assert payload["worker_pids"] == [222, 333]
    assert payload["worker_agent"] == app_core._TASK_MANAGER_CHAT_WORKER


def test_task_manager_bootstrap_returns_running_when_loop_and_worker_exist(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setenv("THOMAS_CHAT_WORKER_POOL_SIZE", "2")
    monkeypatch.setattr(app_core, "_task_manager_loop_enabled", lambda: True)
    monkeypatch.setattr(app_core, "_find_task_manager_loop_pids", lambda: [111])
    monkeypatch.setattr(
        app_core,
        "_find_task_manager_worker_pids",
        lambda agent=app_core._TASK_MANAGER_CHAT_WORKER: [222] if agent == app_core._TASK_MANAGER_CHAT_WORKER else [333],
    )
    ...
    assert payload["loop_pids"] == [111]
    assert payload["worker_agents"] == [app_core._TASK_MANAGER_CHAT_WORKER, f"{app_core._TASK_MANAGER_CHAT_WORKER}-2"]
    assert payload["worker_pids"] == [222, 333]


def test_task_manager_process_regexes_match_full_windows_paths() -> None:
    worker_cmd = (
        r'D:\Workspaces\Thomas\.venv\Scripts\python.exe '
        r'D:\Workspaces\Thomas\scripts\workboard_worker.py '
        r'--workboard D:\Workspaces\Thomas\plans\thomas\WORKBOARD.md '
        r'--agent thomas-chat-worker --task-manager-agent thomas --cycles 0 --poll-seconds 2.0'
    )
    loop_cmd = (
        r'D:\Workspaces\Thomas\.venv\Scripts\python.exe '
        r'D:\Workspaces\Thomas\scripts\workboard_task_manager.py '
        r'--workboard D:\Workspaces\Thomas\plans\thomas\WORKBOARD.md '
        r'--task-manager-agent thomas --monitor --apply --cycles 0'
    )

    assert app_core._TASK_MANAGER_WORKER_CMD_RE.search(worker_cmd)
    assert app_core._TASK_MANAGER_LOOP_CMD_RE.search(loop_cmd)


def test_create_app_skips_background_bootstrap_in_benchmark_single_agent_mode(
    tmp_path: Path, patch_create_app_dependencies: None, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setitem(
        sys.modules,
        "thomas.core.engine_manager",
        SimpleNamespace(get_engine_manager=lambda: (_ for _ in ()).throw(AssertionError("engine manager should not start"))),
    )
    monkeypatch.setattr(
        app_core,
        "_ensure_task_manager_loop_started",
        lambda repo_root: (_ for _ in ()).throw(AssertionError("task manager bootstrap should not run")),
    )

    app = app_core.create_app(_base_config(tmp_path))

    assert app["task_manager_loop_bootstrap"]["reason"] == "benchmark_single_agent"
