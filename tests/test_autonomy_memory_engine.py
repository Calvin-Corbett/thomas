...
def test_add_event_returns_v2_episode_id_when_legacy_is_disabled(tmp_path) -> None:  # noqa: ANN001
    mem = AutonomyMemoryEngine(_cfg(tmp_path), enable_legacy=False, enable_v2=True)
    mem.start()
    try:
        event_id = mem.add_event("telegram:1", "user_message", "remember that I need a quiz pack")

        assert isinstance(event_id, int)
        assert event_id > 0

        ctx = mem.retrieve("quiz pack", thread="telegram:1", budget=900, mode="auto")
        assert "quiz pack" in ctx.text.lower()
    finally:
        mem.close()
...
def test_profile_memory_can_be_enabled_or_disabled_per_thread(tmp_path) -> None:  # noqa: ANN001
    ...
    mem.start()
    try:
        mem.pin("user.name", "Example User")
        ...
        assert "Example User" in with_profile.text.lower()
        ...
        assert "Example User" not in without_profile.text.lower()
    finally:
        mem.close()
