...
from thomas.marketplace.watcher.api import patch_watcher_section
from thomas.marketplace.watcher.ingest import WatcherConfig, ingest_file, is_supported_file
...
def test_supported_file_override_exts(tmp_path: Path):
    ...
    assert (
        is_supported_file(str(f), include_exts=["bin"]) is False
    )  # must include dot in override list after load/normalize
...
def test_ingest_text_file_indexes_and_persistent_dedup(monkeypatch, tmp_path: Path):
    ...
    import thomas.marketplace.watcher.ingest as ingest_mod
