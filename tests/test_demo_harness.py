...
class TestDemoHarness(unittest.TestCase):
    ...
    def test_compute_summary_zeroes_weighted_score_for_invalid_rows(self):
        pack = {
            "tasks": [{"id": "t1", "title": "A", "prompt": "P"}],
            "weights": {"success_rate": 0.6, "speed": 0.15, "follow_up": 0.05, "quality": 0.2},
            "quality_scale": {"min": 1, "max": 5},
        }
        records = [
            {
                "task_id": "t1",
                "competitor": "baseline_raw",
                "success": False,
                "elapsed_seconds": 1,
                "follow_up_prompts": 0,
                "quality_score": 1,
        ...
        summary = compute_summary(pack, records)
        metrics = summary["competitors"]["baseline_raw"]
        self.assertEqual(metrics["invalid_task_count"], 1)
        self.assertEqual(metrics["validity_rate"], 0.0)
        self.assertEqual(metrics["weighted_score"], 0.0)

    def test_compute_summary_tracks_artifact_and_timeout_metrics(self):
        pack = {
            "tasks": [{"id": "t1", "title": "A", "prompt": "P"}],
            "weights": {"success_rate": 0.6, "speed": 0.15, "follow_up": 0.05, "quality": 0.2},
            "quality_scale": {"min": 1, "max": 5},
        ...
        self.assertEqual(metrics["artifact_success_count"], 1)
        self.assertEqual(metrics["artifact_success_rate"], 1.0)
        self.assertEqual(metrics["response_confirmed_count"], 0)
        self.assertEqual(metrics["timeout_count"], 1)
        self.assertEqual(metrics["runaway_guard_count"], 0)
        self.assertEqual(metrics["runner_error_count"], 1)
    ...
    def test_write_run_artifacts_outputs_expected_files(self):
        ...
        self.assertIn("## Execution Validity", report_text)
        self.assertIn("## Metric Panels", report_text)
        self.assertIn("## Ranking (Secondary)", report_text)
