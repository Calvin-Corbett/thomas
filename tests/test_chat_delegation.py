...
class TestChatDelegation(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_bridge_availability_does_not_change_task_manager_path(self):
        emit_event = AsyncMock()
        expected = {
            "execution_id": "exec-task-manager",
            "task_id": "task-task-manager",
            "session_id": "sess-max",
            "backend_type": "task_manager",
            "state": "queued",
    ...
    def test_session_active_delegations_filters_normalizes_and_builds_digest(self):
        rows = [
            ...
            {"execution_id": "exec-sid", "session_id": "sess-1", "updated_at": "2026-03-30T00:00:00Z"},
        ]
        ...
        def _get_execution(execution_id: str, root: Path):  # noqa: ANN001
            ...
            if execution_id == "exec-sid":
                return {
                    "execution_id": execution_id,
                    "session_id": "sess-1",
                    "backend_type": chat_delegation.TASK_MANAGER_BACKEND,
                    "state": "claimed",
                    "progress_summary": "Task claimed by thomas-chat-worker-2.",
                    "claimed_owner": "thomas-chat-worker-2",
        ...
        self.assertEqual([row["execution_id"] for row in delegations], ["exec-sid", "exec-new", "exec-old"])
        self.assertIn("Background work in this chat:", digest)
        self.assertIn("worker [claimed via task manager]", digest)

    async def test_start_background_delegation_uses_task_manager_for_tools_request(self):
        emit_event = AsyncMock()
        expected = {
    ...
    async def test_start_background_delegation_is_disabled_in_benchmark_single_agent_mode(self):
        emit_event = AsyncMock()
        with patch("thomas.server.chat_delegation.benchmark_single_agent_enabled", return_value=True):
            result = await chat_delegation.start_background_delegation(
                {},
