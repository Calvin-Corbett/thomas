...
class TestPolicyRules(unittest.TestCase):
    ...
    def test_protected_internal_write_requires_native_auth(self):
        cfg = PolicyConfig()
        engine = PolicyEngine.from_config(cfg)
        repo_root = __import__("pathlib").Path(__file__).resolve().parents[1]
        ctx = PolicyContext(
            tool_name="fs.write_file",
            args={"path": str(repo_root / "agent_safety.toml")},
            cwd=str(repo_root),
            sandbox_root=str(repo_root),
            iteration=1,
            conversation_summary="update protected policy file",
        )
        dec = engine.evaluate(ctx)
        self.assertEqual(dec.type.value, "REQUIRE_APPROVAL")
        self.assertTrue(bool(dec.meta.get("native_auth", False)))
        self.assertEqual(str(dec.meta.get("repo_path") or ""), "agent_safety.toml")
