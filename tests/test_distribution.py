...
class TestAutoUpdateSkips(unittest.TestCase):
    """Test that auto-update correctly skips in dev/disabled scenarios."""
    ...
    def test_skips_when_env_disabled(self):
        ...
        with patch.dict(os.environ, {"THOMAS_NO_AUTO_UPDATE": "1"}):
            with patch("thomas.cli.commands.updater._is_dev_install", return_value=False):
                result = check_and_auto_update(silent=True)
                self.assertIsNone(result)
...
class TestFetchLatestVersion(unittest.TestCase):
    ...
    def test_returns_none_on_failure(self):
        from thomas.cli.commands.updater import _fetch_latest_version

        with patch("urllib.request.urlopen", side_effect=Exception("Network error")):
            result = _fetch_latest_version(timeout=1.0)
            self.assertIsNone(result)
