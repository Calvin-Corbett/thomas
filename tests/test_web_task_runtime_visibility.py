...
def test_mission_runtime_panel_and_stream_contract() -> None:
    ...
    assert "status === 'executing'" in js
    assert "status === 'requested'" in js
    assert "status === 'classified'" in js
    assert "status === 'claimed'" in js
    assert "safeString(job?.summary)" in js
    assert "safeString(job?.progress_summary)" in js
    assert "No jobs for this chat session yet." in js
    assert "current chat session" in js
    assert "jobsParams.set('session_id', preferredSessionId);" in js
...
def test_chat_presence_runtime_uses_platform_graph_and_motion_debug() -> None:
    ...
    assert "Open Mission Control" in js
