...
def test_dispatch_workers_releases_ready_and_claims_lane(tmp_path: Path, capsys) -> None:
    ...
    assert payload["released_workers"] == []
    assert payload["claim_errors"] == []
    assert payload["claimed_workers"] == []
    assert payload["release_errors"][0]["agent"] == "Codex 3-Worker-1"
    assert "presence gate requires override" in payload["release_errors"][0]["error"]
    assert payload["active_worker_count"] == 1
    assert "summary=[WIP][AUTO-01] task-a: server lane" not in text
    assert gate.evaluate(workboard) == []
