...
import scripts.workboard_task_manager_messages as msg_mod
...
def test_dispatch_idle_agents_once_assigns_online_worker(tmp_path: Path, monkeypatch) -> None:
    workboard = _write_workboard(
        tmp_path,
        up_for_grabs_block=(
            "- task_id=chat-please-make-a-note-123abc; scope=chat/chat-please-make-a-note-123abc; "
            "summary=please make a note that says verify api chat route after restart; reported_by=chat_dispatch"
        ),
    )

    monkeypatch.setattr(
        msg_mod.workboard_message,
        "list_messages",
        lambda _path: (
            True,
            {
                "messages": [
                    {
                        ...
                        "created_at": "3026-04-04T02:41:55+00:00",
                        "updated_at": "3026-04-04T02:41:55+00:00",
                    }
                ]
            },
        ),
    ...
    assert "task_id=chat-please-make-a-note-123abc; agent=note-worker-1;" in text
    assert "task_id=chat-please-make-a-note-123abc; scope=chat;" not in text
    assert gate.evaluate(workboard) == []


def test_dispatch_idle_agents_once_spreads_work_across_longest_idle_workers(tmp_path: Path, monkeypatch) -> None:
    workboard = _write_workboard(
        tmp_path,
        up_for_grabs_block=(
            "- task_id=chat-task-a; scope=chat/chat-task-a; summary=first queued task; reported_by=chat_dispatch\n"
            "- task_id=chat-task-b; scope=chat/chat-task-b; summary=second queued task; reported_by=chat_dispatch\n"
            "- task_id=chat-task-c; scope=chat/chat-task-c; summary=third queued task; reported_by=chat_dispatch"
        ),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda _path: (
            True,
            {
                "messages": [
                    {
                        "from": "thomas-chat-worker",
                        "to": "thomas",
                        "task_id": "none",
                        "kind": "ping",
                        "summary": "worker heartbeat: waiting for assignment",
                        "created_at": "3026-04-04T02:41:57+00:00",
                        "updated_at": "3026-04-04T02:41:57+00:00",
                    },
                    {
                        "from": "thomas-chat-worker-2",
                        "to": "thomas",
                        "task_id": "none",
                        "kind": "ping",
                        "summary": "worker heartbeat: waiting for assignment",
                    ...
                    {
                        "from": "thomas-chat-worker-3",
                        "to": "thomas",
                        "task_id": "none",
                        "kind": "ping",
                        "summary": "worker heartbeat: waiting for assignment",
                        "created_at": "3026-04-04T02:41:56+00:00",
                        "updated_at": "3026-04-04T02:41:56+00:00",
                    },
                ]
            },
        ),
    ...
    assert [assignment["agent"] for assignment in payload["assignments"]] == [
        "thomas-chat-worker-2",
        "thomas-chat-worker-3",
        "thomas-chat-worker",
    ]
    assert "task_id=chat-task-a; agent=thomas-chat-worker-2;" in text
    assert "task_id=chat-task-b; agent=thomas-chat-worker-3;" in text
    assert "task_id=chat-task-c; agent=thomas-chat-worker;" in text
    assert gate.evaluate(workboard) == []


def test_monitor_loop_runs_requested_cycles(monkeypatch, tmp_path: Path) -> None:
    workboard = _write_workboard(tmp_path)
    dispatch_calls: list[int] = []

    monkeypatch.setattr(msg_mod, "_ping_silent_active_agents", lambda **kwargs: (True, {"silent_task_count": 0}))
    monkeypatch.setattr(
        msg_mod.workboard_task_manager_reactivate,
        "_auto_start_all_claimed_agents",
        lambda **kwargs: (True, {"assigned_count": 0}),
    )
    monkeypatch.setattr(
        msg_mod,
        "dispatch_idle_agents_once",
        lambda **kwargs: (dispatch_calls.append(1) or True, {"assigned_count": 0}),
    )

    ok, payload = msg_mod._monitor_loop(
        cycles=2,
        workboard_path=workboard,
        task_manager_agent="thomas",
        max_agent_silence_minutes=5,
        max_dispatch_per_cycle=1,
        online_lookback_minutes=30,
        run_swarm_recovery=False,
        run_auto_start=False,
        run_idle_dispatch=True,
        require_claims_to_have_active_task=False,
        interval_seconds=0,
        now_seed=None,
        apply=True,
...
def test_sweep_inactive_uses_agent_activity_when_claim_line_is_fresh(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(mod.workboard_claim, "release", lambda *args, **kwargs: (True, "released"))
    text = workboard.read_text(encoding="utf-8")
