...
- `release/oss-launch` is the release lane only.
- `publish-clean` is the public-sanitized publish lane only.

Required rules for all agents:
1. Use only the explicitly assigned worktree path for a task.
2. If no worktree is specified, use the registered `master` worktree.
